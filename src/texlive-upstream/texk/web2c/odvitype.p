...
{:4}const{5:}maxfonts=500;maxwidths=100000;linelength=79;stacksize=100;
...
{:21}var{10:}xord:array[ASCIIcode]of ASCIIcode;
...
widthptr:0..maxwidths;{:30}{33:}inwidth:array[0..65535]of integer;
...
count:array[0..9]of integer;{:42}{57:}inpostamble:boolean;
{:57}{67:}textptr:0..linelength;
...
{:101}{108:}k,m,n,p,q:integer;{:108}{122:}showopcodes:cinttype;
{:122}{124:}dviname:constcstring;{:124}{112:}procedure parsearguments;
const noptions=8;var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
endnum:^char;begin{113:}currentoption:=0;
...
{:113}{114:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:114}{115:}longoptions[currentoption].name:='output-level';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
outmode:=4;{:115}{116:}longoptions[currentoption].name:='page-start';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:116}{118:}longoptions[currentoption].name:='max-pages';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
maxpages:=1000000;{:118}{119:}longoptions[currentoption].name:='dpi';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
resolution:=300.0;
{:119}{120:}longoptions[currentoption].name:='magnification';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
newmag:=0;{:120}{121:}longoptions[currentoption].name:='show-opcodes';
...
newmag:=0;{:121}{123:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:123};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('odvitype');
...
begin newmag:=atou(optarg);end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'odvitype',
': Need exactly one file argument.');usage('odvitype');end;
dviname:=extendfilename(cmdline(optind),'dvi');end;
{:112}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'odvitype');parsearguments;
write(stdout,'This is ODVItype, Version 1.5');
...
fontec[maxfonts]:=0;{:31}{58:}inpostamble:=false;{:58}{68:}textptr:=0;
...
end else begin fullname:=kpsefindofm(curname);
if fullname then begin tfmfile:=fopen(fullname,FOPENRBINMODE);
end else begin tfmfile:=nil;end;end;end;{:23}{26:}procedure readtfmword;
...
nco:integer;extrawords:integer;ofmlevel:integer;j,kprime:integer;
begin{35:}nco:=0;extrawords:=0;readtfmword;
if(b0*256+b1)<>0 then begin ofmlevel:=-1;lh:=b2*256+b3;readtfmword;
fontbc[nf]:=b0*256+b1;fontec[nf]:=b2*256+b3;
if fontec[nf]<fontbc[nf]then fontbc[nf]:=fontec[nf]+1;
if widthptr+fontec[nf]-fontbc[nf]+1>maxwidths then begin writeln(stdout,
'---not loaded, ODVItype needs larger width table');goto 9998;end;
...
goto 9997;end;end else begin ofmlevel:=b2*256+b3;readtfmword;
readtfmword;lh:=((b0*256+b1)*256+b2)*256+b3;readtfmword;
fontbc[nf]:=((b0*256+b1)*256+b2)*256+b3;readtfmword;
fontec[nf]:=((b0*256+b1)*256+b2)*256+b3;
if fontec[nf]<fontbc[nf]then fontbc[nf]:=fontec[nf]+1;
if widthptr+fontec[nf]-fontbc[nf]+1>maxwidths then begin writeln(stdout,
'---not loaded, ODVItype needs larger width table');goto 9998;end;
wp:=widthptr+fontec[nf]-fontbc[nf]+1;readtfmword;
nw:=((b0*256+b1)*256+b2)*256+b3;if(nw=0)or(nw>65536)then goto 9997;
for k:=1 to 8 do begin if eof(tfmfile)then goto 9997;readtfmword;end;
if ofmlevel=1 then begin readtfmword;nco:=((b0*256+b1)*256+b2)*256+b3;
readtfmword;readtfmword;extrawords:=(((b0*256+b1)*256+b2)*256+b3)div 2;
for k:=1 to 12 do begin if eof(tfmfile)then goto 9997;readtfmword;end;
end;for k:=1 to lh do begin if eof(tfmfile)then goto 9997;readtfmword;
if k=1 then if b0<128 then tfmchecksum:=((b0*256+b1)*256+b2)*256+b3 else
tfmchecksum:=(((b0-256)*256+b1)*256+b2)*256+b3 else if k=2 then if b0<
128 then tfmdesignsize:=round(tfmconv*(((b0*256+b1)*256+b2)*256+b3))else
goto 9997;end;if ofmlevel=1 then for k:=1 to(nco-29-lh)do readtfmword;
end{:35};
{36:}case ofmlevel of-1:begin if wp>0 then for k:=widthptr to wp-1 do
begin readtfmword;if b0>nw then goto 9997;width[k]:=b0;end;end;
0:begin if wp>0 then for k:=widthptr to wp-1 do begin readtfmword;
if(b0*256+b1)>nw then goto 9997;width[k]:=(b0*256+b1);readtfmword;end;
end;1:begin if wp>0 then begin k:=widthptr;
while k<wp do begin readtfmword;if(b0*256+b1)>nw then goto 9997;
width[k]:=(b0*256+b1);readtfmword;readtfmword;kprime:=k+(b0*256+b1);
for j:=k+1 to kprime do width[j]:=width[k];k:=kprime+1;
for j:=1 to extrawords do readtfmword;end end end;end;{:36};
...
goto 9999;9997:writeln(stdout,'---not loaded, font metric file is bad');
...
{:44}{59:}procedure definefont(e:integer);var f:0..maxfonts;p:integer;
...
if f=nf then{62:}begin{66:}r:=fontname[nf+1]-fontname[nf];
curname:=xmallocarray(char,r);
for k:=fontname[nf]to fontname[nf+1]do begin curname[k-fontname[nf]]:=
xchr[names[k]];end;curname[r]:=0;{:66};opentfmfile;
...
end{:63};end;if outmode=0 then writeln(stdout,' ');
if tfmfile then xfclose(tfmfile,curname);free(curname);
end{:62}else{60:}begin if fontchecksum[f]<>c then writeln(stdout,
...
write(stdout,a:1,': ','y',o-161:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 44;
end;166,167,168,169,170:begin z:=p;
if abs(p)>=5*fontspace[curfont]then vv:=round(conv*(v+p))else vv:=vv+
round(conv*(p));if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','z',o-166:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 44;
end;
...
write(stdout,a:1,': ','setchar',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 41;
end{:88}else case o of 128,129,130,131:begin if outmode>0 then begin
flushtext;showing:=true;write(stdout,a:1,': ','set',o-127:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 41;
end;133,134,135,136:begin if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','put',o-132:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 41;
...
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 42;
end;{83:}138:begin if outmode>1 then begin showing:=true;
...
41:{89:}if(p<fontbc[curfont])or(p>fontec[curfont])then q:=2147483647
...
45:{93:}if showing then if outmode>2 then begin writeln(stdout,' ');
...
{:99}procedure skippages(bopseen:boolean);label 9999;var p:integer;
...
{96:}repeat if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;k:=getbyte;
...
139,247,248,249,250,251,252,253,254,255:begin writeln(stderr,
'Bad DVI file: ','illegal command at byte ',curloc-1:1,'!');uexit(1);
end;others:end;until k=140;{:96};bopseen:=false;end;9999:end;
...
if outmode<4 then{104:}begin if maxv+99<maxvsofar then writeln(stdout,
...
printreal(newmag/1000.0,8,3);writeln(stdout,'')end{:56};
{109:}opendvifile;p:=getbyte;
...
writeln(stdout,' pixels per DVI unit'){:110};p:=getbyte;
write(stdout,'''');while p>0 do begin p:=p-1;
write(stdout,xchr[getbyte]);end;writeln(stdout,'''');
afterpre:=curloc{:109};if outmode=4 then begin{100:}n:=dvilength;
