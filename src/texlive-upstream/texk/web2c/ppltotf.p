{2:}program PLtoTF(plfile,tfmfile,output);const{3:}bufsize=3000;
maxheaderbytes=1000;maxparamwords=254;maxligsteps=32510;maxkerns=5000;
hashsize=32579;{:3}{158:}maxkanji=7237;yokoidnumber=11;tateidnumber=9;
{:158}type{17:}byte=0..255;ASCIIcode=32..127;
{:17}{57:}fourbytes=record b0:byte;b1:byte;b2:byte;b3:byte;end;
{:57}{61:}fixword=integer;{:61}{68:}headerindex=0..maxheaderbytes;
indx=0..32767;{:68}{71:}pointer=0..1032;{:71}var{5:}plfile:text;
{:5}{15:}tfmfile:packed file of 0..255;
{:15}{18:}xord:array[0..255]of ASCIIcode;xchr:array[0..255]of byte;
...
buffer:array[1..bufsize]of 0..255;inputhasended:boolean;
{:23}{25:}charsonline:0..8;perfect:boolean;{:25}{30:}curchar:ASCIIcode;
{:30}{36:}start:array[1..97]of 0..700;
dictionary:array[0..700]of ASCIIcode;startptr:0..97;dictptr:0..700;
{:36}{38:}curname:array[1..20]of ASCIIcode;namelength:0..20;
nameptr:0..97;{:38}{39:}nhash:array[0..100]of 0..97;curhash:0..100;
{:39}{44:}equiv:array[0..97]of byte;curcode:byte;
{:44}{58:}curbytes:fourbytes;
{:58}{65:}fractiondigits:array[1..7]of integer;
...
checksumspecified:boolean;bchar:0..256;
...
charremainder:array[0..256]of 0..65535;{:72}{76:}nextd:fixword;
{:76}{79:}indexvar:array[pointer]of byte;excess:byte;{:79}{81:}c:byte;
...
h,hh:0..hashsize;tt:indx;xligcycle,yligcycle:0..256;{:118}{129:}bc:byte;
ec:byte;lh:byte;lf:0..32767;notfound:boolean;tempwidth:fixword;
{:129}{132:}j:0..maxheaderbytes;p:pointer;q:1..4;
parptr:0..maxparamwords;
{:132}{138:}labeltable:array[0..256]of record rr:-1..32767;cc:byte;end;
labelptr:0..256;sortptr:0..256;lkoffset:0..256;t:0..32767;
extralocneeded:boolean;{:138}{152:}verbose:cinttype;
{:152}{157:}tfmname,plname:constcstring;{:157}{159:}fileformat:0..4;
kanjitype:array[0..maxkanji]of-1..256;kanjitypeindex:0..maxkanji;
nt:integer;glue:array[0..768]of fixword;ng:integer;
{:159}{148:}procedure parsearguments;const noptions=5;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{153:}verbose:=false;{:153};{149:}currentoption:=0;
...
{:149}{150:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:150}{151:}longoptions[currentoption].name:='verbose';
...
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'ppltotf'
,': Need one or two file arguments.');usage('ppltotf');end;
plname:=extendfilename(cmdline(optind),'pl');
if optind+2=argc then begin tfmname:=extendfilename(cmdline(optind+1),
'tfm');
end else begin tfmname:=basenamechangesuffix(plname,'.pl','.tfm');end;
end;{:148}procedure initialize;var{19:}k:integer;{:19}{40:}h:0..100;
{:40}{69:}d:headerindex;{:69}{73:}c:byte;
{:73}begin kpsesetprogramname(argv[0],'ppltotf');initkanji;
parsearguments;{6:}reset(plfile,plname);
if verbose then begin write(stderr,'This is pPLtoTF, Version 3.6-p1.8');
writeln(stderr,versionstring);
writeln(stderr,'process kanji code is ',conststringcast(getencstring),
'.');end;{:6}{16:}rewritebin(tfmfile,tfmname);
{:16}{20:}for k:=0 to 31 do xchr[k]:='?';for k:=32 to 255 do xchr[k]:=k;
...
xord['{']:=123;xord['|']:=124;xord['}']:=125;xord['~']:=126;
{:20}{22:}line:=0;goodindent:=0;indent:=0;level:=0;{:22}{24:}limit:=0;
loc:=0;leftln:=true;rightln:=true;inputhasended:=false;
...
memory[3]:=0;link[3]:=0;memory[4]:=0;link[4]:=0;memptr:=4;
{:74}{119:}hashptr:=0;yligcycle:=256;for k:=0 to hashsize do hash[k]:=0;
{:119}{160:}fileformat:=0;
for kanjitypeindex:=0 to maxkanji do kanjitype[kanjitypeindex]:=-1;
ng:=0;{:160}end;{:2}{27:}procedure showerrorcontext;var k:0..bufsize;
...
inputhasended:=true;
end else begin limit:=inputline2(plfile,ustringcast(buffer),limit+1,
bufsize-1)-1;buffer[limit+1]:=' ';rightln:=eoln(plfile);
if rightln then begin limit:=limit+1;buffer[limit+1]:=' ';end;
...
goodindent:=0;indent:=0;end;end;end{:29};end;end;
{:28}{31:}procedure getkeywordchar;
...
showerrorcontext;end;curchar:=32;end;
{:33}{35:}procedure finishtheproperty;begin while curchar=32 do getnext;
...
var k:0..20;j:0..700;notfound:boolean;begin{43:}curhash:=curname[1];
...
showerrorcontext;end;curcode:=equiv[nameptr];end;
{:49}{51:}function getbyte:byte;var acc:integer;t:ASCIIcode;
...
if acc>255 then begin begin begin if charsonline>0 then writeln(stderr,
' ');write(stderr,'This value shouldn''t exceed 255');showerrorcontext;
end;repeat getnext until(curchar=40)or(curchar=41);end;acc:=0;
curchar:=32;end else getnext;end;
begin if(curchar>41)or(curchar<40)then loc:=loc-1;end;
end{:53}else if t=79 then{54:}begin while(curchar>=48)and(curchar<=55)do
begin acc:=acc*8+curchar-48;
if acc>255 then begin begin begin if charsonline>0 then writeln(stderr,
' ');write(stderr,'This value shouldn''t exceed ''377');
...
if acc>255 then begin begin begin if charsonline>0 then writeln(stderr,
' ');write(stderr,'This value shouldn''t exceed "FF');showerrorcontext;
end;repeat getnext until(curchar=40)or(curchar=41);end;acc:=0;
curchar:=32;end else getnext;end;
begin if(curchar>41)or(curchar<40)then loc:=loc-1;end;
...
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
acc:=0;end;
end{:56}else begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'You need "C" or "D" or "O" or "H" or "F" here');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
curchar:=32;getbyte:=acc;end;{:51}{59:}procedure getfourbytes;
var c:integer;r:integer;q:integer;begin repeat getnext;
until curchar<>32;r:=0;curbytes.b0:=0;curbytes.b1:=0;curbytes.b2:=0;
curbytes.b3:=0;
if curchar=72 then r:=16 else if curchar=79 then r:=8 else begin begin
if charsonline>0 then writeln(stderr,' ');
write(stderr,'An octal ("O") or hex ("H") value is needed here');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
...
(stderr,' ');write(stderr,'Illegal digit');showerrorcontext;end;
...
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
acc:=0;curchar:=32;end else getnext;end{:64};intpart:=acc;acc:=0;
...
write(stderr,'Memory overflow: more than 1028 widths, etc');
showerrorcontext;end;
writeln(stderr,'Congratulations! It''s hard to make this error.');
...
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
{:83}{86:}procedure readfourbytes(l:headerindex);begin getfourbytes;
...
showerrorcontext;end;end;end;{:96}{107:}procedure printoctal(c:byte);
begin write(stderr,'''',(c div 64):1,((c div 8)mod 8):1,(c mod 8):1);
end;{:107}{121:}function hashinput(p,c:indx):boolean;label 30;
var cc:0..3;zz:0..255;y:0..255;key:integer;t:integer;
...
{:123}{124:}function f(h,x,y:indx):indx;begin case classvar[h]of 0:;
...
end;4:begin xligcycle:=x;yligcycle:=y;ligz[h]:=257;classvar[h]:=0;end;
end;f:=ligz[h];end;{:124}{136:}procedure outscaled(x:fixword);
var n:byte;m:0..65535;
...
putbyte(m div 256,tfmfile);putbyte(m mod 256,tfmfile);end;
{:136}{146:}{175:}function getnextraw:byte;
begin while loc=limit do fillbuffer;loc:=loc+1;getnextraw:=buffer[loc];
if multistrlen(ustringcast(buffer),loc+2,loc)=2 then curchar:=32 else
curchar:=xord[buffer[loc]];end;function todig(ch:byte):byte;
begin if(ch>=65)and(ch<=70)then todig:=ch-55 else if(ch>=48)and(ch<=57)
then todig:=ch-48 else begin begin begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'This expression is out of JIS-code encoding.');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
todig:=0;end;end;procedure printjishex(jiscode:integer);
var dig:array[0..4]of byte;i:byte;begin dig[0]:=Hi(jiscode)div 16;
dig[1]:=Hi(jiscode)mod 16;dig[2]:=Lo(jiscode)div 16;
dig[3]:=Lo(jiscode)mod 16;
for i:=0 to 3 do if dig[i]<10 then write(stderr,dig[i])else case dig[i]
of 10:write(stderr,'A');11:write(stderr,'B');12:write(stderr,'C');
13:write(stderr,'D');14:write(stderr,'E');15:write(stderr,'F');end;end;
function validjiscode(cx:integer):boolean;
var firstbyte,secondbyte:integer;begin validjiscode:=true;
firstbyte:=cx div 256;secondbyte:=cx mod 256;
if(firstbyte<33)or((firstbyte>40)and(firstbyte<48))or(firstbyte>116)then
validjiscode:=false;
if(secondbyte<33)or(secondbyte>126)then validjiscode:=false;end;
function jistoindex(jis:integer):integer;
var firstbyte,secondbyte:integer;begin firstbyte:=jis div 256-33;
secondbyte:=jis mod 256-33;
if firstbyte<8 then jistoindex:=firstbyte*94+secondbyte else jistoindex
:=(firstbyte-7)*94+secondbyte;end;
function indextojis(ix:integer):integer;
begin if ix<=8*94-1 then indextojis:=(ix div 94+33)*256+(ix mod 94+33)
else indextojis:=((ix+7*94)div 94+33)*256+((ix+7*94)mod 94+33);end;
function getkanji:integer;var ch:byte;cx,jiscode:integer;
begin repeat ch:=getnextraw;until ch<>' ';
if ch=')'then begin loc:=loc-1;jiscode:=0;
end else if(ch='J')or(ch='j')then begin repeat ch:=getnextraw;
until ch<>' ';cx:=todig(xord[ch])*4096;loc:=loc+1;ch:=xord[buffer[loc]];
cx:=cx+todig(ch)*256;loc:=loc+1;ch:=xord[buffer[loc]];
cx:=cx+todig(ch)*16;loc:=loc+1;ch:=xord[buffer[loc]];cx:=cx+todig(ch);
jiscode:=toDVI(fromJIS(cx));curchar:=ch;
if not validjiscode(jiscode)then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'jis code ',jiscode:1,' is invalid');
showerrorcontext;end;
end else if multistrlen(ustringcast(buffer),loc+2,loc)=2 then begin
jiscode:=toDVI(fromBUFF(ustringcast(buffer),loc+2,loc));loc:=loc+1;
curchar:=32;
if not validjiscode(jiscode)then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'jis code ',jiscode:1,' is invalid');
showerrorcontext;end;end else jiscode:=-1;getkanji:=jiscode;end;
{:175}procedure paramenter;begin{48:}namelength:=5;curname[16]:=83;
...
curname[11]:=79;curname[12]:=80;curname[13]:=83;curname[14]:=80;
curname[15]:=65;curname[16]:=67;curname[17]:=73;curname[18]:=78;
curname[19]:=71;curname[20]:=53;entername(33);{:48}{164:}namelength:=10;
curname[11]:=69;curname[12]:=88;curname[13]:=84;curname[14]:=82;
curname[15]:=65;curname[16]:=83;curname[17]:=80;curname[18]:=65;
curname[19]:=67;curname[20]:=69;entername(27);namelength:=12;
curname[9]:=69;curname[10]:=88;curname[11]:=84;curname[12]:=82;
curname[13]:=65;curname[14]:=83;curname[15]:=84;curname[16]:=82;
curname[17]:=69;curname[18]:=84;curname[19]:=67;curname[20]:=72;
entername(28);namelength:=11;curname[10]:=69;curname[11]:=88;
curname[12]:=84;curname[13]:=82;curname[14]:=65;curname[15]:=83;
curname[16]:=72;curname[17]:=82;curname[18]:=73;curname[19]:=78;
curname[20]:=75;entername(29);{:164};end;procedure nameenter;
...
curname[19]:=69;curname[20]:=82;entername(20);namelength:=6;
curname[15]:=67;curname[16]:=72;curname[17]:=65;curname[18]:=82;
curname[19]:=87;curname[20]:=68;entername(51);namelength:=6;
curname[15]:=67;curname[16]:=72;curname[17]:=65;curname[18]:=82;
curname[19]:=72;curname[20]:=84;entername(52);namelength:=6;
curname[15]:=67;curname[16]:=72;curname[17]:=65;curname[18]:=82;
curname[19]:=68;curname[20]:=80;entername(53);namelength:=6;
curname[15]:=67;curname[16]:=72;curname[17]:=65;curname[18]:=82;
curname[19]:=73;curname[20]:=67;entername(54);namelength:=10;
curname[11]:=78;curname[12]:=69;curname[13]:=88;curname[14]:=84;
curname[15]:=76;curname[16]:=65;curname[17]:=82;curname[18]:=71;
curname[19]:=69;curname[20]:=82;entername(55);namelength:=7;
curname[14]:=86;curname[15]:=65;curname[16]:=82;curname[17]:=67;
curname[18]:=72;curname[19]:=65;curname[20]:=82;entername(56);
namelength:=3;curname[18]:=84;curname[19]:=79;curname[20]:=80;
entername(57);namelength:=3;curname[18]:=77;curname[19]:=73;
curname[20]:=68;entername(58);namelength:=3;curname[18]:=66;
curname[19]:=79;curname[20]:=84;entername(59);namelength:=3;
curname[18]:=82;curname[19]:=69;curname[20]:=80;entername(60);
namelength:=3;curname[18]:=69;curname[19]:=88;curname[20]:=84;
entername(60);namelength:=7;curname[14]:=67;curname[15]:=79;
...
entername(70);namelength:=4;curname[17]:=83;curname[18]:=84;
curname[19]:=79;curname[20]:=80;entername(71);namelength:=4;
curname[17]:=83;curname[18]:=75;curname[19]:=73;curname[20]:=80;
entername(72);namelength:=3;curname[18]:=75;curname[19]:=82;
curname[20]:=78;entername(73);namelength:=3;curname[18]:=76;
curname[19]:=73;curname[20]:=71;entername(74);namelength:=4;
curname[17]:=47;curname[18]:=76;curname[19]:=73;curname[20]:=71;
entername(76);namelength:=5;curname[16]:=47;curname[17]:=76;
curname[18]:=73;curname[19]:=71;curname[20]:=62;entername(80);
namelength:=4;curname[17]:=76;curname[18]:=73;curname[19]:=71;
curname[20]:=47;entername(75);namelength:=5;curname[16]:=76;
curname[17]:=73;curname[18]:=71;curname[19]:=47;curname[20]:=62;
entername(79);namelength:=5;curname[16]:=47;curname[17]:=76;
curname[18]:=73;curname[19]:=71;curname[20]:=47;entername(77);
namelength:=6;curname[15]:=47;curname[16]:=76;curname[17]:=73;
curname[18]:=71;curname[19]:=47;curname[20]:=62;entername(81);
namelength:=7;curname[14]:=47;curname[15]:=76;curname[16]:=73;
curname[17]:=71;curname[18]:=47;curname[19]:=62;curname[20]:=62;
entername(85);{:47}{163:}namelength:=4;curname[17]:=84;curname[18]:=89;
curname[19]:=80;curname[20]:=69;entername(13);namelength:=8;
curname[13]:=71;curname[14]:=76;curname[15]:=85;curname[16]:=69;
curname[17]:=75;curname[18]:=69;curname[19]:=82;curname[20]:=78;
entername(14);namelength:=11;curname[10]:=67;curname[11]:=72;
curname[12]:=65;curname[13]:=82;curname[14]:=83;curname[15]:=73;
curname[16]:=78;curname[17]:=84;curname[18]:=89;curname[19]:=80;
curname[20]:=69;entername(15);namelength:=9;curname[12]:=68;
curname[13]:=73;curname[14]:=82;curname[15]:=69;curname[16]:=67;
curname[17]:=84;curname[18]:=73;curname[19]:=79;curname[20]:=78;
entername(16);namelength:=4;curname[17]:=71;curname[18]:=76;
curname[19]:=85;curname[20]:=69;entername(75);{:163};paramenter;end;
procedure readligkern;var krnptr:0..maxkerns;c:byte;
begin{161:}if(fileformat>1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'This is an illegal command for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=1{:161};
{94:}begin lkstepended:=false;
...
stderr,' ');write(stderr,'STOP must follow LIG or KRN');
showerrorcontext;end else begin ligkern[nl-1].b0:=128;
lkstepended:=false;end{:99};
72:{100:}if not lkstepended then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'SKIP must follow LIG or KRN');
showerrorcontext;end else begin c:=getbyte;
...
showerrorcontext;end else nl:=nl+1;lkstepended:=true;end{:101};end;
finishtheproperty;end;
end{:95}else if curchar=41 then skiptoendofitem else junkerror;end;
begin loc:=loc-1;level:=level+1;curchar:=41;end;end{:94};end;
procedure readcharinfo;var c:byte;
begin{161:}if(fileformat>1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'This is an illegal command for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=1{:161};
{103:}begin c:=getbyte;
if verbose then{108:}begin if charsonline=8 then begin writeln(stderr,
' ');charsonline:=1;
...
charremainder[c]:=getbyte;end;
56:{105:}begin if ne=256 then begin if charsonline>0 then writeln(stderr
,' ');write(stderr,'At most 256 VARCHAR specs are allowed');
showerrorcontext;end else begin checktag(c);chartag[c]:=3;
charremainder[c]:=ne;exten[ne].b0:=0;exten[ne].b1:=0;exten[ne].b2:=0;
exten[ne].b3:=0;while level=2 do begin while curchar=32 do getnext;
...
end;finishtheproperty;end;
end{:106}else if curchar=41 then skiptoendofitem else junkerror;end;
ne:=ne+1;begin loc:=loc-1;level:=level+1;curchar:=41;end;end;end{:105};
end;finishtheproperty;end;
end{:104}else if curchar=41 then skiptoendofitem else junkerror;end;
...
if curcode=0 then skiptoendofitem else if(curcode>16)or((fileformat=1)
and(curcode>12))then begin begin if charsonline>0 then writeln(stderr,
' ');
write(stderr,'This property name doesn''t belong on the outer level');
showerrorcontext;end;skiptoendofitem;
end else begin{85:}case curcode of 1:begin checksumspecified:=true;
readfourbytes(0);end;2:{88:}begin nextd:=getfix;
...
headerptr:=headerptr+1;end;readfourbytes(4*c);end;end{:91};
...
if c=0 then begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'PARAMETER index must not be zero');showerrorcontext;end;
skiptoendofitem;
...
param[c]:=getfix;finishtheproperty;end;end;
end{:93}else if curchar=41 then skiptoendofitem else junkerror;end;
begin loc:=loc-1;level:=level+1;curchar:=41;end;end{:92};10:readligkern;
11:bchar:=getbyte;12:readcharinfo;13:readkanjiinfo;14:readgluekern;
15:readcharsintype;16:readdirection;end{:85};finishtheproperty;end;
end{:84}else if(curchar=41)and not inputhasended then begin begin if
charsonline>0 then writeln(stderr,' ');
write(stderr,'Extra right parenthesis');showerrorcontext;end;loc:=loc+1;
curchar:=32;end else if not inputhasended then junkerror;
until inputhasended{:82};end;procedure corrandcheck;var c:0..256;
hh:0..hashsize;ligptr:0..maxligsteps;g:byte;
begin{110:}if nl>0 then{116:}begin if charremainder[256]<32767 then
begin ligkern[nl].b0:=255;ligkern[nl].b1:=0;ligkern[nl].b2:=0;
...
1:{120:}begin ligptr:=charremainder[c];
repeat if hashinput(ligptr,c)then begin if ligkern[ligptr].b2<128 then
begin if fileformat=1 then begin if ligkern[ligptr].b1<>bchar then begin
g:=ligkern[ligptr].b1;if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'LIG character examined by',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
begin g:=ligkern[ligptr].b3;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'LIG character generated by',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
...
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'GLUE character generated by',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
end else if ligkern[ligptr].b1<>bchar then begin g:=ligkern[ligptr].b1;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'KRN character examined by',' ');printoctal(c);
...
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'The character NEXTLARGER than',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
3:{112:}begin if exten[charremainder[c]].b0>0 then begin g:=exten[
charremainder[c]].b0;if(g>=128)and(c<128)then sevenunsafe:=true;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'TOP piece of character',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
if exten[charremainder[c]].b1>0 then begin g:=exten[charremainder[c]].b1
;if(g>=128)and(c<128)then sevenunsafe:=true;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'MID piece of character',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
if exten[charremainder[c]].b2>0 then begin g:=exten[charremainder[c]].b2
;if(g>=128)and(c<128)then sevenunsafe:=true;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'BOT piece of character',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;
begin g:=exten[charremainder[c]].b3;
if(g>=128)and(c<128)then sevenunsafe:=true;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'REP piece of character',' ');printoctal(c);
writeln(stderr,' had no CHARACTER spec.');end;end;end{:112};end{:111};
if charremainder[256]<32767 then begin c:=256;
{120:}begin ligptr:=charremainder[c];
repeat if hashinput(ligptr,c)then begin if ligkern[ligptr].b2<128 then
begin if fileformat=1 then begin if ligkern[ligptr].b1<>bchar then begin
g:=ligkern[ligptr].b1;if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'LIG character examined by',' ');printoctal(c);
...
hashlist[hh];
if classvar[tt]>0 then tt:=f(tt,(hash[tt]-1)div 256,(hash[tt]-1)mod 256)
;end;
if(hashptr=hashsize)or(yligcycle<256)then begin if hashptr<hashsize then
begin write(stderr,'Infinite ligature loop starting with ');
...
{126:}if ne>0 then for g:=0 to ne-1 do begin begin c:=exten[g].b0;
...
);printoctal(c);writeln(stderr,'!');end;end;begin c:=exten[g].b1;
if c>0 then if charwd[c]=0 then begin exten[g].b1:=0;
if charwd[0]=0 then charwd[0]:=sortin(1,0);
write(stderr,'Unused ','VARCHAR MID',' refers to nonexistent character '
);printoctal(c);writeln(stderr,'!');end;end;begin c:=exten[g].b2;
if c>0 then if charwd[c]=0 then begin exten[g].b2:=0;
if charwd[0]=0 then charwd[0]:=sortin(1,0);
write(stderr,'Unused ','VARCHAR BOT',' refers to nonexistent character '
);printoctal(c);writeln(stderr,'!');end;end;begin c:=exten[g].b3;
if charwd[c]=0 then begin exten[g].b3:=0;
if charwd[0]=0 then charwd[0]:=sortin(1,0);
write(stderr,'Unused ','VARCHAR REP',' refers to nonexistent character '
);printoctal(c);writeln(stderr,'!');end;end;end{:126};
if fileformat=1 then for c:=0 to 255 do{113:}if chartag[c]=2 then begin
g:=charremainder[c];while(g<c)and(chartag[g]=2)do g:=charremainder[g];
if g=c then begin chartag[c]:=0;
write(stderr,'A cycle of NEXTLARGER characters has been broken at ');
printoctal(c);writeln(stderr,'.');end;end{:113};
{115:}delta:=shorten(1,255);setindices(1,delta);
if delta>0 then begin write(stderr,'I had to round some ','width',
's by ');fprintreal(stderr,(((delta+1)div 2)/1048576),1,7);
writeln(stderr,' units.');end;delta:=shorten(2,15);setindices(2,delta);
if delta>0 then begin write(stderr,'I had to round some ','height',
's by ');fprintreal(stderr,(((delta+1)div 2)/1048576),1,7);
writeln(stderr,' units.');end;delta:=shorten(3,15);setindices(3,delta);
if delta>0 then begin write(stderr,'I had to round some ','depth',
's by ');fprintreal(stderr,(((delta+1)div 2)/1048576),1,7);
writeln(stderr,' units.');end;delta:=shorten(4,63);setindices(4,delta);
if delta>0 then begin write(stderr,'I had to round some ',
'italic correction','s by ');
fprintreal(stderr,(((delta+1)div 2)/1048576),1,7);
writeln(stderr,' units.');end;{:115}{:110}end;procedure readkanjiinfo;
var c:byte;
begin{162:}if(fileformat=1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'You can use this command only for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=2{:162};
{170:}begin c:=getbyte;
if verbose then{108:}begin if charsonline=8 then begin writeln(stderr,
' ');charsonline:=1;
end else begin if charsonline>0 then write(stderr,' ');
charsonline:=charsonline+1;end;printoctal(c);end{:108};
while level=1 do begin while curchar=32 do getnext;
if curchar=40 then{171:}begin getname;
if curcode=0 then skiptoendofitem else if(curcode<51)and(curcode>54)then
begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'This property name doesn''t belong in a TYPE list');
showerrorcontext;end;skiptoendofitem;
end else begin case curcode of 51:charwd[c]:=sortin(1,getfix);
52:charht[c]:=sortin(2,getfix);53:chardp[c]:=sortin(3,getfix);
54:charic[c]:=sortin(4,getfix);end;finishtheproperty;end;
end{:171}else if curchar=41 then skiptoendofitem else junkerror;end;
if charwd[c]=0 then charwd[c]:=sortin(1,0);begin loc:=loc-1;
level:=level+1;curchar:=41;end;end;{:170};end;procedure readgluekern;
var krnptr:0..maxkerns;c:byte;
begin{162:}if(fileformat=1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'You can use this command only for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=2{:162};
{165:}begin while level=1 do begin while curchar=32 do getnext;
if curchar=40 then{166:}begin getname;
if curcode=0 then skiptoendofitem else begin case curcode of 70:{167:}
begin c:=getbyte;case chartag[c]of 0:;
1:begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'This character already appeared in a GLUEKERN LABEL');
showerrorcontext;end;2:begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'Impossible: a list tag in a kanji format file?');
showerrorcontext;end;3:begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'Impossible: an extensible tag in a kanji format file?');
showerrorcontext;end;end;
if nl>255 then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,
'GLUEKERN with more than 255 commands cannot have further labels');
showerrorcontext;end else begin chartag[c]:=1;charremainder[c]:=nl;
lkstepended:=false;end;end{:167};
71:{99:}if not lkstepended then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'STOP must follow LIG or KRN');
showerrorcontext;end else begin ligkern[nl-1].b0:=128;
lkstepended:=false;end{:99};73:{168:}begin ligkern[nl].b0:=0;
ligkern[nl].b1:=getbyte;ligkern[nl].b2:=128;kern[nk]:=getfix;krnptr:=0;
while kern[krnptr]<>kern[nk]do krnptr:=krnptr+1;
if krnptr=nk then begin if nk<256 then nk:=nk+1 else begin begin if
charsonline>0 then writeln(stderr,' ');
write(stderr,'At most 256 different kerns are allowed');
showerrorcontext;end;krnptr:=255;end;end;ligkern[nl].b3:=krnptr;
if nl=511 then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'GLUEKERN table should never exceed 511 LIG/KRN commands');
showerrorcontext;end else nl:=nl+1;lkstepended:=true;end{:168};
75:{169:}begin ligkern[nl].b0:=0;ligkern[nl].b1:=getbyte;
ligkern[nl].b2:=0;glue[3*ng+0]:=getfix;glue[3*ng+1]:=getfix;
glue[3*ng+2]:=getfix;krnptr:=0;
while(glue[3*krnptr+0]<>glue[3*ng+0])or(glue[3*krnptr+1]<>glue[3*ng+1])
or(glue[3*krnptr+2]<>glue[3*ng+2])do krnptr:=krnptr+1;
if krnptr=ng then begin if ng<256 then ng:=ng+1 else begin begin if
charsonline>0 then writeln(stderr,' ');
write(stderr,'At most 256 different glues are allowed');
showerrorcontext;end;krnptr:=255;end;end;ligkern[nl].b3:=krnptr;
if nl=511 then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'GLUEKERN table should never exceed 511 GLUE/KRN commands')
;showerrorcontext;end else nl:=nl+1;lkstepended:=true;end{:169};
others:begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'This property name doesn''t belong in a GLUEKERN list');
showerrorcontext;end;skiptoendofitem;end;end;finishtheproperty;end;
end{:166}else if curchar=41 then skiptoendofitem else junkerror;end;
begin loc:=loc-1;level:=level+1;curchar:=41;end;end;{:165};end;
procedure readcharsintype;var typenum:byte;jiscode:integer;
begin{162:}if(fileformat=1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'You can use this command only for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=2{:162};
{172:}begin typenum:=getbyte;
if typenum=0 then begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,
'You cannot list the chars in type 0. It is the default type');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);
end else begin repeat jiscode:=getkanji;
if jiscode<0 then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,
'Illegal characters. I was expecting a jis code or character');
showerrorcontext;
end else if jiscode=0 then else if kanjitype[jistoindex(jiscode)]>=0
then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'jis code ',jiscode:1,' is already in type ',kanjitype[
jistoindex(jiscode)]);showerrorcontext;
end else kanjitype[jistoindex(jiscode)]:=typenum;until jiscode=0;
repeat getnext until(curchar=40)or(curchar=41);end end{:172};end;
procedure readdirection;
begin{162:}if(fileformat=1)then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'You can use this command only for kanji format files.');
showerrorcontext;end else if(fileformat=0)then fileformat:=2{:162};
{173:}begin while curchar=32 do getnext;
if curchar=84 then begin if verbose then writeln(stderr,
'This is tatekumi format');fileformat:=4;
end else if curchar=89 then begin if verbose then writeln(stderr,
'This is yokokumi format');fileformat:=3;
end else begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'The dir value should be "TATE" or "YOKO"');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);
end{:173};end;{:146}{147:}begin initialize;nameenter;readinput;
if verbose then writeln(stderr,'.');corrandcheck;
{128:}case fileformat of 1:;0,2:begin fileformat:=3;
writeln(stderr,'Input file is in kanji YOKO-kumi format.');end;
3:writeln(stderr,'Input file is in kanji YOKO-kumi format.');
4:writeln(stderr,'Input file is in kanji TATE-kumi format.');end;
{130:}lh:=headerptr div 4;if fileformat<>1 then begin bc:=0;ec:=0;nt:=1;
for kanjitypeindex:=0 to maxkanji do begin if kanjitype[kanjitypeindex]>
0 then nt:=nt+1;
if kanjitype[kanjitypeindex]>ec then ec:=kanjitype[kanjitypeindex];end;
end else begin notfound:=true;bc:=0;
...
ec-1;if bc>ec then bc:=1;end;memory[1]:=memory[1]+1;
memory[2]:=memory[2]+1;memory[3]:=memory[3]+1;memory[4]:=memory[4]+1;
{139:}{140:}labelptr:=0;labeltable[0].rr:=-1;
...
end else begin extralocneeded:=false;lkoffset:=0;end;
...
if fileformat<>1 then lf:=7+nt+lh+(ec-bc+1)+memory[1]+memory[2]+memory[3
]+memory[4]+nl+lkoffset+nk+3*ng+np else lf:=6+lh+(ec-bc+1)+memory[1]+
memory[2]+memory[3]+memory[4]+nl+lkoffset+nk+ne+np;{:130};
{131:}if fileformat=3 then begin putbyte((yokoidnumber)div 256,tfmfile);
putbyte((yokoidnumber)mod 256,tfmfile);putbyte((nt)div 256,tfmfile);
putbyte((nt)mod 256,tfmfile);
end else if fileformat=4 then begin putbyte((tateidnumber)div 256,
tfmfile);putbyte((tateidnumber)mod 256,tfmfile);
putbyte((nt)div 256,tfmfile);putbyte((nt)mod 256,tfmfile);end;
putbyte((lf)div 256,tfmfile);putbyte((lf)mod 256,tfmfile);
putbyte((lh)div 256,tfmfile);putbyte((lh)mod 256,tfmfile);
putbyte((bc)div 256,tfmfile);putbyte((bc)mod 256,tfmfile);
putbyte((ec)div 256,tfmfile);putbyte((ec)mod 256,tfmfile);
putbyte((memory[1])div 256,tfmfile);putbyte((memory[1])mod 256,tfmfile);
putbyte((memory[2])div 256,tfmfile);putbyte((memory[2])mod 256,tfmfile);
putbyte((memory[3])div 256,tfmfile);putbyte((memory[3])mod 256,tfmfile);
putbyte((memory[4])div 256,tfmfile);putbyte((memory[4])mod 256,tfmfile);
putbyte((nl+lkoffset)div 256,tfmfile);
putbyte((nl+lkoffset)mod 256,tfmfile);putbyte((nk)div 256,tfmfile);
putbyte((nk)mod 256,tfmfile);
if fileformat<>1 then begin putbyte((ng*3)div 256,tfmfile);
putbyte((ng*3)mod 256,tfmfile)end else begin putbyte((ne)div 256,tfmfile
);putbyte((ne)mod 256,tfmfile);end;putbyte((np)div 256,tfmfile);
putbyte((np)mod 256,tfmfile);{:131};
{133:}if not checksumspecified then{134:}begin curbytes.b0:=bc;
...
for j:=0 to headerptr-1 do putbyte(headerbytes[j],tfmfile);{:133};
if fileformat<>1 then{174:}begin putbyte((0)div 256,tfmfile);
putbyte((0)mod 256,tfmfile);putbyte((0)div 256,tfmfile);
putbyte((0)mod 256,tfmfile);
for kanjitypeindex:=0 to maxkanji do begin if kanjitype[kanjitypeindex]>
0 then begin putbyte((indextojis(kanjitypeindex))div 256,tfmfile);
putbyte((indextojis(kanjitypeindex))mod 256,tfmfile);
putbyte((kanjitype[kanjitypeindex])div 256,tfmfile);
putbyte((kanjitype[kanjitypeindex])mod 256,tfmfile);
if verbose then begin write(stderr,'char index = ',kanjitypeindex);
write(stderr,' (jis ');printjishex(indextojis(kanjitypeindex));
write(stderr,') is type ');printoctal(kanjitype[kanjitypeindex]);
writeln('');end;end;end;end;{:174};{135:}indexvar[0]:=0;
for c:=bc to ec do begin putbyte(indexvar[charwd[c]],tfmfile);
putbyte(indexvar[charht[c]]*16+indexvar[chardp[c]],tfmfile);
putbyte(indexvar[charic[c]]*4+chartag[c],tfmfile);
putbyte(charremainder[c],tfmfile);end{:135};
{137:}for q:=1 to 4 do begin putbyte(0,tfmfile);putbyte(0,tfmfile);
putbyte(0,tfmfile);putbyte(0,tfmfile);p:=link[q];
while p>0 do begin outscaled(memory[p]);p:=link[p];end;end;{:137};
{142:}if extralocneeded then begin putbyte(255,tfmfile);
...
if(fileformat<>1)and(ng>0)then for krnptr:=0 to ng-1 do begin outscaled(
glue[3*krnptr+0]);outscaled(glue[3*krnptr+1]);
outscaled(glue[3*krnptr+2]);end;
{143:}if ne>0 then for c:=0 to ne-1 do begin putbyte(exten[c].b0,tfmfile
);putbyte(exten[c].b1,tfmfile);putbyte(exten[c].b2,tfmfile);
putbyte(exten[c].b3,tfmfile);end;{:143};
...
end{:144}{:128};if not perfect then uexit(1);end.{:147}
