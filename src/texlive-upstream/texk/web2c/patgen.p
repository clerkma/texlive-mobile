{3:}{11:}{$C-,A+,D-}
{:11}program PATGEN(dictionary,patterns,translate,patout);
const{27:}triesize=550000;triecsize=260000;maxops=4080;maxval=10;
maxdot=15;maxlen=50;maxbuflen=3000;{:27}type{12:}ASCIIcode=0..255;
textchar=ASCIIcode;textfile=text;{:12}{13:}packedASCIIcode=0..255;
{:13}{20:}internalcode=ASCIIcode;packedinternalcode=packedASCIIcode;
{:20}{22:}classtype=0..5;digit=0..9;hyftype=0..3;
{:22}{29:}qindex=1..255;valtype=0..maxval;dottype=0..maxdot;
optype=0..maxops;wordindex=0..maxlen;triepointer=0..triesize;
triecpointer=0..triecsize;opword=packed record dot:dottype;val:valtype;
op:optype end;{:29}var{4:}patstart,patfinish:dottype;
hyphstart,hyphfinish:valtype;goodwt,badwt,thresh:integer;
{:4}{16:}xord:array[textchar]of ASCIIcode;
xchr:array[ASCIIcode]of textchar;
{:16}{23:}xclass:array[textchar]of classtype;
xint:array[textchar]of internalcode;xdig:array[0..9]of textchar;
xext:array[internalcode]of textchar;xhyf:array[1..3]of textchar;
{:23}{25:}cmax:internalcode;
{:25}{30:}triec:packed array[triepointer]of packedinternalcode;
...
ops:array[optype]of opword;
{:30}{31:}trieqc:array[qindex]of internalcode;
trieql,trieqr:array[qindex]of triepointer;qmax:qindex;qmaxthresh:qindex;
{:31}{33:}triemax:triepointer;triebmax:triepointer;
triecount:triepointer;opcount:optype;
{:33}{40:}pat:array[dottype]of internalcode;patlen:dottype;
{:40}{43:}triecmax,triecbmax,trieccount:triecpointer;
trieckmax:triecpointer;patcount:integer;
{:43}{51:}dictionary,patterns,translate,patout,pattmp:textfile;
fname:^char;badfrac,denom,eff:real;
{:51}{52:}buf:array[1..maxbuflen]of textchar;bufptr:0..maxbuflen;
{:52}{55:}imax:internalcode;lefthyphenmin,righthyphenmin:dottype;
{:55}{66:}goodpatcount,badpatcount:integer;
goodcount,badcount,misscount:integer;levelpatterncount:integer;
moretocome:boolean;{:66}{74:}word:array[wordindex]of internalcode;
dots:array[wordindex]of hyftype;dotw:array[wordindex]of digit;
hval:array[wordindex]of valtype;nomore:array[wordindex]of boolean;
wlen:wordindex;wordwt:digit;wtchg:boolean;
{:74}{78:}hyfmin,hyfmax,hyflen:wordindex;
{:78}{84:}gooddot,baddot:hyftype;dotmin,dotmax,dotlen:wordindex;
{:84}{87:}procesp,hyphp:boolean;patdot:dottype;hyphlevel:valtype;
filnam:packed array[1..8]of char;{:87}{91:}maxpat:valtype;
{:91}{95:}n1,n2,n3:integer;i:valtype;j:dottype;k:dottype;dot1:dottype;
morethislevel:array[dottype]of boolean;
{:95}{98:}procedure parsearguments;const noptions=2;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{99:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:99}{100:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:100}{101:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:101};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval='?'then begin usage('patgen');
...
printversionandexit('This is PATGEN, Version 2.4',nil,
'Frank M. Liang and Peter Breitenlohner',nil);end;
until getoptreturnval=-1;
if(optind+4<>argc)then begin writeln(stderr,'patgen',
': Need exactly four arguments.');usage('patgen');end;end;
{:98}procedure initialize;var{15:}bad:integer;i:textchar;j:ASCIIcode;
{:15}begin kpsesetprogramname(argv[0],'patgen');parsearguments;
write(output,'This is PATGEN, Version 2.4');
writeln(output,versionstring);{14:}bad:=0;if 255<127 then bad:=1;
if(0<>0)or(0<>0)then bad:=2;
{28:}if(triecsize<4096)or(triesize<triecsize)then bad:=3;
if maxops>triesize then bad:=4;if maxval>10 then bad:=5;
if maxbuflen<maxlen then bad:=6;
{:28}if bad>0 then begin writeln(stderr,'Bad constants---case ',bad:1);
...
xchr[121]:='y';xchr[122]:='z';
{:17}{18:}for i:=chr(0)to chr(255)do xord[i]:=0;
for j:=0 to 255 do xord[xchr[j]]:=j;xord[' ']:=32;xord[chr(9)]:=32;
{:18}{24:}for i:=chr(0)to chr(255)do begin xclass[i]:=5;xint[i]:=0;end;
...
begin writeln(stderr,'PATGEN capacity exceeded, sorry [',256:1,
...
while true do begin t:=triel[t];s:=t-trieqc[1];
{37:}if s>triesize-256 then begin writeln(stderr,
...
end;trietaken[s]:=true;firstfit:=s end;
{:35}{38:}procedure unpack(s:triepointer);var c:internalcode;
t:triepointer;begin qmax:=1;for c:=1 to cmax do begin t:=s+c;
...
if opcount=maxops then begin writeln(stderr,
...
;goto 10;end;if h>1 then h:=h-1 else h:=maxops;end;10:end;
{:39}{41:}procedure insertpattern(val:valtype;dot:dottype);
var i:dottype;s,t:triepointer;begin i:=1;s:=1+pat[i];t:=triel[s];
while(t>0)and(i<patlen)do begin i:=i+1;t:=t+pat[i];
if triec[t]<>pat[i]then{42:}begin if triec[t]=0 then begin triel[trier[t
]]:=triel[t];trier[triel[t]]:=trier[t];triec[t]:=pat[i];triel[t]:=0;
trier[t]:=0;if t>triemax then triemax:=t;
end else begin unpack(t-pat[i]);trieqc[qmax]:=pat[i];trieql[qmax]:=0;
trieqr[qmax]:=0;t:=firstfit;triel[s]:=t;t:=t+pat[i];end;
triecount:=triecount+1;end{:42};s:=t;t:=triel[s];end;trieql[1]:=0;
trieqr[1]:=0;qmax:=1;while i<patlen do begin i:=i+1;trieqc[1]:=pat[i];
t:=firstfit;triel[s]:=t;s:=t+pat[i];triecount:=triecount+1;end;
trier[s]:=newtrieop(val,dot,trier[s]);end;
{:41}{44:}procedure initcounttrie;var c:internalcode;
...
triecl[0]:=triecmax+1;triecr[triecmax+1]:=0;patcount:=0;end;
{:44}{45:}function firstcfit:triecpointer;label 40,41;
var a,b:triecpointer;q:qindex;
begin{46:}if qmax>3 then a:=triecr[triecmax+1]else a:=0;
while true do begin a:=triecl[a];b:=a-trieqc[1];
{47:}if b>trieckmax-256 then begin if trieckmax=triecsize then begin
writeln(stderr,'PATGEN capacity exceeded, sorry [',triecsize:1,
' count trie nodes','].');uexit(1);end;;
write(output,trieckmax div 1024:1,'K ');
...
triectaken[b]:=false;end;
{:48}{49:}function insertcpat(fpos:wordindex):triecpointer;
var spos:wordindex;a,b:triecpointer;begin spos:=fpos-patlen;
spos:=spos+1;b:=1+word[spos];a:=triecl[b];
while(a>0)and(spos<fpos)do begin spos:=spos+1;a:=a+word[spos];
if triecc[a]<>word[spos]then{50:}begin if triecc[a]=0 then begin triecl[
triecr[a]]:=triecl[a];triecr[triecl[a]]:=triecr[a];
triecc[a]:=word[spos];triecl[a]:=0;triecr[a]:=0;
if a>triecmax then triecmax:=a;end else begin unpackc(a-word[spos]);
trieqc[qmax]:=word[spos];trieql[qmax]:=0;trieqr[qmax]:=0;a:=firstcfit;
triecl[b]:=a;a:=a+word[spos];end;trieccount:=trieccount+1;end{:50};b:=a;
a:=triecl[a];end;trieql[1]:=0;trieqr[1]:=0;qmax:=1;
while spos<fpos do begin spos:=spos+1;trieqc[1]:=word[spos];
a:=firstcfit;triecl[b]:=a;b:=a+word[spos];trieccount:=trieccount+1;end;
insertcpat:=b;patcount:=patcount+1;end;
{:49}{54:}procedure readtranslate;label 30;var c:textchar;n:integer;
j:ASCIIcode;bad:boolean;lower:boolean;i:dottype;s,t:triepointer;
begin imax:=1;fname:=cmdline(4);reset(translate,fname);
if eof(translate)then{56:}begin lefthyphenmin:=2;righthyphenmin:=3;
...
bufptr:=0;repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'Line too long');uexit(1);end;;end;
...
end;{57:}bad:=false;n:=0;
if buf[1]=' 'then else if xclass[buf[1]]=1 then n:=xint[buf[1]]else bad
:=true;if xclass[buf[2]]=1 then n:=10*n+xint[buf[2]]else bad:=true;
if(n>=1)and(n<maxdot)then lefthyphenmin:=n else bad:=true;n:=0;
if buf[3]=' 'then else if xclass[buf[3]]=1 then n:=xint[buf[3]]else bad
:=true;if xclass[buf[4]]=1 then n:=10*n+xint[buf[4]]else bad:=true;
if(n>=1)and(n<maxdot)then righthyphenmin:=n else bad:=true;
if bad then begin bad:=false;
repeat write(output,'left_hyphen_min, right_hyphen_min: ');
input2ints(n1,n2);
if(n1>=1)and(n1<maxdot)and(n2>=1)and(n2<maxdot)then begin lefthyphenmin
:=n1;righthyphenmin:=n2;end else begin n1:=0;
writeln(output,'Specify 1<=left_hyphen_min,right_hyphen_min<=',maxdot-1:
1,' !');end;until n1>0;end;
...
repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'Bad hyphenation data');uexit(1);end;;end{:57};
...
trieqr[qmax]:=0;t:=firstfit;triel[s]:=t;t:=t+pat[i];end;
triecount:=triecount+1;end{:42}else if trier[t]>0 then bad:=true;s:=t;
t:=triel[s];end;if t>1 then bad:=true;trieql[1]:=0;trieqr[1]:=0;qmax:=1;
...
repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'Bad representation');uexit(1);end;;end;end{:58};
end;xfclose(translate,'inputfile');
writeln(output,'left_hyphen_min = ',lefthyphenmin:1,
...
begin l:=1+trier[a];
for j:=1 to i-1 do begin if triecmax=triecsize then begin writeln(stderr
...
if i<patlen then traversecounttrie(triecl[a],i+1)else{65:}if goodwt*
triecl[a]<thresh then begin insertpattern(maxval,patdot);
...
write(output,goodpatcount:1,' good and ',badpatcount:1,
' bad patterns added');
levelpatterncount:=levelpatterncount+goodpatcount;
if moretocome then writeln(output,' (more to come)')else writeln(output,
' ');write(output,'finding ',goodcount:1,' good and ',badcount:1,
' bad hyphens');
if goodpatcount>0 then begin write(output,', efficiency = ');
printreal(goodcount/(goodpatcount+badcount/(thresh/goodwt)),1,2);
writeln(output);end else writeln(output,' ');
writeln(output,'pattern trie has ',triecount:1,' nodes, ','trie_max = ',
triemax:1,', ',opcount:1,' outputs');end;
{:67}{68:}function deletepatterns(s:triepointer):triepointer;
var c:internalcode;t:triepointer;allfreed:boolean;h,n:optype;
begin allfreed:=true;for c:=1 to cmax do begin t:=s+c;
if triec[t]=c then begin{69:}begin h:=0;ops[0].op:=trier[t];
n:=ops[0].op;
...
if(triel[t]>0)or(trier[t]>0)or(s=1)then allfreed:=false else{70:}begin
triel[trier[triemax+1]]:=t;trier[t]:=trier[triemax+1];
...
writeln(output,oldtriecount-triecount:1,' nodes and ',oldopcount-opcount
:1,' outputs deleted');qmaxthresh:=7;end;
{:71}{72:}procedure outputpatterns(s:triepointer;patlen:dottype);
var c:internalcode;t:triepointer;h:optype;d:dottype;l:triecpointer;
begin for c:=1 to cmax do begin t:=s+c;
if triec[t]=c then begin pat[patlen]:=c;h:=trier[t];
if h>0 then{73:}begin for d:=0 to patlen do hval[d]:=0;
repeat d:=ops[h].dot;if hval[d]<ops[h].val then hval[d]:=ops[h].val;
h:=ops[h].op;until h=0;if hval[0]>0 then write(patout,xdig[hval[0]]);
...
repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'PATGEN capacity exceeded, sorry [','word length=',
...
write(output,buf[bufptr]);until bufptr=maxbuflen;writeln(output,' ');
end;
begin writeln(stderr,'PATGEN capacity exceeded, sorry [','word length=',
...
write(output,buf[bufptr]);until bufptr=maxbuflen;writeln(output,' ');
end;begin writeln(stderr,'Bad representation');uexit(1);end;;end;
...
write(output,buf[bufptr]);until bufptr=maxbuflen;writeln(output,' ');
end;begin writeln(stderr,'Bad character');uexit(1);end;;end;end;
until bufptr=maxbuflen;40:wlen:=wlen+1;word[wlen]:=1;end;
{:76}{77:}procedure hyphenate;label 30;var spos,dpos,fpos:wordindex;
t:triepointer;h:optype;v:valtype;
...
fname:=cmdline(1);reset(dictionary,fname);{75:}xclass['.']:=5;
xclass[xhyf[1]]:=2;xint[xhyf[1]]:=0;xclass[xhyf[2]]:=2;xint[xhyf[2]]:=2;
xclass[xhyf[3]]:=2;xint[xhyf[3]]:=2;{:75}{79:}hyfmin:=lefthyphenmin+1;
hyfmax:=righthyphenmin+1;hyflen:=hyfmin+hyfmax;
{:79}{85:}if procesp then begin dotmin:=patdot;dotmax:=patlen-patdot;
...
end else begin gooddot:=1;baddot:=3;end;end;
{:85}if procesp then begin initcounttrie;
writeln(output,'processing dictionary with pat_len = ',patlen:1,
', pat_dot = ',patdot:1);end;
if hyphp then begin strcpy(filnam,'pattmp. ');
filnam[7]:=xdig[hyphlevel];rewrite(pattmp,filnam);
writeln(output,'writing pattmp.',xdig[hyphlevel]);end;
{89:}while not eof(dictionary)do begin readword;
...
xfclose(dictionary,'inputfile');writeln(output,' ');
writeln(output,goodcount:1,' good, ',badcount:1,' bad, ',misscount:1,
' missed');
if(goodcount+misscount)>0 then begin printreal((100*goodcount/(goodcount
+misscount)),1,2);write(output,' %, ');
printreal((100*badcount/(goodcount+misscount)),1,2);
write(output,' %, ');
printreal((100*misscount/(goodcount+misscount)),1,2);
writeln(output,' %');end;
if procesp then writeln(output,patcount:1,' patterns, ',trieccount:1,
' nodes in count trie, ','triec_max = ',triecmax:1);
if hyphp then xfclose(pattmp,'outputfile');end;
{:88}{90:}procedure readpatterns;label 30,40;var c:textchar;d:digit;
i:dottype;t:triepointer;begin xclass['.']:=3;xint['.']:=1;
levelpatterncount:=0;maxpat:=0;fname:=cmdline(2);reset(patterns,fname);
...
write(output,buf[bufptr]);until bufptr=maxbuflen;writeln(output,' ');
end;begin writeln(stderr,'Bad hyphenation value');uexit(1);end;;end;
...
repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'Bad character');uexit(1);end;;end;end;
until bufptr=maxbuflen{:92};
40:{93:}if patlen>0 then for i:=0 to patlen do begin if hval[i]<>0 then
insertpattern(hval[i],i);
if i>1 then if i<patlen then if pat[i]=1 then begin begin bufptr:=0;
repeat bufptr:=bufptr+1;write(output,buf[bufptr]);
until bufptr=maxbuflen;writeln(output,' ');end;
begin writeln(stderr,'Bad edge_of_word');uexit(1);end;;end;end{:93};end;
xfclose(patterns,'inputfile');
writeln(output,levelpatterncount:1,' patterns read in');
writeln(output,'pattern trie has ',triecount:1,' nodes, ','trie_max = ',
...
repeat write(output,'hyph_start, hyph_finish: ');input2ints(n1,n2);
if(n1>=1)and(n1<maxval)and(n2>=1)and(n2<maxval)then begin hyphstart:=n1;
hyphfinish:=n2;end else begin n1:=0;
writeln(output,'Specify 1<=hyph_start,hyph_finish<=',maxval-1:1,' !');
end;until n1>0;hyphlevel:=maxpat;
for i:=hyphstart to hyphfinish do begin hyphlevel:=i;
levelpatterncount:=0;
if hyphlevel>hyphstart then writeln(output,' ')else if hyphstart<=maxpat
then writeln(output,'Largest hyphenation value ',maxpat:1,
' in patterns should be less than hyph_start');
repeat write(output,'pat_start, pat_finish: ');input2ints(n1,n2);
if(n1>=1)and(n1<=n2)and(n2<=maxdot)then begin patstart:=n1;
patfinish:=n2;end else begin n1:=0;
writeln(output,'Specify 1<=pat_start<=pat_finish<=',maxdot:1,' !');end;
until n1>0;repeat write(output,'good weight, bad weight, threshold: ');
input3ints(n1,n2,n3);if(n1>=1)and(n2>=1)and(n3>=1)then begin goodwt:=n1;
badwt:=n2;thresh:=n3;end else begin n1:=0;
writeln(output,'Specify good weight, bad weight, threshold>=1 !');end;
until n1>0;{96:}for k:=0 to maxdot do morethislevel[k]:=true;
...
writeln(output,'total of ',levelpatterncount:1,
' patterns at hyph_level ',hyphlevel:1);end;findletters(triel[1],1);
fname:=cmdline(3);rewrite(patout,fname);outputpatterns(1,1);
xfclose(patout,'outputfile');{97:}procesp:=false;hyphp:=true;
write(output,'hyphenate word list? ');begin buf[1]:=getc(stdin);
readln(stdin);end;if(buf[1]='Y')or(buf[1]='y')then dodictionary{:97};
end.{:94}
