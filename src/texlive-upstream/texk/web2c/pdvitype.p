...
end else if getoptreturnval=63 then begin usage('pdvitype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(PDVITYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is pDVItype, Version 3.6-p0.4',nil,
'D.E. Knuth',nil);
...
if(optind+1<>argc)then begin writeln(stderr,'pdvitype',
': Need exactly one file argument.');usage('pdvitype');end;
dviname:=extendfilename(cmdline(optind),'dvi');end;
{:120}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'pdvitype');initkanji;parsearguments;
write(stdout,'This is pDVItype, Version 3.6-p0.4');
writeln(stdout,versionstring);{11:}for i:=0 to 31 do xchr[i]:='?';
...
xchr[126]:='~';for i:=127 to 255 do xchr[i]:=i;
...
begin if textptr>=linelength-3 then flushtext;c:=toBUFF(fromDVI(c));
textptr:=textptr+1;textbuf[textptr]:=Hi(c);textptr:=textptr+1;
textbuf[textptr]:=Lo(c);end;
