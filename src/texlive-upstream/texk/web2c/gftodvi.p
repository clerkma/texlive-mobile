{3:}program GFtoDVI(output);const{5:}maxlabels=2000;poolsize=10000;
maxstrings=1100;terminallinelength=150;filenamesize=50;fontmemsize=2000;
dvibufsize=800;widestrow=8192;liglookahead=20;
{:5}type{9:}scaled=integer;{:9}{10:}ASCIIcode=0..255;
{:10}{11:}textfile=packed file of ASCIIcode;{:11}{45:}eightbits=0..255;
...
strnumber=0..maxstrings;{:70}{79:}keywordcode=0..19;
{:79}{104:}dviindex=0..dvibufsize;{:104}{139:}nodepointer=0..maxlabels;
{:139}var{12:}xord:array[ASCIIcode]of ASCIIcode;
xchr:array[ASCIIcode]of ASCIIcode;
{:12}{16:}buffer:array[0..terminallinelength]of 0..255;
{:16}{18:}bufptr:0..terminallinelength;linelength:0..terminallinelength;
{:18}{37:}lf,lh,bc,ec,nw,nh,nd,ni,nl,nk,ne,np:0..32767;
{:37}{46:}gffile:bytefile;dvifile:bytefile;tfmfile:bytefile;
{:46}{48:}curloc:integer;nameoffile:^ASCIIcode;
{:48}{49:}b0,b1,b2,b3:eightbits;
{:49}{53:}fontinfo:array[fontindex]of memoryword;fmemptr:fontindex;
...
{:53}{71:}strpool:packed array[poolpointer]of ASCIIcode;
strstart:array[strnumber]of poolpointer;poolptr:poolpointer;
strptr:strnumber;initstrptr:strnumber;{:71}{76:}l:integer;
{:76}{80:}curgf:eightbits;curstring:strnumber;labeltype:eightbits;
{:80}{86:}curname:strnumber;curarea:strnumber;curext:strnumber;
{:86}{87:}areadelimiter:poolpointer;extdelimiter:poolpointer;
{:87}{93:}jobname:strnumber;{:93}{96:}interaction:boolean;
fontsnotloaded:boolean;fontname:array[internalfontnumber]of strnumber;
fontarea:array[internalfontnumber]of strnumber;
fontat:array[internalfontnumber]of scaled;{:96}{102:}totalpages:integer;
maxv:scaled;maxh:scaled;lastbop:integer;
{:102}{105:}dvibuf:array[dviindex]of eightbits;halfbuf:dviindex;
dvilimit:dviindex;dviptr:dviindex;dvioffset:integer;
{:105}{117:}boxwidth:scaled;boxheight:scaled;boxdepth:scaled;
ligstack:array[1..liglookahead]of quarterword;dummyinfo:fourquarters;
suppresslig:boolean;{:117}{127:}c:array[1..120]of 1..4095;
d:array[1..120]of 2..4096;twotothe:array[0..13]of 1..8192;
{:127}{134:}ruleslant:real;slantn:integer;slantunit:real;
slantreported:real;{:134}{140:}xl,xr,yt,yb:array[1..maxlabels]of scaled;
...
xoffset,yoffset:scaled;preminx,premaxx,preminy,premaxy:scaled;
{:155}{158:}ruleptr:nodepointer;{:158}{160:}labeltail:nodepointer;
titlehead,titletail:nodepointer;{:160}{166:}charcode,ext:integer;
minx,maxx,miny,maxy:integer;x,y:integer;z:integer;
{:166}{168:}xratio,yratio,slantratio:real;
unscxratio,unscyratio,unscslantratio:real;fudgefactor:real;
deltax,deltay:scaled;dvix,dviy:scaled;overcol:scaled;
pageheight,pagewidth:scaled;{:168}{174:}grayrulethickness:scaled;
tempx,tempy:scaled;{:174}{182:}overflowline:integer;
{:182}{183:}delta:scaled;halfxheight:scaled;thricexheight:scaled;
dotwidth,dotheight:scaled;{:183}{207:}b:array[0..4095]of 0..120;
rho:array[0..4095]of 1..4096;
{:207}{211:}a:array[0..widestrow]of 0..4095;
{:211}{212:}blankrows:integer;{:212}{220:}k,m,p,q,r,s,t,dx,dy:integer;
timestamp:strnumber;uselogo:boolean;{:220}{226:}verbose:cinttype;
{:226}{229:}overflowlabeloffset:integer;offsetinpoints:real;
{:229}{222:}procedure parsearguments;const noptions=4;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{227:}verbose:=false;{:227}{230:}overflowlabeloffset:=10000000;
{:230};{223:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:223}{224:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:224}{225:}longoptions[currentoption].name:='verbose';
...
{:225}{228:}longoptions[currentoption].name:='overflow-label-offset';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:228}{231:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:231};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('gftodvi');
...
printversionandexit('This is GFtoDVI, Version 3.0',nil,'D.E. Knuth',nil)
;
end else if(strcmp(longoptions[optionindex].name,'overflow-label-offset'
)=0)then begin offsetinpoints:=atof(optarg);
overflowlabeloffset:=round(offsetinpoints*65536);end;
until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'gftodvi',
': Need exactly one file argument.');usage('gftodvi');end;end;
{:222}procedure initialize;var i,j,m,n:integer;
begin kpsesetprogramname(argv[0],'gftodvi');
kpseinitprog('GFTODVI',0,nil,nil);parsearguments;
if verbose then begin write(stdout,'This is GFtoDVI, Version 3.0');
writeln(stdout,versionstring);end;{13:}xchr[32]:=' ';xchr[33]:='!';
...
{:13}{14:}for i:=1 to 31 do xchr[i]:=chr(i);
for i:=127 to 255 do xchr[i]:=chr(i);
{:14}{15:}for i:=0 to 255 do xord[chr(i)]:=32;
for i:=1 to 255 do xord[xchr[i]]:=i;xord['?']:=63;{:15}{54:}fmemptr:=0;
{:54}{97:}interaction:=false;fontsnotloaded:=true;fontname[1]:=29;
fontname[2]:=30;fontname[3]:=31;fontname[4]:=0;fontname[5]:=32;
for k:=1 to 5 do begin fontarea[k]:=0;fontat[k]:=0;end;
{:97}{103:}totalpages:=0;maxv:=0;maxh:=0;lastbop:=-1;
...
if j=k then c[m]:=n else c[m]:=c[m-1]+twotothe[j-1];end;end{:129};
{:126}{142:}yy[0]:=-1073741824;yy[maxlabels]:=1073741824;{:142}end;
{:3}{8:}procedure jumpout;begin uexit(0);end;{:8}{17:}procedure inputln;
begin fflush(stdout);if eoln(stdin)then readln(stdin);linelength:=0;
while(linelength<terminallinelength)and not eoln(stdin)do begin buffer[
linelength]:=xord[getc(stdin)];linelength:=linelength+1;end;end;
{:17}{47:}procedure opengffile;
begin gffile:=kpseopenfile(stringcast(nameoffile),kpsegfformat);
curloc:=0;end;procedure opentfmfile;
begin tfmfile:=kpseopenfile(stringcast(nameoffile),kpsetfmformat);end;
procedure opendvifile;begin rewritebin(dvifile,stringcast(nameoffile));
end;{:47}{50:}procedure readtfmword;begin read(tfmfile,b0);
read(tfmfile,b1);read(tfmfile,b2);read(tfmfile,b3);end;
{:50}{51:}function getbyte:integer;var b:eightbits;
...
{:51}{58:}procedure readfontinfo(f:integer;s:scaled);label 30,11;
var k:fontindex;lf,lh,bc,ec,nw,nh,nd,ni,nl,nk,ne,np:0..65535;
bchlabel:integer;bchar:0..256;qw:fourquarters;sw:scaled;z:scaled;
alpha:integer;beta:1..16;begin{59:}{60:}begin readtfmword;lf:=b0*256+b1;
lh:=b2*256+b3;readtfmword;bc:=b0*256+b1;ec:=b2*256+b3;
...
{62:}begin if lh<2 then goto 11;begin readtfmword;qw.B0:=b0+0;
qw.B1:=b1+0;qw.B2:=b2+0;qw.B3:=b3+0;fontcheck[f]:=qw;end;readtfmword;
if b0>127 then goto 11;z:=((b0*256+b1)*256+b2)*16+(b3 div 16);
if z<65536 then goto 11;while lh>2 do begin readtfmword;lh:=lh-1;end;
fontdsize[f]:=z;if s>0 then z:=s;fontsize[f]:=z;end{:62};
{63:}for k:=fmemptr to widthbase[f]-1 do begin begin readtfmword;
...
if fontinfo[italicbase[f]].sc<>0 then goto 11;end{:64};
{66:}begin bchlabel:=32767;bchar:=256;
...
sw:=(((((b3*z)div 256)+(b2*z))div 256)+(b1*z))div beta;
if b0=0 then fontinfo[k].sc:=sw else if b0=255 then fontinfo[k].sc:=sw-
alpha else goto 11;end;end{:66};
{67:}for k:=extenbase[f]to parambase[f]-1 do begin begin readtfmword;
qw.B0:=b0+0;qw.B1:=b1+0;qw.B2:=b2+0;qw.B3:=b3+0;fontinfo[k].qqqq:=qw;
...
begin if(b3<bc)or(b3>ec)then goto 11 end;end{:67};
{68:}begin for k:=1 to np do if k=1 then begin readtfmword;
...
for k:=np+1 to 8 do fontinfo[parambase[f]+k-1].sc:=0;end{:68};
{69:}fontbc[f]:=bc;fontec[f]:=ec;
...
case f of 1:begin writeln(stderr,'titles!');uexit(1);end;
2:begin writeln(stderr,'labels!');uexit(1);end;
3:begin writeln(stderr,'pixels!');uexit(1);end;
4:begin writeln(stderr,'slants!');uexit(1);end;
5:begin writeln(stderr,'METAFONT logo!');uexit(1);end;end;30:end;
{:58}{74:}function makestring:strnumber;
begin if strptr=maxstrings then begin writeln(stderr,'Too many labels!')
;uexit(1);end;strptr:=strptr+1;strstart[strptr]:=poolptr;
makestring:=strptr-1;end;{:74}{75:}procedure firststring(c:integer);
begin if strptr<>c then begin writeln(stderr,'?');uexit(1);end;
while l>0 do begin begin strpool[poolptr]:=buffer[l];poolptr:=poolptr+1;
end;l:=l-1;end;strptr:=strptr+1;strstart[strptr]:=poolptr;end;
{:75}{81:}function interpretxxx:keywordcode;label 30,31,45;
var k:integer;j:integer;l:0..13;m:keywordcode;n1:0..13;n2:poolpointer;
c:keywordcode;begin c:=19;curstring:=0;case curgf of 244:goto 30;
243:begin k:=signedquad;goto 30;end;239:k:=getbyte;240:k:=gettwobytes;
241:k:=getthreebytes;242:k:=signedquad;
others:begin writeln(stderr,'internal error');uexit(1);end;end;
{82:}j:=0;if k<2 then goto 45;while true do begin l:=j;
if j=k then goto 31;if j=13 then goto 45;j:=j+1;buffer[j]:=getbyte;
if buffer[j]=32 then goto 31;end;
31:{83:}for m:=0 to 18 do if(strstart[m+1]-strstart[m])=l then begin n1
:=0;n2:=strstart[m];
...
begin if poolptr+k-j>poolsize then begin writeln(stderr,
...
curgf:=getbyte;end{:82};30:curgf:=getbyte;interpretxxx:=c;end;
{:81}{84:}function getyyy:scaled;var v:scaled;
...
241:k:=getthreebytes;242:k:=signedquad;
others:begin writeln(stderr,'internal error');uexit(1);end;end;
for j:=1 to k do curgf:=getbyte;30:curgf:=getbyte;end;
{:85}{89:}procedure beginname;begin areadelimiter:=0;extdelimiter:=0;
end;{:89}{90:}function morename(c:ASCIIcode):boolean;
...
{:91}{92:}procedure packfilename(n,a,e:strnumber);var k:integer;
c:ASCIIcode;j:integer;namelength:integer;
begin namelength:=(strstart[a+1]-strstart[a])+(strstart[n+1]-strstart[n]
)+(strstart[e+1]-strstart[e]);
nameoffile:=xmallocarray(ASCIIcode,namelength);k:=-1;
...
nameoffile[k]:=xchr[c];end;nameoffile[namelength]:=0;end;
{:92}{94:}procedure startgf;label 30;var argbuffer:cstring;
argbufptr:integer;begin argbuffer:=cmdline(optind);argbufptr:=0;
...
linelength:=linelength+1;argbufptr:=argbufptr+1;end;bufptr:=0;
buffer[linelength]:=63;while buffer[bufptr]=32 do bufptr:=bufptr+1;
if bufptr<linelength then begin{95:}if buffer[linelength-1]=47 then
begin interaction:=true;linelength:=linelength-1;end;beginname;
...
packfilename(jobname,0,20);opendvifile;end;{:94}{108:}procedure dviswap;
begin if dviptr>(2147483647-dvioffset)then begin writeln(stderr,
'dvi length exceeds "7FFFFFFF');uexit(1);end;
...
{:110}{111:}procedure dvifontdef(f:internalfontnumber);var k:integer;
...
dviptr:=dviptr+1;if dviptr=dvilimit then dviswap;end;
{112:}for k:=strstart[fontarea[f]]to strstart[fontarea[f]+1]-1 do begin
...
n2:poolpointer;
begin if interaction then{99:}while true do begin 45:begin writeln(
stdout);write(stdout,'Special font substitution: ');end;22:inputln;
...
begin if poolptr+linelength-bufptr>poolsize then begin writeln(stderr,
...
if fontarea[f]=34 then fontarea[f]:=0;dvifontdef(f);end;
{137:}if(strstart[fontname[4]+1]-strstart[fontname[4]])=0 then ruleslant
:=0.0 else begin ruleslant:=fontinfo[1+parambase[4]].sc/65536;
...
if not(i.B0>0)then begin writeln(stderr,'Missing pixel char!');uexit(1);
...
if xratio*yratio=0 then begin writeln(stderr,'Vanishing pixel size!');
uexit(1);end;fudgefactor:=(slantratio/xratio)/yratio;
{:169}{175:}grayrulethickness:=fontinfo[8+parambase[3]].sc;
if grayrulethickness=0 then grayrulethickness:=26214;
{:175}{184:}i:=fontinfo[charbase[3]+0].qqqq;
if not(i.B0>0)then begin writeln(stderr,'Missing dot char!');uexit(1);
...
halfxheight:=thricexheight div 6;
{:184}{205:}for k:=0 to 4095 do b[k]:=0;
...
dviptr:=dviptr+1;if dviptr=dvilimit then dviswap;end;end;
{:113}{114:}procedure dviscaled(x:real);var n:integer;m:integer;
k:integer;begin n:=round(x/6553.6);
if n<0 then begin begin dvibuf[dviptr]:=45;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;n:=-n;end;m:=n div 10;k:=0;
repeat k:=k+1;buffer[k]:=(m mod 10)+48;m:=m div 10;until m=0;
repeat begin dvibuf[dviptr]:=buffer[k];dviptr:=dviptr+1;
...
dviptr:=dviptr+1;if dviptr=dvilimit then dviswap;end;end;end;
{:114}{116:}procedure hbox(s:strnumber;f:internalfontnumber;
sendit:boolean);label 22,30;var k,endk,maxk:poolpointer;
i,j:fourquarters;curl:0..256;curr:0..256;bchar:0..256;
stackptr:0..liglookahead;l:fontindex;kernamount:scaled;hd:eightbits;
x:scaled;savec:ASCIIcode;begin boxwidth:=0;boxheight:=0;boxdepth:=0;
k:=strstart[s];maxk:=strstart[s+1];savec:=strpool[maxk];
strpool[maxk]:=32;
while k<maxk do begin if strpool[k]=32 then{119:}begin boxwidth:=
boxwidth+fontinfo[2+parambase[f]].sc;
if sendit then begin begin dvibuf[dviptr]:=146;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;
dvifour(fontinfo[2+parambase[f]].sc);end;k:=k+1;
end{:119}else begin endk:=k;repeat endk:=endk+1;until strpool[endk]=32;
...
l:=l+j.B0+1;end;30:{:120};
{121:}if(i.B0>0)then begin boxwidth:=boxwidth+fontinfo[widthbase[f]+i.B0
].sc+kernamount;hd:=i.B1-0;x:=fontinfo[heightbase[f]+(hd)div 16].sc;
...
end;end;strpool[maxk]:=savec;end;
{:116}{138:}procedure slantcomplaint(r:real);
...
printreal(r,10,5);write(stdout,'!');slantreported:=r;end;end;
{:138}{141:}function getavail:nodepointer;begin maxnode:=maxnode+1;
...
if yb[p]-yy[p]>maxdepth then maxdepth:=yb[p]-yy[p];end;
{:143}{145:}function overlap(p,q:nodepointer):boolean;label 10;
var ythresh:scaled;xleft,xright,ytop,ybot:scaled;r:nodepointer;
begin xleft:=xl[p];xright:=xr[p];ytop:=yt[p];ybot:=yb[p];
{146:}ythresh:=ybot+maxheight;r:=next[q];
...
while yy[r]>ythresh do begin if ybot>yt[r]then if xleft<xr[r]then if
xright>xl[r]then if ytop<yb[r]then begin overlap:=true;goto 10;end;
r:=prev[r];end{:147};overlap:=false;10:end;
{:145}{150:}function nearestdot(p:nodepointer;d0:scaled):nodepointer;
...
if d<d0 then twin:=true else if d<dmin then begin dmin:=d;bestq:=q;end;
q:=prev[q];end{:152};nearestdot:=bestq;end;
{:150}{167:}procedure convert(x,y:scaled);begin x:=x+xoffset;
y:=y+yoffset;dviy:=-round(yratio*y)+deltay;
dvix:=round(xratio*x+slantratio*y)+deltax;end;
{:167}{171:}procedure dvigoto(x,y:scaled);
...
if dviptr=dvilimit then dviswap;end;dvifour(y);end;end;
{:171}{185:}procedure topcoords(p:nodepointer);
begin xx[p]:=dvix-(boxwidth div 2);xl[p]:=xx[p]-delta;
xr[p]:=xx[p]+boxwidth+delta;yb[p]:=dviy-dotheight;yy[p]:=yb[p]-boxdepth;
yt[p]:=yy[p]-boxheight-delta;end;
{:185}{186:}procedure botcoords(p:nodepointer);
begin xx[p]:=dvix-(boxwidth div 2);xl[p]:=xx[p]-delta;
...
yb[p]:=yy[p]+boxdepth+delta;yt[p]:=yy[p]-boxheight-delta;end;
{:186}{194:}function placelabel(p:nodepointer):boolean;label 10,40;
var oct:0..15;dfl:nodepointer;begin hbox(info[p],2,false);dvix:=xx[p];
dviy:=yy[p];{195:}dfl:=xl[p];oct:=xr[p];
{196:}case oct of 0,4,9,13:leftcoords(p);1,2,8,11:botcoords(p);
3,7,10,14:rightcoords(p);6,5,15,12:topcoords(p);end;
if not overlap(p,dfl)then goto 40{:196};
{197:}case oct of 0,3,15,12:botcoords(p);1,5,10,14:leftcoords(p);
2,6,9,13:rightcoords(p);7,4,8,11:topcoords(p);end;
if not overlap(p,dfl)then goto 40{:197};
{198:}case oct of 0,3,14,13:topcoords(p);1,5,11,15:rightcoords(p);
2,6,8,12:leftcoords(p);7,4,9,10:botcoords(p);end;
if not overlap(p,dfl)then goto 40{:198};
{199:}case oct of 0,4,8,12:rightcoords(p);1,2,9,10:topcoords(p);
...
blankrows:=0;curgf:=getbyte;
while true do begin{213:}repeat{214:}if blankrows>0 then blankrows:=
blankrows-1 else if curgf<>69 then begin x:=z;
if startingcol>x then startingcol:=x;
{215:}while true do begin 22:if(curgf>=74)and(curgf<=238)then begin z:=
curgf-74;paintblack:=true;curgf:=getbyte;goto 31;
...
others:begin writeln(stderr,'Bad GF file: ','Improper opcode',
'! (at byte ',curloc-1:1,')');uexit(1);end end;
{216:}if x+k>finishingcol then finishingcol:=x+k;
...
dviptr:=dviptr+1;if dviptr=dvilimit then dviswap;end;
{209:}begin dvibuf[dviptr]:=146;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;
dvifour(round(unscxratio*j+unscslantratio*y)+deltax){:209};
repeat v:=b[a[j]];a[j]:=a[j]-c[v];k:=j;j:=j+1;
while b[a[j]]=v do begin a[j]:=a[j]-c[v];j:=j+1;end;k:=j-k;
{210:}21:if k=1 then typeset(v)else begin i:=fontinfo[charbase[3]+v].
qqqq;if((i.B2-0)mod 4)=2 then begin if odd(k)then typeset(v);k:=k div 2;
...
buffer[3]:=115;buffer[2]:=101;buffer[1]:=116;firststring(18);
{:77}{78:}l:=7;buffer[7]:=46;buffer[6]:=50;buffer[5]:=54;buffer[4]:=48;
...
firststring(34);{:88};startgf;
{221:}if getbyte<>247 then begin writeln(stderr,'Bad GF file: ',
'No preamble','! (at byte ',curloc-1:1,')');uexit(1);end;
if getbyte<>131 then begin writeln(stderr,'Bad GF file: ','Wrong ID',
...
prev[maxlabels]:=0;maxheight:=0;maxdepth:=0;
{:144}{156:}rulethickness:=0;offsetx:=0;offsety:=0;xoffset:=0;
yoffset:=0;preminx:=268435456;premaxx:=-268435456;preminy:=268435456;
...
0:{163:}if(labeltype<47)or(labeltype>56)then begin writeln(stdout);
...
{109:}if dvilimit=halfbuf then writedvi(halfbuf,dvibufsize-1);
if dviptr>(2147483647-dvioffset)then begin writeln(stderr,
'dvi length exceeds "7FFFFFFF');uexit(1);end;
...
;uexit(1);end;{164:}begin if fontsnotloaded then loadfonts;
{165:}if curgf=67 then begin ext:=signedquad;charcode:=ext mod 256;
...
uexit(1);end{:165};
{170:}if preminx<minx*65536 then offsetx:=offsetx+minx*65536-preminx;
...
if verbose then begin write(stdout,'[',totalpages:1);fflush(stdout);end;
...
convert(xr[p],yb[p]);
if abs(tempx-dvix)<6554 then{176:}begin if tempy>dviy then begin k:=
tempy;tempy:=dviy;dviy:=k;end;dvigoto(dvix-(xx[p]div 2),dviy);
begin dvibuf[dviptr]:=137;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;dvifour(dviy-tempy);dvifour(xx[p]);
begin dvibuf[dviptr]:=142;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;
end{:176}else if abs(tempy-dviy)<6554 then{177:}begin if tempx<dvix then
begin k:=tempx;tempx:=dvix;dvix:=k;end;dvigoto(dvix,dviy+(xx[p]div 2));
begin dvibuf[dviptr]:=137;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;dvifour(xx[p]);dvifour(tempx-dvix);
begin dvibuf[dviptr]:=142;dviptr:=dviptr+1;
if dviptr=dvilimit then dviswap;end;
end{:177}else{178:}if(ruleslant=0)or(abs(tempx+ruleslant*(tempy-dviy)-
dvix)>xx[p])then slantcomplaint((dvix-tempx)/(tempy-dviy))else begin if
tempy>dviy then begin k:=tempy;tempy:=dviy;dviy:=k;k:=tempx;tempx:=dvix;
dvix:=k;end;m:=round((dviy-tempy)/slantunit);
if m>0 then begin dvigoto(dvix,dviy);q:=((m-1)div slantn)+1;k:=m div q;
p:=m mod q;q:=q-p;{179:}typeset(k);dy:=round(k*slantunit);
...
if dviptr=dvilimit then dviswap;end;end;end{:178};end;end{:173};
{181:}overflowline:=1;
if next[maxlabels]<>0 then begin next[labeltail]:=0;
...
if prev[p]>48 then begin next[q]:=next[p];
{190:}begin hbox(info[p],2,false);dvix:=xx[p];dviy:=yy[p];
...
if prev[p]=47 then next[q]:=r else q:=p;end;end{:193};
{200:}{201:}p:=next[0];while p<>maxlabels do begin q:=next[p];
...
dviptr:=dviptr+1;if dviptr=dvilimit then dviswap;end;
{203:}if overflowline>1 then pagewidth:=overcol+10000000;
...
fflush(stdout);end;end{:164};curgf:=getbyte;strptr:=initstrptr;
