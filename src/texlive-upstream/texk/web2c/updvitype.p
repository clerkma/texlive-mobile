...
{:4}const{5:}maxfonts=500;maxwidths=25000;linelength=79;stacksize=100;
namesize=10000;{:5}type{8:}ASCIIcode=0..255;
{:8}{9:}textfile=packed file of ASCIIcode;{:9}{21:}eightbits=0..255;
bytefile=packed file of eightbits;{:21}{32:}jfmchartypehashvalue=0..346;
{:32}var{10:}xord:array[ASCIIcode]of ASCIIcode;
xchr:array[0..255]of ASCIIcode;{:10}{22:}dvifile:bytefile;
tfmfile:bytefile;{:22}{24:}curloc:integer;curname:^char;
...
widthptr:0..maxwidths;fntjfmp:array[0..maxfonts]of boolean;
jfmcharcode:array[0..maxwidths]of integer;
jfmchartype:array[0..maxwidths]of integer;
jfmcharfont:array[0..maxwidths]of integer;jfmchartypecount:integer;
curjfmchartype:integer;
{:30}{33:}jfmchartypehashtable:array[jfmchartypehashvalue]of integer;
jfmchartypehashlink:array[0..maxwidths]of integer;
{:33}{36:}ptexp:boolean;dd:eightbits;
ddstack:array[0..stacksize]of eightbits;
{:36}{39:}inwidth:array[0..255]of integer;tfmchecksum:integer;
tfmdesignsize:integer;tfmconv:real;
{:39}{45:}pixelwidth:array[0..maxwidths]of integer;conv:real;
trueconv:real;numerator,denominator:integer;mag:integer;
{:45}{47:}outmode:0..4;maxpages:integer;resolution:real;newmag:integer;
{:47}{48:}startcount:array[0..9]of integer;
startthere:array[0..9]of boolean;startvals:0..9;
count:array[0..9]of integer;{:48}{63:}inpostamble:boolean;
{:63}{73:}textptr:0..linelength;
textbuf:array[1..linelength]of ASCIIcode;
{:73}{80:}h,v,w,x,y,z,hh,vv:integer;
hstack,vstack,wstack,xstack,ystack,zstack:array[0..stacksize]of integer;
hhstack,vvstack:array[0..stacksize]of integer;{:80}{81:}maxv:integer;
maxh:integer;maxs:integer;maxvsofar,maxhsofar,maxssofar:integer;
totalpages:integer;pagecount:integer;{:81}{86:}s:integer;ss:integer;
curfont:integer;showing:boolean;{:86}{105:}oldbackpointer:integer;
newbackpointer:integer;started:boolean;{:105}{109:}postloc:integer;
firstbackpointer:integer;startloc:integer;afterpre:integer;
{:109}{116:}k,m,n,p,q:integer;{:116}{130:}showopcodes:cinttype;
{:130}{134:}dviname:constcstring;{:134}{120:}procedure parsearguments;
const noptions=10;var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
endnum:^char;begin{121:}currentoption:=0;
...
{:121}{122:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:122}{123:}longoptions[currentoption].name:='output-level';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
outmode:=4;{:123}{124:}longoptions[currentoption].name:='page-start';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:124}{126:}longoptions[currentoption].name:='max-pages';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
maxpages:=1000000;{:126}{127:}longoptions[currentoption].name:='dpi';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
resolution:=300.0;
{:127}{128:}longoptions[currentoption].name:='magnification';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
newmag:=0;{:128}{129:}longoptions[currentoption].name:='show-opcodes';
...
{:129}{131:}longoptions[currentoption].name:='sjis-terminal';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(sjisterminal);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;
{:131}{132:}longoptions[currentoption].name:='kanji';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:132}{133:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:133};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('updvitype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(UPDVITYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is upDVItype, Version 3.6-p0.4-u1.21',nil,
'D.E. Knuth',nil);
end else if(strcmp(longoptions[optionindex].name,'output-level')=0)then
begin if(optarg[0]<'0')or(optarg[0]>'4')or(optarg[1]<>0)then begin
writeln(stderr,'Value for --output-level must be >= 0 and <= 4.');
uexit(1);end;outmode:=optarg[0]-'0';
end else if(strcmp(longoptions[optionindex].name,'page-start')=0)then
begin{125:}k:=0;m:=0;
while optarg[m]do begin if optarg[m]=42 then begin startthere[k]:=false;
m:=m+1;end else if optarg[m]=46 then begin k:=k+1;
if k>=10 then begin writeln(stderr,'updvitype',
': More than ten count registers specified.');uexit(1);end;m:=m+1;
end else begin startcount[k]:=strtol(optarg+m,addressof(endnum),10);
if endnum=optarg+m then begin writeln(stderr,'updvitype',
': -page-start values must be numeric or *.');uexit(1);end;
startthere[k]:=true;m:=m+endnum-(optarg+m);end;end;startvals:=k;{:125};
end else if(strcmp(longoptions[optionindex].name,'max-pages')=0)then
begin maxpages:=atou(optarg);
end else if(strcmp(longoptions[optionindex].name,'dpi')=0)then begin
resolution:=atof(optarg);
end else if(strcmp(longoptions[optionindex].name,'magnification')=0)then
begin newmag:=atou(optarg);
end else if(strcmp(longoptions[optionindex].name,'kanji')=0)then begin
setpriorfileenc;if(not setencstring(optarg,optarg))then begin writeln(
'Bad kanji encoding "',stringcast(optarg),'".');end;end;
until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'updvitype',
': Need exactly one file argument.');usage('updvitype');end;
dviname:=extendfilename(cmdline(optind),'dvi');end;
{:120}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'updvitype');initkanji;parsearguments;
write(stdout,'This is upDVItype, Version 3.6-p0.4-u1.21');
write(stdout,' (');write(stdout,conststringcast(getencstring));
write(stdout,')');writeln(stdout,versionstring);
...
{:11}{12:}for i:=0 to 255 do xord[chr(i)]:=32;
for i:=32 to 126 do xord[xchr[i]]:=i;
{:12}{34:}for i:=0 to 346 do jfmchartypehashtable[i]:=0;
jfmchartype[0]:=0;jfmchartypecount:=1;{:34}{37:}nf:=0;widthptr:=0;
fontname[0]:=1;fontspace[maxfonts]:=0;fontbc[maxfonts]:=1;
fontec[maxfonts]:=0;{:37}{64:}inpostamble:=false;{:64}{74:}textptr:=0;
{:74}{82:}maxv:=2147483548;maxh:=2147483548;maxs:=stacksize+1;
maxvsofar:=0;maxhsofar:=0;maxssofar:=0;pagecount:=0;
{:82}{106:}oldbackpointer:=-1;started:=false;{:106}end;
{:3}{23:}procedure opendvifile;begin resetbin(dvifile,dviname);
curloc:=0;end;procedure opentfmfile;var fullname:^char;
begin fullname:=kpsefindtfm(curname);
if fullname then begin tfmfile:=fopen(fullname,FOPENRBINMODE);
end else begin tfmfile:=nil;end;end;{:23}{26:}procedure readtfmword;
...
begin xfseek(dvifile,0,2,dviname);curloc:=xftell(dvifile,dviname);
dvilength:=curloc;end;procedure movetobyte(n:integer);
begin xfseek(dvifile,n,0,dviname);curloc:=n;end;
{:28}{35:}function getjfmchartype(fntn:integer;jfmc:integer):integer;
var p:integer;ct:integer;
begin p:=jfmchartypehashtable[(jfmc+fntn)mod 347];ct:=0;
while p<>0 do if(jfmcharcode[p]=jfmc)and(jfmcharfont[p]=fntn)then begin
ct:=jfmchartype[p];p:=0;end else p:=jfmchartypehashlink[p];
getjfmchartype:=ct;end;{:35}{38:}procedure printfont(f:integer);
var k:0..namesize;
...
{:38}{40:}function inTFM(z:integer):boolean;label 9997,9998,9999;
var k:integer;lh:integer;nt:integer;jfmh:integer;nw:integer;
wp:0..maxwidths;alpha,beta:integer;begin{41:}readtfmword;lh:=b0*256+b1;
if(lh=11)or(lh=9)then begin write(stdout,' (JFM');fntjfmp[nf]:=true;
if lh=9 then write(stdout,' tate');write(stdout,')');nt:=b2*256+b3;
readtfmword;end else begin nt:=0;fntjfmp[nf]:=false;end;lh:=b2*256+b3;
readtfmword;fontbc[nf]:=b0*256+b1;fontec[nf]:=b2*256+b3;
...
goto 9997;end;for k:=1 to nt do begin readtfmword;
jfmcharcode[jfmchartypecount]:=b0*256+b1;
jfmchartype[jfmchartypecount]:=b2*256+b3;
jfmcharfont[jfmchartypecount]:=nf;
jfmh:=(jfmcharcode[jfmchartypecount]+nf)mod 347;
jfmchartypehashlink[jfmchartypecount]:=jfmchartypehashtable[jfmh];
jfmchartypehashtable[jfmh]:=jfmchartypecount;
jfmchartypecount:=jfmchartypecount+1 end;{:41};
{42:}if wp>0 then for k:=widthptr to wp-1 do begin readtfmword;
if b0>nw then goto 9997;width[k]:=b0;end;{:42};
{43:}{44:}begin alpha:=16;while z>=8388608 do begin z:=z div 2;
alpha:=alpha+alpha;end;beta:=256 div alpha;alpha:=alpha*z;end{:44};
...
end{:43};{46:}if inwidth[0]<>0 then goto 9997;
...
pixelwidth[k]:=round(conv*(width[k]));end{:46};widthptr:=wp;inTFM:=true;
goto 9999;9997:writeln(stdout,'---not loaded, TFM file is bad');
9998:inTFM:=false;9999:end;{:40}{50:}function startmatch:boolean;
var k:0..9;match:boolean;begin match:=true;
for k:=0 to startvals do if startthere[k]and(startcount[k]<>count[k])
then match:=false;startmatch:=match;end;
{:50}{65:}procedure definefont(e:integer);var f:0..maxfonts;p:integer;
n:integer;c,q,d,m:integer;r:0..namesize;j,k:0..namesize;
mismatch:boolean;begin if nf=maxfonts then begin writeln(stderr,
'DVItype capacity exceeded (max fonts=',maxfonts:1,')!');uexit(1);end;
fontnum[nf]:=e;f:=0;while fontnum[f]<>e do f:=f+1;{67:}c:=signedquad;
...
if not showing then if m<>1000 then write(stdout,' scaled ',m:1){:67};
...
if f=nf then{68:}begin{72:}r:=fontname[nf+1]-fontname[nf];
curname:=xmallocarray(char,r);
for k:=fontname[nf]to fontname[nf+1]do begin curname[k-fontname[nf]]:=
xchr[names[k]];end;curname[r]:=0;{:72};opentfmfile;
...
'---not loaded, bad design size (',d:1,')!')else if inTFM(q)then{69:}
...
write(stdout,' (this font is magnified ',d:1,'%)');end;nf:=nf+1;
end{:69};end;if outmode=0 then writeln(stdout,' ');
if tfmfile then xfclose(tfmfile,curname);free(curname);
end{:68}else{66:}begin if fontchecksum[f]<>c then writeln(stdout,
...
'---font name doesn''t match previous definition!');end{:66};end;
{:65}{75:}procedure flushtext;var k:0..linelength;
begin if textptr>0 then begin if outmode>0 then begin write(stdout,'[');
for k:=1 to textptr do write(stdout,xchr[textbuf[k]]);
writeln(stdout,']');end;textptr:=0;end;end;
{:75}{76:}procedure outtext(c:ASCIIcode);
begin if textptr=linelength-2 then flushtext;textptr:=textptr+1;
if c>=177 then textbuf[textptr]:=63 else textbuf[textptr]:=c;end;
{:76}{77:}procedure outkanji(c:integer);
begin if textptr>=linelength-5 then flushtext;c:=toBUFF(fromDVI(c));
if BYTE1(c)<>0 then begin textptr:=textptr+1;textbuf[textptr]:=BYTE1(c);
end;if BYTE2(c)<>0 then begin textptr:=textptr+1;
textbuf[textptr]:=BYTE2(c);end;
if BYTE3(c)<>0 then begin textptr:=textptr+1;textbuf[textptr]:=BYTE3(c);
end;textptr:=textptr+1;textbuf[textptr]:=BYTE4(c);end;
{:77}{78:}ifdef('HEX_CHAR_CODE')procedure printhexnumber(c:integer);
var n:integer;b:array[1..8]of integer;begin n:=1;
while(n<8)and(c<>0)do begin b[n]:=c mod 16;c:=c div 16;n:=n+1 end;
write(stdout,'"');if n=1 then write(stdout,xchr['0'])else begin n:=n-1;
while n>0 do begin if b[n]<=9 then write(stdout,(b[n]):1)else case b[n]
of 10:write(stdout,xchr['A']);11:write(stdout,xchr['B']);
12:write(stdout,xchr['C']);13:write(stdout,xchr['D']);
14:write(stdout,xchr['E']);15:write(stdout,xchr['F']);end;
n:=n-1 end end end;
endif('HEX_CHAR_CODE'){:78}{83:}function firstpar(o:eightbits):integer;
...
;138,139,140,141,142,247,248,249,250,251,252,253,254:firstpar:=0;
147:firstpar:=w;152:firstpar:=x;161:firstpar:=y;166:firstpar:=z;
255:firstpar:=getbyte;
171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,
189,190,191,192,193,194,195,196,197,198,199,200,201,202,203,204,205,206,
207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,222,223,224,
225,226,227,228,229,230,231,232,233,234:firstpar:=o-171;
others:begin writeln(stderr,'internal error');uexit(1);end;end;end;
{:83}{84:}function rulepixels(x:integer):integer;var n:integer;
begin n:=trunc(conv*x);
if n<conv*x then rulepixels:=n+1 else rulepixels:=n;end;
{:84}{87:}{90:}function specialcases(o:eightbits;p,a:integer):boolean;
label 46,44,30,9998;var q:integer;k:integer;badchar:boolean;
pure:boolean;vvv:integer;begin pure:=true;
case o of{93:}157,158,159,160:begin if abs(p)>=5*fontspace[curfont]then
begin if dd=0 then vv:=round(conv*(v+p))else hh:=round(conv*(h-p))end
else if dd=0 then vv:=vv+round(conv*(p))else hh:=hh-round(conv*(p));
if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','down',o-156:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 44;
end;161,162,163,164,165:begin y:=p;
if abs(p)>=5*fontspace[curfont]then begin if dd=0 then vv:=round(conv*(v
+p))else hh:=round(conv*(h-p))end else if dd=0 then vv:=vv+round(conv*(p
))else hh:=hh-round(conv*(p));if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','y',o-161:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 44;
end;166,167,168,169,170:begin z:=p;
if abs(p)>=5*fontspace[curfont]then begin if dd=0 then vv:=round(conv*(v
+p))else hh:=round(conv*(h-p))end else if dd=0 then vv:=vv+round(conv*(p
))else hh:=hh-round(conv*(p));if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','z',o-166:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 44;
end;
{:93}{94:}171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,
186,187,188,189,190,191,192,193,194,195,196,197,198,199,200,201,202,203,
204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220,221,
222,223,224,225,226,227,228,229,230,231,232,233,234:begin if outmode>0
then begin flushtext;showing:=true;write(stdout,a:1,': ','fntnum',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 46;
end;235,236,237,238:begin if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','fnt',o-234:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 46;
end;243,244,245,246:begin if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','fntdef',o-242:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
definefont(p);goto 30;end;
{:94}239,240,241,242:{95:}begin if outmode>0 then begin flushtext;
showing:=true;write(stdout,a:1,': ','xxx ''');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
badchar:=false;if p<0 then if not showing then begin flushtext;
showing:=true;write(stdout,a:1,': ','string of negative length!');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
...
goto 30;end{:95};247:begin if not showing then begin flushtext;
...
44:{100:}if dd=0 then begin if(v>0)and(p>0)then if v>2147483647-p then
begin if not showing then begin flushtext;showing:=true;
...
maxvsofar:=abs(v);end;goto 30 end else begin p:=-p;
if(h>0)and(p>0)then if h>2147483647-p then begin if not showing then
begin flushtext;showing:=true;
write(stdout,a:1,': ','arithmetic overflow! parameter changed from ',p:1
,' to ',2147483647-h:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','arithmetic overflow! parameter changed from '
,p:1,' to ',2147483647-h:1);p:=2147483647-h;end;
if(h<0)and(p<0)then if-h>p+2147483647 then begin if not showing then
begin flushtext;showing:=true;
write(stdout,a:1,': ','arithmetic overflow! parameter changed from ',p:1
,' to ',(-h)-2147483647:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','arithmetic overflow! parameter changed from '
,p:1,' to ',(-h)-2147483647:1);p:=(-h)-2147483647;end;
vvv:=round(conv*(h+p));
if abs(vvv-hh)>2 then if vvv>hh then hh:=vvv-2 else hh:=vvv+2;
if showing then if outmode>2 then begin write(stdout,' h:=',h:1);
if p>=0 then write(stdout,'+');
write(stdout,p:1,'=',h+p:1,', hh:=',hh:1);end;h:=h+p;
...
maxhsofar:=abs(h);end;goto 30 end{:100};46:{102:}fontnum[nf]:=p;
...
);printfont(curfont);end;goto 30{:102};9998:pure:=false;
30:specialcases:=pure;end;{:90}function dopage:boolean;
...
hh:=0;vv:=0;dd:=0;while true do{88:}begin a:=curloc;showing:=false;
o:=getbyte;p:=firstpar(o);
if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;
{89:}if o<128 then{96:}begin if(o>32)and(o<=126)then begin outtext(p);
if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','setchar',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
end else if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','setchar',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
ifdef('HEX_CHAR_CODE')write(stdout,' (');printhexnumber(p);
write(stdout,')');endif('HEX_CHAR_CODE')goto 41;
end{:96}else case o of 128,129,130,131,133,134,135,136:begin if fntjfmp[
curfont]=true then begin curjfmchartype:=getjfmchartype(curfont,p);
outkanji(p);if o<133 then begin if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','set',o-127:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
end else begin if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','put',o-132:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;end;
ifdef('HEX_CHAR_CODE')write(stdout,'(');printhexnumber(p);
write(stdout,')');
endif('HEX_CHAR_CODE')write(stdout,' type=',curjfmchartype);
p:=curjfmchartype end else begin if o<133 then begin if outmode>0 then
begin flushtext;showing:=true;
write(stdout,a:1,': ','set',o-127:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
end else begin if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','put',o-132:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;end;
ifdef('HEX_CHAR_CODE')write(stdout,'(');printhexnumber(p);
write(stdout,')');endif('HEX_CHAR_CODE')end;goto 41;end;
132:begin if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','setrule');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 42;
end;137:begin if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','putrule');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 42;
end;255:begin if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','dir ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;dd:=p;
goto 30;end;{91:}138:begin if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','nop');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;goto 30;
end;139:begin if not showing then begin flushtext;showing:=true;
...
:1,')');if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','DVItype capacity exceeded (stack size=',
stacksize:1,')');goto 9998;end;end;ddstack[s]:=dd;hstack[s]:=h;
vstack[s]:=v;wstack[s]:=w;xstack[s]:=x;ystack[s]:=y;zstack[s]:=z;
hhstack[s]:=hh;vvstack[s]:=vv;s:=s+1;ss:=s-1;goto 45;end;
142:begin if outmode>0 then begin flushtext;showing:=true;
write(stdout,a:1,': ','pop');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;
if s=0 then if not showing then begin flushtext;showing:=true;
write(stdout,a:1,': ','(illegal at level zero)!');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','(illegal at level zero)!')else begin s:=s-1;
hh:=hhstack[s];vv:=vvstack[s];dd:=ddstack[s];h:=hstack[s];v:=vstack[s];
w:=wstack[s];x:=xstack[s];y:=ystack[s];z:=zstack[s];end;ss:=s;goto 45;
end;{:91}{92:}143,144,145,146:begin if(p>=fontspace[curfont])or(p<=-4*
fontspace[curfont])then begin outtext(32);
if dd=0 then hh:=round(conv*(h+p))else vv:=round(conv*(v+p));
end else if dd=0 then hh:=hh+round(conv*(p))else vv:=vv+round(conv*(p));
if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','right',o-142:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;q:=p;
goto 43;end;147,148,149,150,151:begin w:=p;
if(p>=fontspace[curfont])or(p<=-4*fontspace[curfont])then begin outtext(
32);if dd=0 then hh:=round(conv*(h+p))else vv:=round(conv*(v+p));
end else if dd=0 then hh:=hh+round(conv*(p))else vv:=vv+round(conv*(p));
if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','w',o-147:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;q:=p;
goto 43;end;152,153,154,155,156:begin x:=p;
if(p>=fontspace[curfont])or(p<=-4*fontspace[curfont])then begin outtext(
32);if dd=0 then hh:=round(conv*(h+p))else vv:=round(conv*(v+p));
end else if dd=0 then hh:=hh+round(conv*(p))else vv:=vv+round(conv*(p));
if outmode>1 then begin showing:=true;
write(stdout,a:1,': ','x',o-152:1,' ',p:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');end;q:=p;
goto 43;end;
{:92}others:if specialcases(o,p,a)then goto 30 else goto 9998 end{:89};
41:{97:}if p<0 then p:=255-((-1-p)mod 256)else if p>=256 then p:=p mod
256;
...
if q=2147483647 then q:=0 else if dd=0 then hh:=hh+pixelwidth[widthbase[
curfont]+p]else vv:=vv+pixelwidth[widthbase[curfont]+p];goto 43{:97};
42:{98:}q:=signedquad;
...
if dd=0 then hh:=hh+rulepixels(q)else vv:=vv+rulepixels(q);goto 43{:98};
43:{99:}if dd=0 then begin if(h>0)and(q>0)then if h>2147483647-q then
begin if not showing then begin flushtext;showing:=true;
...
maxhsofar:=abs(h);end;
goto 30 end else begin if(v>0)and(q>0)then if v>2147483647-q then begin
if not showing then begin flushtext;showing:=true;
write(stdout,a:1,': ','arithmetic overflow! parameter changed from ',q:1
,' to ',2147483647-q:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','arithmetic overflow! parameter changed from '
,q:1,' to ',2147483647-q:1);q:=2147483647-v;end;
if(v<0)and(q<0)then if-v>q+2147483647 then begin if not showing then
begin flushtext;showing:=true;
write(stdout,a:1,': ','arithmetic overflow! parameter changed from ',q:1
,' to ',(-v)-2147483647:1);
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','arithmetic overflow! parameter changed from '
,q:1,' to ',(-v)-2147483647:1);q:=(-v)-2147483647;end;
hhh:=round(conv*(v+q));
if abs(hhh-vv)>2 then if hhh>vv then vv:=hhh-2 else vv:=hhh+2;
if showing then if outmode>2 then begin write(stdout,' v:=',v:1);
if q>=0 then write(stdout,'+');
write(stdout,q:1,'=',v+q:1,', vv:=',vv:1);end;v:=v+q;
if abs(v)>maxvsofar then begin if abs(v)>maxv+99 then begin if not
showing then begin flushtext;showing:=true;
write(stdout,a:1,': ','warning: |v|>',maxv:1,'!');
if showopcodes and(o>=128)then write(stdout,' {',o:1,'}');
end else write(stdout,' ','warning: |v|>',maxv:1,'!');maxv:=abs(v);end;
maxvsofar:=abs(v);end;goto 30 end{:99};
45:{101:}if showing then if outmode>2 then begin writeln(stdout,' ');
begin write(stdout,'level ',ss:1,':(h=',h:1,',v=',v:1,',w=',w:1,',x=',x:
1,',y=',y:1,',z=',z:1);if ptexp then write(stdout,',d=',dd:1);
write(stdout,',hh=',hh:1,',vv=',vv:1,')');end;end;goto 30{:101};
30:if showing then writeln(stdout,' ');end{:88};
9998:writeln(stdout,'!');dopage:=false;9999:end;
{:87}{103:}{107:}procedure scanbop;var k:0..255;
begin repeat if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;k:=getbyte;
if(k>=243)and(k<247)then begin definefont(firstpar(k));k:=138;end;
until k<>138;
if k=248 then inpostamble:=true else begin if k<>139 then begin writeln(
stderr,'Bad DVI file: ','byte ',curloc-1:1,' is not bop','!');uexit(1);
end;newbackpointer:=curloc-1;pagecount:=pagecount+1;
...
oldbackpointer:=newbackpointer;end;end;
{:107}procedure skippages(bopseen:boolean);label 9999;var p:integer;
k:0..255;downthedrain:integer;begin showing:=false;
...
{104:}repeat if eof(dvifile)then begin writeln(stderr,'Bad DVI file: ',
'the file ended prematurely','!');uexit(1);end;k:=getbyte;
...
139,247,248,249,250,251,252,253,254:begin writeln(stderr,
'Bad DVI file: ','illegal command at byte ',curloc-1:1,'!');uexit(1);
end;others:end;until k=140;{:104};bopseen:=false;end;9999:end;
{:103}{111:}procedure readpostamble;var k:integer;p,q,m:integer;
...
if ptexp then writeln(stdout,'pTeX DVI (id=',3:1,')');
if outmode<4 then{112:}begin if maxv+99<maxvsofar then writeln(stdout,
...
pagecount:1,' pages, not ',totalpages:1,'!');end{:112};
{114:}repeat k:=getbyte;if(k>=243)and(k<247)then begin p:=firstpar(k);
definefont(p);writeln(stdout,' ');k:=138;end;until k<>138;
if k<>249 then writeln(stdout,'byte ',curloc-1:1,' is not postpost!')
{:114};{113:}q:=signedquad;
if q<>postloc then writeln(stdout,'bad postamble pointer in byte ',
curloc-4:1,'!');m:=getbyte;
if(m<>2)and(m<>3)then writeln(stdout,'identification in byte ',curloc-1:
1,' should be ',2:1,' or ',3:1,'!');k:=curloc;m:=223;
...
'not enough signature bytes at end of file (',curloc-k:1,')');{:113};
end;{:111}{115:}begin initialize;
{62:}writeln(stdout,'Options selected:');
...
printreal(newmag/1000.0,8,3);writeln(stdout,'')end{:62};
{117:}opendvifile;p:=getbyte;
...
'!');{118:}numerator:=signedquad;denominator:=signedquad;
...
writeln(stdout,' pixels per DVI unit'){:118};p:=getbyte;
write(stdout,'''');while p>0 do begin p:=p-1;
write(stdout,xchr[getbyte]);end;writeln(stdout,'''');
afterpre:=curloc{:117};if outmode=4 then begin{108:}n:=dvilength;
...
;uexit(1);end;movetobyte(m);k:=getbyte;m:=m-1;until k<>223;ptexp:=(k=3);
if(k<>2)and(not ptexp)then begin writeln(stderr,'Bad DVI file: ',
'ID byte is ',k:1,'!');uexit(1);end;movetobyte(m-3);q:=signedquad;
...
firstbackpointer:=signedquad{:108};inpostamble:=true;readpostamble;
inpostamble:=false;{110:}q:=postloc;p:=firstbackpointer;startloc:=-1;
...
if startloc<0 then begin writeln(stderr,
'starting page number could not be found!');uexit(1);end;
if oldbackpointer<0 then startloc:=afterpre;movetobyte(startloc);end;
if pagecount<>totalpages then writeln(stdout,'there are really ',
pagecount:1,' pages, not ',totalpages:1,'!'){:110};end;skippages(false);
if not inpostamble then{119:}begin while maxpages>0 do begin maxpages:=
maxpages-1;writeln(stdout,' ');
write(stdout,curloc-45:1,': beginning of page ');
for k:=0 to startvals do begin write(stdout,count[k]:1);
if k<startvals then write(stdout,'.')else writeln(stdout,' ');end;
if not dopage then begin writeln(stderr,'Bad DVI file: ',
'page ended unexpectedly','!');uexit(1);end;scanbop;
if inpostamble then goto 30;end;30:end{:119};
