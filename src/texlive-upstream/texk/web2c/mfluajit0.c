...
zshowmacro ( halfword p , integer q , integer l ) 
{
  ...
  case 3 : 
    {
      printchar ( 60 ) ;
      printcmdmod ( 57 , mem [p ].hhfield .lhfield ) ;
      print ( 503 ) ;
    } 
    break ;
  case 4 : 
...
{
  /* 30 31 */ halfword p, q  ;
  mfluaprintpath ( h , s , nuline ) ;
  printdiagnostic ( 518 , s , nuline ) ;
  println () ;
  p = h ;
  do {
      q = mem [p ].hhfield .v.RH ;
...
zfillspec ( halfword h ) 
{
  ...
  do {
	...
	r = s ;
      ...
      mfluaPREmovetoedges ( p ) ;
      movetoedges ( m0 , n0 , m1 , n1 ) ;
      mfluaPOSTmovetoedges ( p ) ;
    } 
    p = mem [q ].hhfield .v.RH ;
  } while ( ! ( p == h ) ) ;
  tossknotlist ( h ) ;
...
zdualmoves ( halfword h , halfword p , halfword q ) 
{
  ...
  while ( true ) {
    ...
    while ( mem [r ].hhfield .b1 != k ) {
      ...
      mfluaprinttransitionlinefrom ( xx , yy - 32768L ) ;
	;
#ifdef STAT
      if ( internal [10 ]> 65536L ) 
      {
...
zfillenvelope ( halfword spechead ) 
{
  ...
  do {
    ...
    if ( n1 - n0 >= movesize ) 
    overflow ( 541 , movesize ) ;
    mfluaPREoffsetprep ( p , h ) ;
    offsetprep ( p , h ) ;
    mfluaPOSToffsetprep ( p , h ) ;
    q = p ;
    while ( mem [q ].hhfield .b1 != 0 ) q = mem [q ].hhfield .v.RH ;
    if ( odd ( octantnumber [octant ]) ) 
    {
...
makeopdef ( void ) 
{
  ...
  mem [r ].hhfield .v.RH = scantoks ( 17 , p , 0 , 0 ) ;
  scannerstatus = 0 ;
...
scandef ( void ) 
{
  ...
  mem [q ].hhfield .v.RH = p ;
  if ( m == 1 ) 
  mem [p ].hhfield .v.RH = scantoks ( 17 , r , 0 , n ) ;
  else {
    ...
    mem [q ].hhfield .v.RH = scantoks ( 17 , r , p , n ) ;
  } 
  if ( warninginfo == 21 ) 
...
expand ( void ) 
{
  ...
  case 9 : 
	...
	if ( ( strstart [curexp + 1 ]- strstart [curexp ]) > 0 ) 
	{
	  ...
	  j = strstart [curexp ];
	  curinput .limitfield = k ;
	  mfluarunscript ( j , first , curinput .limitfield ) ;
	  luacurexp = makestring () ;
	  if ( ( strstart [luacurexp + 1 ]- strstart [luacurexp ]) > 0 ) 
	  {
	    beginfilereading () ;
	    {
	      if ( strref [luacurexp ]< 127 ) 
	      incr ( strref [luacurexp ]) ;
	    } 
		...
		if ( strref [luacurexp ]> 1 ) 
		decr ( strref [luacurexp ]) ;
		else flushstring ( luacurexp ) ;
	      } 
	    } 
	  } 
	  flushcurexp ( 0 ) ;
...
openlogfile ( void ) 
{
  ...
  logopened = true ;
  {
    fprintf ( logfile , "%s%s",  "This is MFLuaJIT, Version 2.7182818" , "-0.5" ) ;
...
initgf ( void ) 
{
  ...
  } 
  {
    ...
    openlogfile () ;
    packjobname ( gfext ) ;
    while ( ! bopenout ( gffile ) ) promptfilename ( 757 , gfext ) ;
...
zshipout ( eightbits c ) 
{
  ...
  if ( internal [11 ]> 0 ) 
  printedges ( 1058 , true , xoff , yoff ) ;
} 
void 
...
defdelims ( void ) 
{
  ...
  getclearsymbol () ;
  rdelim = cursym ;
  eqtb [ldelim ].lhfield = 32 ;
  eqtb [ldelim ].v.RH = rdelim ;
  eqtb [rdelim ].lhfield = 63 ;
...
donewinternal ( void ) 
{
  do {
    ...
    overflow ( 935 , maxinternal ) ;
    getclearsymbol () ;
    incr ( intptr ) ;
    eqtb [cursym ].lhfield = 41 ;
...
docull ( void ) 
{
  ...
  if ( curtype != 20 ) 
  {
    ...
    if ( ! getpair ( 68 ) ) 
    goto lab45 ;
    while ( ( curcmd == 67 ) && ( curmod == 16 ) ) if ( scanwith () ) 
    w = curexp ;
    if ( curx > cury ) 
    goto lab45 ;
    if ( keeping == 0 ) 
...
dotfmcommand ( void ) 
{
  ...
  case 4 : 
	...
	do {
	    if ( j > headersize ) 
	  overflow ( 1008 , headersize ) ;
	  headerbyte [j ]= getcode () ;
	  incr ( j ) ;
	} while ( ! ( curcmd != 83 ) ) ;
	else do {
	    if ( j > maxfontdimen ) 
	  overflow ( 1009 , maxfontdimen ) ;
	  while ( j > np ) {
...
scanexpression ( void ) 
{
  ...
  lab22: if ( curcmd <= 52 ) {
	...
	if ( ( d < 49 ) || ( ( d == 49 ) && ( ( mem [p ].hhfield .b0 == 14 ) 
	|| ( mem [p ].hhfield .b0 == 9 ) ) ) ) 
	{
	  cyclehit = false ;
	  {
	  ...
	  mfluaPREmakechoices ( p ) ;
	  makechoices ( p ) ;
	  mfluaPOSTmakechoices ( p ) ;
	  curtype = 9 ;
	  curexp = p ;
	} 
	else {
...
closefilesandterminate ( void ) 
{
  ...
  if ( ( gfprevptr > 0 ) || ( internal [33 ]> 0 ) ) 
  {
	...
	if ( charexists [k ]) {
      ...
      openlogfile () ;
      packjobname ( 1046 ) ;
      while ( ! bopenout ( tfmfile ) ) promptfilename ( 1047 , 1046 ) ;
