{3:}program GFtype(gffile,output);const{5:}inflinelength=20;
suplinelength=1023;{:5}type{8:}ASCIIcode=32..126;
{:8}{9:}textfile=packed file of 0..255;{:9}{20:}eightbits=0..255;
bytefile=packed file of eightbits;{:20}{36:}pixel=0..1;
{:36}var{4:}linelength:integer;maxrows:integer;maxcols:integer;
maxrow:integer;maxcol:integer;{:4}{10:}xord:array[0..255]of ASCIIcode;
xchr:array[0..255]of 0..255;{:10}{21:}gffile:bytefile;
{:21}{23:}curloc:integer;{:23}{25:}wantsmnemonics:cinttype;
wantspixels:cinttype;{:25}{35:}m,n:integer;paintswitch:pixel;
{:35}{37:}imagearray:^pixel;
{:37}{41:}minmstated,maxmstated,minnstated,maxnstated:integer;
...
{:74}{75:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:75}{76:}longoptions[currentoption].name:='mnemonics';
...
{:76}{77:}longoptions[currentoption].name:='images';
longoptions[currentoption].hasarg:=0;
longoptions[currentoption].flag:=addressof(wantspixels);
longoptions[currentoption].val:=1;currentoption:=currentoption+1;
{:77}{78:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:78};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('gftype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(GFTYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is GFtype, Version 3.1',nil,'D.R. Fuchs',nil);
end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'gftype',
': Need exactly one file argument.');usage('gftype');end;end;
{:73}procedure initialize;var i:integer;bounddefault:integer;
boundname:constcstring;begin kpsesetprogramname(argv[0],'gftype');
kpseinitprog('GFTYPE',0,nil,nil);parsearguments;
write(stdout,'This is GFtype, Version 3.1');
writeln(stdout,versionstring);{6:}bounddefault:=500;
boundname:='line_length';
setupboundvariable(addressof(linelength),boundname,bounddefault);;
bounddefault:=8191;boundname:='max_rows';
setupboundvariable(addressof(maxrows),boundname,bounddefault);;
bounddefault:=8191;boundname:='max_cols';
setupboundvariable(addressof(maxcols),boundname,bounddefault);;
begin if linelength<inflinelength then linelength:=inflinelength else if
linelength>suplinelength then linelength:=suplinelength end;
if maxrows>8191 then maxrows:=8191;if maxcols>8191 then maxcols:=8191;
imagearray:=nil;{:6}{11:}for i:=0 to 31 do xchr[i]:='?';xchr[32]:=' ';
...
{:12}{26:}{:26}{47:}for i:=0 to 255 do charptr[i]:=-1;totalchars:=0;
{:47}{63:}minmoverall:=maxint;maxmoverall:=-maxint;minnoverall:=maxint;
maxnoverall:=-maxint;{:63}end;{:3}{22:}procedure opengffile;
begin gffile:=kpseopenfile(cmdline(optind),kpsegfformat);curloc:=0;
{34:}write(stdout,'Options selected: Mnemonic output = ');
...
-256)*256+b)*256+c)*256+d;end;
{:24}{45:}procedure printscaled(s:integer);var delta:integer;
...
238:firstpar:=o-74;others:begin writeln(stderr,'internal error');
uexit(1);end end;end;{:48}{49:}function dochar:boolean;label 9998,9999;
var o:eightbits;p,q:integer;aok:boolean;begin aok:=true;
while true do{50:}begin a:=curloc;o:=getbyte;p:=firstpar(o);
if eof(gffile)then begin writeln(stderr,'Bad GF file: ',
'the file ended prematurely','!');uexit(1);end;
{51:}if o<=67 then{56:}begin if wantsmnemonics then write(stdout,
' paint ');
repeat{57:}if wantsmnemonics then if paintswitch=0 then write(stdout,'('
,p:1,')')else write(stdout,p:1);m:=m+p;
if m>maxmobserved then maxmobserved:=m-1;
if wantspixels then{58:}if paintswitch=1 then if n<=maxrow then begin l
:=m-p;r:=m-1;if r>maxcol then r:=maxcol;m:=l;
while m<=r do begin imagearray[m+(maxcol+1)*n]:=1;m:=m+1;end;m:=l+p;
end{:58};paintswitch:=1-paintswitch{:57};a:=curloc;o:=getbyte;
p:=firstpar(o);if eof(gffile)then begin writeln(stderr,'Bad GF file: ',
'the file ended prematurely','!');uexit(1);end;until o>67;end{:56};
case o of 70,71,72,73:{60:}begin if wantsmnemonics then begin writeln(
stdout);write(stdout,a:1,': ','skip',(o-70)mod 4:1,' ',p:1);end;
...
156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173,
174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,189,190,191,
192,193,194,195,196,197,198,199,200,201:{59:}begin if wantsmnemonics
then begin writeln(stdout);write(stdout,a:1,': ','newrow ',p:1);end;
n:=n+1;m:=p;paintswitch:=1;
if wantsmnemonics then write(stdout,' (n=',maxnstated-n:1,')');end{:59};
202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,
220,221,222,223,224,225,226,227,228,229,230,231,232,233,234,235,236,237,
...
write(stdout,a:1,': ','eoc');end;writeln(stdout);goto 9999;end;
{:52}239,240,241,242:{53:}begin if wantsmnemonics then begin writeln(
stdout);write(stdout,a:1,': ','xxx ''');end;badchar:=false;b:=16;
...
writeln(stdout);end;end{:53};
243:{55:}begin if wantsmnemonics then begin writeln(stdout);
...
'max n should be >=',maxnoverall:1,'!');writeln(stdout);end;
...
{64:}if k<>249 then begin write(stdout,a:1,': ','! ',
'should be postpost!');writeln(stdout);end;
...
if not eof(gffile)then begin writeln(stderr,'Bad GF file: ',
...
if o<>247 then begin writeln(stderr,'Bad GF file: ',
'First byte isn''t start of preamble!','!');uexit(1);end;o:=getbyte;
if o<>131 then begin writeln(stderr,'Bad GF file: ',
'identification byte should be ',131:1,' not ',o:1,'!');uexit(1);end;
o:=getbyte;write(stdout,'''');while o>0 do begin o:=o-1;
write(stdout,xchr[getbyte]);end;writeln(stdout,'''');{:68};
{69:}repeat gfprevptr:=curloc;{70:}repeat a:=curloc;o:=getbyte;
p:=firstpar(o);if eof(gffile)then begin writeln(stderr,'Bad GF file: ',
'the file ended prematurely','!');uexit(1);end;
if o=243 then begin{55:}begin if wantsmnemonics then begin writeln(
stdout);write(stdout,a:1,': ','yyy ',p:1,' (');end;
if wantsmnemonics then begin printscaled(p);write(stdout,')');end;
end{:55};o:=244;
end else if(o>=239)and(o<=242)then begin{53:}begin if wantsmnemonics
then begin writeln(stdout);write(stdout,a:1,': ','xxx ''');end;
badchar:=false;b:=16;if p<0 then begin writeln(stdout);
write(stdout,a:1,': ','! ','string of negative length!');
writeln(stdout);end;while p>0 do begin q:=getbyte;
if(q<32)or(q>126)then badchar:=true;
if wantsmnemonics then begin write(stdout,xchr[q]);
...
if o<>248 then begin if o<>67 then if o<>68 then begin writeln(stderr,
...
if wantspixels then{38:}begin maxcol:=maxmstated-minmstated-1;
if maxcol>maxcols then maxcol:=maxcols;maxrow:=maxnstated-minnstated;
if maxrow>maxrows then maxrow:=maxrows;
if(maxrow>=0)and(maxcol>=0)then imagearray:=xcallocarray(pixel,maxcol,
maxrow);end{:38};m:=0;n:=0;paintswitch:=0;{:71};
if not dochar then begin writeln(stderr,'Bad GF file: ',
'char ended unexpectedly','!');uexit(1);end;maxnobserved:=n;
if wantspixels then{40:}begin{42:}if(maxmobserved>maxcol)or(maxnobserved
>maxrow)then writeln(stdout,
'(The character is too large to be displayed in full.)');
if maxcol>maxmobserved then maxcol:=maxmobserved;
if maxrow>maxnobserved then maxrow:=maxnobserved;{:42};
if maxcol>=0 then{43:}begin writeln(stdout,
'.<--This pixel''s lower left corner is at (',minmstated:1,',',
maxnstated+1:1,') in METAFONT coordinates');n:=0;
while n<=maxrow do begin m:=0;b:=0;
while m<=maxcol do begin if imagearray[m+(maxcol+1)*n]=0 then b:=b+1
else begin while b>0 do begin write(stdout,' ');b:=b-1;end;
write(stdout,'*');end;m:=m+1;end;writeln(stdout);n:=n+1;end;
writeln(stdout,'.<--This pixel''s upper left corner is at (',minmstated:
1,',',maxnstated-maxrow:1,') in METAFONT coordinates');
end{:43}else writeln(stdout,'(The character is entirely blank.)');
if(maxrow>=0)and(maxcol>=0)then begin libcfree(imagearray);
imagearray:=nil;end;end{:40};
{72:}maxmobserved:=minmstated+maxmobserved+1;n:=maxnstated-maxnobserved;
