#define WEAVE
#include "cpascal.h"
#define maxbytes ( 65536L ) 
#define maxnames ( 10220 ) 
...
cinttype noxref  ;
constcstring webname, chgname, texname  ;
...
initialize ( void ) 
{
  ...
  integer h  ;
  ASCIIcode c  ;
  kpsesetprogramname ( argv [0 ], "weave" ) ;
  parsearguments () ;
  history = 0 ;
  xchr [32 ]= ' ' ;
  xchr [33 ]= '!' ;
  ...
  while ( true ) {
	...
	while ( true ) {
	  ...
	  if ( ( ( a != 32 ) && ( a < 140 ) ) || ( a > 142 ) ) 
	  {
		...
		92 ) && ( outbuf [outptr - 2 ]== 89 ) && ( outbuf [outptr - 
		3 ]== 92 ) ) 
		goto lab21 ;
	      } 
	      {
