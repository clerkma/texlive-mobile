...
typedef short jfmchartypehashvalue  ;
ASCIIcode xord[256]  ;
ASCIIcode xchr[256]  ;
bytefile dvifile  ;
bytefile tfmfile  ;
...
integer nf  ;
integer widthptr  ;
boolean fntjfmp[maxfonts + 1]  ;
integer jfmcharcode[maxwidths + 1]  ;
integer jfmchartype[maxwidths + 1]  ;
integer jfmcharfont[maxwidths + 1]  ;
integer jfmchartypecount  ;
integer curjfmchartype  ;
integer jfmchartypehashtable[347]  ;
integer jfmchartypehashlink[maxwidths + 1]  ;
boolean ptexp  ;
eightbits dd  ;
eightbits ddstack[stacksize + 1]  ;
integer inwidth[256]  ;
integer tfmchecksum  ;
...
#define noptions ( 10 ) 
  ...
  do {
	...
	else if ( optarg [m ]== 46 ) 
	{
	  ...
	  if ( k >= 10 ) 
	  {
	    fprintf ( stderr , "%s%s\n",  "updvitype" ,             ": More than ten count registers specified." ) ;
	    uexit ( 1 ) ;
	  } 
	  m = m + 1 ;
	} 
	  ...
	  if ( endnum == optarg + m ) 
	  {
	    fprintf ( stderr , "%s%s\n",  "updvitype" ,             ": -page-start values must be numeric or *." ) ;
	    uexit ( 1 ) ;
	  } 
	  startthere [k ]= true ;
	  m = m + endnum - ( optarg + m ) ;
  ...
  if ( ( optind + 1 != argc ) ) 
  {
    fprintf ( stderr , "%s%s\n",  "updvitype" , ": Need exactly one file argument." ) ;
    usage ( "updvitype" ) ;
  } 
...
{
  integer i  ;
  kpsesetprogramname ( argv [0 ], "updvitype" ) ;
  initkanji () ;
  parsearguments () ;
  Fputs ( stdout ,  "This is upDVItype, Version 3.6-p0.4-u1.21" ) ;
  Fputs ( stdout ,  " (" ) ;
  ...
  {register integer for_end; i = 32 ;for_end = 126 ; if ( i <= for_end) do 
    xord [xchr [i ]]= i ;
  while ( i++ < for_end ) ;} 
  {register integer for_end; i = 0 ;for_end = 346 ; if ( i <= for_end) do 
    jfmchartypehashtable [i ]= 0 ;
  while ( i++ < for_end ) ;} 
  jfmchartype [0 ]= 0 ;
  jfmchartypecount = 1 ;
  nf = 0 ;
...
integer 
zgetjfmchartype ( integer fntn , integer jfmc ) 
{
  register integer Result; integer p  ;
  integer ct  ;
  p = jfmchartypehashtable [( jfmc + fntn ) % 347 ];
  ct = 0 ;
  while ( p != 0 ) if ( ( jfmcharcode [p ]== jfmc ) && ( jfmcharfont [p ]
  == fntn ) ) 
  {
    ct = jfmchartype [p ];
    p = 0 ;
  } 
  else p = jfmchartypehashlink [p ];
  Result = ct ;
  return Result ;
} 
void 
...
zouttext ( ASCIIcode c ) 
{
  ...
  if ( c >= 177 ) 
  textbuf [textptr ]= 63 ;
  else textbuf [textptr ]= c ;
} 
void 
zoutkanji ( integer c ) 
{
  if ( textptr >= linelength - 5 ) 
  flushtext () ;
  c = toBUFF ( fromDVI ( c ) ) ;
  if ( BYTE1 ( c ) != 0 ) 
  {
    textptr = textptr + 1 ;
    textbuf [textptr ]= BYTE1 ( c ) ;
  } 
  if ( BYTE2 ( c ) != 0 ) 
  {
    textptr = textptr + 1 ;
    textbuf [textptr ]= BYTE2 ( c ) ;
  } 
  if ( BYTE3 ( c ) != 0 ) 
  {
    textptr = textptr + 1 ;
    textbuf [textptr ]= BYTE3 ( c ) ;
  } 
  textptr = textptr + 1 ;
  textbuf [textptr ]= BYTE4 ( c ) ;
} 
#ifdef HEX_CHAR_CODE
void 
zprinthexnumber ( integer c ) 
{
  integer n  ;
  integer b[9]  ;
  n = 1 ;
...
zspecialcases ( eightbits o , integer p , integer a ) 
{
  ...
  lab44: if ( dd == 0 ) 
  {
	...
	if ( ! showing ) 
	{
	  flushtext () ;
	  showing = true ;
	  fprintf ( stdout , "%ld%s%s%ld%s%ld",  (long)a , ": " ,           "arithmetic overflow! parameter changed from " , (long)p , " to " ,           (long)2147483647L - h ) ;
	  if ( showopcodes && ( o >= 128 ) ) 
	  fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	} 
	else
	fprintf ( stdout , "%c%s%ld%s%ld",  ' ' ,         "arithmetic overflow! parameter changed from " , (long)p , " to " ,         (long)2147483647L - h ) ;
	p = 2147483647L - h ;
      } 
    } 
    if ( ( h < 0 ) && ( p < 0 ) ) {
	
      if ( - (integer) h > p + 2147483647L ) 
      {
	if ( ! showing ) 
	{
	  flushtext () ;
	  showing = true ;
	  fprintf ( stdout , "%ld%s%s%ld%s%ld",  (long)a , ": " ,           "arithmetic overflow! parameter changed from " , (long)p , " to " , (long)(           - (integer) h ) - 2147483647L ) ;
	  if ( showopcodes && ( o >= 128 ) ) 
	  fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	} 
	else
	fprintf ( stdout , "%c%s%ld%s%ld",  ' ' ,         "arithmetic overflow! parameter changed from " , (long)p , " to " , (long)(         - (integer) h ) - 2147483647L ) ;
	p = ( - (integer) h ) - 2147483647L ;
      } 
    } 
    vvv = round ( conv * ( h + p ) ) ;
    if ( abs ( vvv - hh ) > 2 ) {
	...
	if ( p >= 0 ) 
	putc ( '+' ,  stdout );
	fprintf ( stdout , "%ld%c%ld%s%ld",  (long)p , '=' , (long)h + p , ", hh:=" , (long)hh ) ;
      } 
    } 
    h = h + p ;
    if ( abs ( h ) > maxhsofar ) 
...
dopage ( void ) 
{
  ...
  while ( true ) {
    ...
    case 136 : 
      {
	if ( fntjfmp [curfont ]== true ) 
	{
	  curjfmchartype = getjfmchartype ( curfont , p ) ;
	  outkanji ( p ) ;
	  if ( o < 133 ) 
	  {
	    if ( outmode > 1 ) 
	...
	;
	  ...
	  putc ( ')' ,  stdout );
#endif /* HEX_CHAR_CODE */
	  fprintf ( stdout , "%s%ld",  " type=" , (long)curjfmchartype ) ;
	  p = curjfmchartype ;
	} 
	else {
	...
	if ( outmode > 0 ) 
	{
	  flushtext () ;
	  showing = true ;
	  fprintf ( stdout , "%ld%s%s%ld",  (long)a , ": " , "dir " , (long)p ) ;
	  if ( showopcodes && ( o >= 128 ) ) 
	  fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	} 
	dd = p ;
    ...
    else vv = vv + pixelwidth [widthbase [curfont ]+ p ];
    goto lab43 ;
    lab42: q = signedquad () ;
    if ( showing ) 
    {
	...
	if ( v > 2147483647L - q ) 
	{
	  if ( ! showing ) 
	  {
	    flushtext () ;
	    showing = true ;
	    fprintf ( stdout , "%ld%s%s%ld%s%ld",  (long)a , ": " ,             "arithmetic overflow! parameter changed from " , (long)q , " to " ,             (long)2147483647L - q ) ;
	    if ( showopcodes && ( o >= 128 ) ) 
	    fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	  } 
	  else
	  fprintf ( stdout , "%c%s%ld%s%ld",  ' ' ,           "arithmetic overflow! parameter changed from " , (long)q , " to " ,           (long)2147483647L - q ) ;
	  q = 2147483647L - v ;
	} 
      } 
      if ( ( v < 0 ) && ( q < 0 ) ) {
	  
	if ( - (integer) v > q + 2147483647L ) 
	{
	  if ( ! showing ) 
	  {
	    flushtext () ;
	    showing = true ;
	    fprintf ( stdout , "%ld%s%s%ld%s%ld",  (long)a , ": " ,             "arithmetic overflow! parameter changed from " , (long)q , " to " , (long)(             - (integer) v ) - 2147483647L ) ;
	    if ( showopcodes && ( o >= 128 ) ) 
	    fprintf ( stdout , "%s%ld%c",  " {" , (long)o , '}' ) ;
	  } 
	  else
	  fprintf ( stdout , "%c%s%ld%s%ld",  ' ' ,           "arithmetic overflow! parameter changed from " , (long)q , " to " , (long)(           - (integer) v ) - 2147483647L ) ;
	  q = ( - (integer) v ) - 2147483647L ;
	} 
      } 
      hhh = round ( conv * ( v + q ) ) ;
      if ( abs ( hhh - vv ) > 2 ) {
	...
	if ( outmode > 2 ) 
	{
	  ...
	  if ( q >= 0 ) 
	  putc ( '+' ,  stdout );
	  fprintf ( stdout , "%ld%c%ld%s%ld",  (long)q , '=' , (long)v + q , ", vv:=" , (long)vv ) ;
	} 
      } 
      v = v + q ;
      if ( abs ( v ) > maxvsofar ) 
	...
	{ putc ( ' ' ,  stdout );  putc ( '\n',  stdout ); }
	{
	  fprintf ( stdout , "%s%ld%s%ld%s%ld%s%ld%s%ld%s%ld%s%ld",  "level " , (long)ss , ":(h=" , (long)h , ",v=" , (long)v , ",w=" , (long)w           , ",x=" , (long)x , ",y=" , (long)y , ",z=" , (long)z ) ;
	  if ( ptexp ) 
	  fprintf ( stdout , "%s%ld",  ",d=" , (long)dd ) ;
	  fprintf ( stdout , "%s%ld%s%ld%c",  ",hh=" , (long)hh , ",vv=" , (long)vv , ')' ) ;
	} 
      } 
    } 
    goto lab30 ;
...
readpostamble ( void ) 
{
  ...
  m = getbyte () ;
  if ( ( m != 2 ) && ( m != 3 ) ) 
  fprintf ( stdout , "%s%ld%s%ld%s%ld%c\n",  "identification in byte " , (long)curloc - 1 , " should be " ,   (long)2 , " or " , (long)3 , '!' ) ;
  k = curloc ;
  m = 223 ;
