...
{:14}{334:}numbltinfns=38;{:334}type{22:}ASCIIcode=0..255;
...
bwhile:hashloc;bwidth:hashloc;bwrite:hashloc;biskanjistr:hashloc;
bdefault:hashloc;
...
label 15;var i:cinttype;begin last:=0;
if(eof(f))then inputln:=false else begin last:=inputline2(f,ustringcast(
buffer),last,bufsize,addressof(i));
while(not eof(f))and(last>0)and(i<>10)and(i<>13)do begin bufferoverflow;
last:=inputline2(f,ustringcast(buffer),last,bufsize,addressof(i));end;
...
precedingwhite:=false;end;
161:if(isinternalEUC)then begin if((fromBUFF(exbuf,exbuflength,exbufptr)
=41380)or(fromBUFF(exbuf,exbuflength,exbufptr)=41378))then begin
precedingwhite:=false;
andfound:=true end else if(fromBUFF(exbuf,exbuflength,exbufptr)=41377)
then begin exbuf[exbufptr]:=32;exbuf[exbufptr+1]:=32;
precedingwhite:=true;end;exbufptr:=exbufptr+2;
end else begin exbufptr:=exbufptr+multibytelen(exbuf[exbufptr]);
precedingwhite:=false;end;
227,239:if(isinternalUPTEX)then begin if((fromBUFF(exbuf,exbuflength,
exbufptr)=65292)or(fromBUFF(exbuf,exbuflength,exbufptr)=12289))then
begin precedingwhite:=false;
andfound:=true end else if(fromBUFF(exbuf,exbuflength,exbufptr)=12288)
then begin exbuf[exbufptr]:=32;exbuf[exbufptr+1]:=32;
exbuf[exbufptr+2]:=32;precedingwhite:=true;end;exbufptr:=exbufptr+3;
end else begin exbufptr:=exbufptr+multibytelen(exbuf[exbufptr]);
...
if multibytelen(svbuffer[namebfptr])>1 then begin if(exbufptr=bufsize)
then bufferoverflow;begin exbuf[exbufptr]:=svbuffer[namebfptr+1];
exbufptr:=exbufptr+1;end;end;
if multibytelen(svbuffer[namebfptr])>2 then begin if(exbufptr=bufsize)
then bufferoverflow;begin exbuf[exbufptr]:=svbuffer[namebfptr+2];
exbufptr:=exbufptr+1;end;end;
if multibytelen(svbuffer[namebfptr])>3 then begin if(exbufptr=bufsize)
then bufferoverflow;begin exbuf[exbufptr]:=svbuffer[namebfptr+3];
exbufptr:=exbufptr+1;end;end;
namebfptr:=namebfptr+multibytelen(svbuffer[namebfptr])-1;goto 15;
...
others:begin if(isinternalEUC)then begin case(fromBUFF(strpool,spptr+1,
spptr-1))of 41379,41381,41385,41386:if(strpool[spptr-2]<>143)then begin
if(litstack[litstkptr]>=cmdstrptr)then begin strptr:=strptr+1;
poolptr:=strstart[strptr];end;litstkptr:=litstkptr+1;
end else{363:}begin if(poplit1<cmdstrptr)then begin begin while(poolptr+
...
pushlitstk(makestring,1);end{:363};
...
begin while(poolptr+1>poolsize)do pooloverflow;end;end;
begin strpool[poolptr]:=46;poolptr:=poolptr+1;end;
pushlitstk(makestring,1);end{:363};end;end;
if(isinternalUPTEX)then begin case(fromBUFF(strpool,spptr+1,spptr-2))of
12290,65294,65311,65281,8263,8252,8264,8265:begin if(litstack[litstkptr]
>=cmdstrptr)then begin strptr:=strptr+1;poolptr:=strstart[strptr];end;
litstkptr:=litstkptr+1;end;
others:{363:}begin if(poplit1<cmdstrptr)then begin begin while(poolptr+(
strstart[poplit1+1]-strstart[poplit1])+1>poolsize)do pooloverflow;end;
...
begin while(poolptr+1>poolsize)do pooloverflow;end;end;
begin strpool[poolptr]:=46;poolptr:=poolptr+1;end;
pushlitstk(makestring,1);end{:363};end;end;end;end;end{:362};end;
...
if(poptyp1<>1)then begin printwrongstklit(poplit1,poptyp1,1);
pushlitstk(0,0);
end else if((strstart[poplit1+1]-strstart[poplit1])<>1)then if(strpool[
strstart[poplit1]]>127)then pushlitstk(strpool[strstart[poplit1]],0)else
begin begin write(logfile,'"');write(standardoutput,'"');end;
...
if(exbufptr<exbuflength)then begin if(isinternalEUC)then if((fromBUFF(
exbuf,exbuflength,exbufptr-2)=41380)or(fromBUFF(exbuf,exbuflength,
exbufptr-2)=41378))then exbufptr:=exbufptr-2 else exbufptr:=exbufptr-4;
if(isinternalUPTEX)then if((fromBUFF(exbuf,exbuflength,exbufptr-3)=65292
)or(fromBUFF(exbuf,exbuflength,exbufptr-3)=12289))then exbufptr:=
exbufptr-3 else exbufptr:=exbufptr-4;end;
...
var tps,tpe:poolpointer;begin poplitstk(poplit1,poptyp1);
...
while(tps<spptr)do begin tps:=tps+multibytelen(strpool[tps]);end;
tpe:=tps;while(tpe<spend)do begin tpe:=tpe+multibytelen(strpool[tpe]);
end;if tps<>spptr then begin if tps>strstart[poplit3]then spptr:=spptr-1
else spptr:=spptr+1;end;
if tpe<>spend then begin if tpe<strstart[poplit3+1]then spend:=spend+1
else spend:=spend-1;end;
begin while(poolptr+spend-spptr>poolsize)do pooloverflow;end;
while(spptr<spend)do begin begin strpool[poolptr]:=strpool[spptr];
poolptr:=poolptr+1;end;
if multibytelen(strpool[spptr])>1 then begin strpool[poolptr]:=strpool[
spptr+1];poolptr:=poolptr+1;end;
if multibytelen(strpool[spptr])>2 then begin strpool[poolptr]:=strpool[
spptr+2];poolptr:=poolptr+1;end;
if multibytelen(strpool[spptr])>3 then begin strpool[poolptr]:=strpool[
spptr+3];poolptr:=poolptr+1;end;
if multibytelen(strpool[spptr])>0 then spptr:=spptr+multibytelen(strpool
[spptr])else spptr:=spptr+1;end;pushlitstk(makestring,1);end{:439};end;
10:end;{:438}{440:}procedure xswap;begin poplitstk(poplit1,poptyp1);
poplitstk(poplit2,poptyp2);
...
end else if(strpool[spxptr1-1]>127)then begin numtextchars:=numtextchars
+multibytelen(strpool[spxptr1-1]);
spxptr1:=spxptr1-1+multibytelen(strpool[spxptr1-1]);
end else numtextchars:=numtextchars+1;end;spend:=spxptr1;end{:446};
...
poplit1);end;{:455}{483:}procedure xiskanjistr;label 10;
begin poplitstk(poplit1,poptyp1);
if poptyp1<>1 then begin printwrongstklit(poplit1,poptyp1,1);
pushlitstk(0,0);end else begin spptr:=strstart[poplit1];
spend:=strstart[poplit1+1];
while spptr<spend do begin if strpool[spptr]>127 then begin pushlitstk(1
,0);goto 10;end else begin spptr:=spptr+1;end;end;pushlitstk(0,0);end;
10:end;{:483}{326:}procedure executefn(exfnloc:hashloc);{344:}label 51;
var rpoplt1,rpoplt2:integer;rpoptp1,rpoptp2:stktype;
{:344}wizptr:wizfnloc;
...
51:end{:450};35:xwidth;36:xwrite;37:xiskanjistr;
...
var i:0..255;begin if(not setencstring(nil,'EUC'))then uexit(1);
{103:}initkanji;parsearguments;
{:103}{484:}if(isinternalUPTEX)then begin for i:=128 to 191 do lexclass[
i]:=2;for i:=192 to 193 do lexclass[i]:=0;
for i:=194 to 244 do lexclass[i]:=2;for i:=245 to 255 do lexclass[i]:=0;
for i:=192 to 193 do idclass[i]:=0;for i:=245 to 255 do idclass[i]:=0;
for i:=128 to 191 do charwidth[i]:=257;
for i:=194 to 223 do charwidth[i]:=771;
for i:=224 to 239 do charwidth[i]:=514;
for i:=240 to 244 do charwidth[i]:=257;
end else begin for i:=128 to 160 do lexclass[i]:=0;
for i:=161 to 254 do lexclass[i]:=2;lexclass[255]:=0;
for i:=128 to 160 do idclass[i]:=0;idclass[255]:=0;lexclass[27]:=2;
lexclass[143]:=2;for i:=161 to 254 do charwidth[i]:=514;
charwidth[143]:=0;end;
{:484}nameoffile:=xmallocarray(ASCIIcode,strlen(cmdline(optind))+5);
...
{:155}{:218}{:12}{481:}procedure setupparams;var bounddefault:integer;
boundname:constcstring;begin kpsesetprogramname(argv[0],'pbibtex');
...
end;{:481}{482:}procedure computehashprime;var hashwant:integer;
...
{:482}{13:}procedure initialize;var{23:}i:integer;{:23}{67:}k:hashloc;
...
xchr[9]:=chr(9);for i:=128 to 255 do xchr[i]:=chr(i);xchr[27]:=chr(27);
{:27}{28:}for i:=0 to 255 do xord[i]:=127;for i:=1 to 126 do xord[i]:=i;
for i:=128 to 255 do xord[i]:=i;
{:28}{32:}for i:=0 to 255 do lexclass[i]:=5;
for i:=0 to 31 do lexclass[i]:=0;lexclass[127]:=0;lexclass[9]:=1;
lexclass[13]:=1;lexclass[32]:=1;lexclass[126]:=4;lexclass[45]:=4;
...
for i:=0 to 31 do idclass[i]:=0;idclass[27]:=1;idclass[32]:=0;
idclass[9]:=0;idclass[34]:=0;idclass[35]:=0;idclass[37]:=0;
idclass[39]:=0;idclass[40]:=0;idclass[41]:=0;idclass[44]:=0;
idclass[61]:=0;idclass[123]:=0;idclass[125]:=0;
...
const noptions=7;var longoptions:array[0..noptions]of getoptstruct;
...
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:476}{477:}longoptions[currentoption].name:='sjis-terminal';
...
{:477}{478:}longoptions[currentoption].name:='kanji';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:478}{479:}longoptions[currentoption].name:='kanji-internal';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:479}{480:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:480};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('upbibtex');
...
printversionandexit('This is upBibTeX, Version 0.99d-j0.33-u1.21',
'Oren Patashnik',nil,nil);
end else if(strcmp(longoptions[optionindex].name,'kanji')=0)then begin
if(not setencstring(optarg,nil))then writeln('Bad kanji encoding "',
stringcast(optarg),'".');
end else if(strcmp(longoptions[optionindex].name,'kanji-internal')=0)
then begin if(not(setencstring(nil,optarg)and(isinternalEUC or
isinternalUPTEX)))then writeln('Bad internal kanji encoding "',
stringcast(optarg),'".');end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'upbibtex',
': Need exactly one file argument.');usage('upbibtex');end;end;
...
'This is upBibTeX, Version 0.99d-j0.33-u1.21');
write(standardoutput,'This is upBibTeX, Version 0.99d-j0.33-u1.21');end;
begin write(logfile,' (',conststringcast(getencstring),')');
write(standardoutput,' (',conststringcast(getencstring),')');end;
begin writeln(logfile,versionstring);
writeln(standardoutput,versionstring);end;
end else begin begin write(logfile,
'This is upBibTeX, Version 0.99d-j0.33-u1.21');end;
