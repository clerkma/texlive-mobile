...
zprintcs ( integer p ) 
{
  ...
  else if ( ( ( p >= 24527 ) && ( p <= 28083 ) ) || ( p > eqtbtop ) ) 
  printesc ( 581 ) ;
...
zshownodelist ( integer p ) 
{
  ...
  while ( p > memmin ) {
	...
	break ;
      case 1 : 
	{
	  printwritewhatsit ( 677 , p ) ;
	  printmark ( mem [p + 1 ].hh .v.RH ) ;
	} 
	break ;
      case 2 : 
	printwritewhatsit ( 1723 , p ) ;
	break ;
      case 3 : 
	{
	  printesc ( 1724 ) ;
	...
	if ( memtop - 2 != pagetail ) 
	{
	  ...
	  if ( pagecontents > 0 ) 
	  {
	    ...
	    while ( r != memtop ) {
	      ...
	      printint ( t ) ;
	      print ( 1390 ) ;
	      if ( eqtb [27282 + t ].cint == 1000 ) 
	      t = mem [r + 3 ].cint ;
	      else t = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [27282 + 
	      t ].cint ;
	      printscaled ( t ) ;
	      if ( mem [r ].hh.b0 == 1 ) 
	      {
...
zprintcmdchr ( quarterword cmd , halfword chrcode ) 
{
  ...
  case 73 : 
    ...
    printparam ( chrcode - 27177 ) ;
      ...
      printesc ( 536 ) ;
      printint ( chrcode - 27282 ) ;
    } 
    break ;
  case 74 : 
    if ( chrcode < 27828 ) 
    printlengthparam ( chrcode - 27794 ) ;
    else {
	
      printesc ( 573 ) ;
      printint ( chrcode - 27828 ) ;
    } 
    break ;
  case 45 : 
    printesc ( 583 ) ;
...
zshoweqtb ( halfword n ) 
{
  ...
  if ( n < 1 ) 
  printchar ( 63 ) ;
  else if ( ( n < 24528 ) || ( ( n > 28083 ) && ( n <= eqtbtop ) ) ) 
  {
    sprintcs ( n ) ;
    printchar ( 61 ) ;
...
zscansomethinginternal ( smallnumber level , boolean negative ) 
{
  ...
  case 89 : 
	...
	{case 0 : 
	  curval = eqtb [27282 + curval ].cint ;
	  break ;
	case 1 : 
	  curval = eqtb [27828 + curval ].cint ;
	  break ;
	case 2 : 
...
convtoks ( void ) 
{
  ...
  case 18 : 
    {
      bool = scankeyword ( 890 ) ;
      if ( scankeyword ( 891 ) ) 
      curval = 2 ;
      else if ( scankeyword ( 890 ) ) 
      curval = 1 ;
      else curval = 0 ;
      savescannerstatus = scannerstatus ;
      savewarningindex = warningindex ;
...
openlogfile ( void ) 
{
  ...
  logopened = true ;
  {
    ...
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.14159265" , "-2.6" ,     "-1.40.17" ) ;
    else
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.14159265" , "-2.6" ,     "-1.40.17" ) ;
...
zoutwhat ( halfword p ) 
{
  ...
  case 2 : 
	...
	else if ( j < 16 ) 
	{
	  ...
	  aopenout ( writefile [j ]) ) promptfilename ( 1912 , 945 ) ;
	  writeopen [j ]= true ;
	  if ( logopened ) 
	  {
	    oldsetting = selector ;
...
zdvishipout ( halfword p ) 
{
  ...
  synctexsheet ( eqtb [27194 ].cint ) ;
  {
    ...
    printchar ( 91 ) ;
    j = 9 ;
    while ( ( eqtb [27282 + j ].cint == 0 ) && ( j > 0 ) ) decr ( j ) ;
    {register integer for_end; k = 0 ;for_end = j ; if ( k <= for_end) do 
      {
	printint ( eqtb [27282 + k ].cint ) ;
	if ( k < j ) 
	printchar ( 46 ) ;
      } 
    while ( k++ < for_end ) ;} 
    ...
    27813 ].cint > 1073741823L ) || ( mem [p + 1 ].cint + eqtb [27812 ]
    .cint > 1073741823L ) ) 
    {
      {
	if ( interaction == 3 ) 
    ...
    if ( mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [27813 ].cint > 
    maxv ) 
    maxv = mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [27813 ].cint ;
    if ( mem [p + 1 ].cint + eqtb [27812 ].cint > maxh ) 
    maxh = mem [p + 1 ].cint + eqtb [27812 ].cint ;
    dvih = 0 ;
    dviv = 0 ;
    curh = eqtb [27812 ].cint ;
    dvif = 0 ;
    curhoffset = eqtb [27812 ].cint ;
    curvoffset = eqtb [27813 ].cint ;
    if ( eqtb [27817 ].cint != 0 ) 
    curpagewidth = eqtb [27817 ].cint ;
    else curpagewidth = mem [p + 1 ].cint + 2 * curhoffset + 2 * 4736286L ;
    if ( eqtb [27818 ].cint != 0 ) 
    curpageheight = eqtb [27818 ].cint ;
    ...
    if ( outputfilename == 0 ) 
    {
      ...
      openlogfile () ;
      packjobname ( 949 ) ;
      while ( ! bopenout ( dvifile ) ) promptfilename ( 950 , 949 ) ;
    ...
    {register integer for_end; k = 0 ;for_end = 9 ; if ( k <= for_end) do 
      dvifour ( eqtb [27282 + k ].cint ) ;
    while ( k++ < for_end ) ;} 
    dvifour ( lastbop ) ;
    lastbop = pageloc ;
    curv = mem [p + 3 ].cint + eqtb [27813 ].cint ;
    tempptr = p ;
    if ( mem [p ].hh.b0 == 1 ) 
    vlistout () ;
    else hlistout () ;
...
newdummyfont ( void ) 
{
  ...
  Result = readfontinfo ( 513 , 1003 , 345 , -1000 ) ;
  return Result ;
} 
void 
...
checkpdfminorversion ( void ) 
{
  ...
  if ( ! pdfminorversionwritten ) 
  {
    ...
    pdfosenable = true ;
      ...
      {
	pdfwarning ( 1009 , 1010 , true , true ) ;
...
ensurepdfopen ( void ) 
{
  ...
  while ( ! bopenout ( pdffile ) ) promptfilename ( 950 , 1014 ) ;
...
pdfreaddummyfont ( void ) 
{
  ...
  if ( pdfdummyfont == 0 ) 
  {
    pdfdummyfont = readfontinfo ( 513 , 1042 , 345 , onebp ) ;
...
zliteral ( strnumber s , integer literalmode , boolean warn ) 
{
  ...
  if ( literalmode == 3 ) 
  {
    ...
    if ( strinstr ( s , 1059 , ( strstart [1055 ]- strstart [1054 ]) ) ) 
    {
      j = j + ( strstart [1060 ]- strstart [1059 ]) ;
      literalmode = 2 ;
    } 
    else if ( strinstr ( s , 1060 , ( strstart [1055 ]- strstart [1054 ]) 
    ) ) 
    {
...
zadjustinterwordglue ( halfword p , halfword g ) 
{
  ...
  {
    pdfwarning ( 1074 , 1075 , false , false ) ;
    return ;
  } 
  c = 256 ;
  if ( ( p >= himemmin ) ) 
...
ztfmlookup ( strnumber s , scaled fs ) 
{
  ...
  if ( fs != 0 ) 
  {
    ...
    do 
      if ( ( fontarea [k ]!= 1073 ) && streqstr ( fontname [k ], s ) && ( 
      fontsize [k ]== fs ) ) 
      {
	flushstr ( s ) ;
	Result = k ;
...
zdovf ( internalfontnumber f ) 
{
  ...
  return ;
  stacklevel = 0 ;
  packfilename ( fontname [f ], 345 , 1095 ) ;
  if ( ! vfbopenin ( vffile ) ) 
  return ;
  if ( vfbyte () != 247 ) 
...
zwarndestdup ( integer id , smallnumber byname , strnumber s1 , strnumber s2 ) 
{
  ...
  return ;
  pdfwarning ( s1 , 1834 , false , false ) ;
  if ( byname > 0 ) 
  {
    print ( 1820 ) ;
    printmark ( id ) ;
...
zdolink ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  pushlinklevel ( p ) ;
  setrectdimens ( p , parentbox , x , y , mem [p + 1 ].cint , mem [p + 2 ]
  .cint , mem [p + 3 ].cint , eqtb [27819 ].cint ) ;
  objtab [mem [p + 6 ].cint ].int4 = p ;
  {
    pdfappendlistarg = mem [p + 6 ].cint ;
...
endlink ( void ) 
{
  ...
  -1073741824L ) ) 
  {
    ...
    if ( isshippingpage && matrixused () ) 
    {
      matrixrecalculate ( curh + eqtb [27819 ].cint ) ;
      mem [p + 1 ].cint = getllx () - eqtb [27819 ].cint ;
      mem [p + 2 ].cint = curpageheight - getury () - eqtb [27819 ].cint ;
      mem [p + 3 ].cint = geturx () + eqtb [27819 ].cint ;
      mem [p + 4 ].cint = curpageheight - getlly () + eqtb [27819 ].cint ;
    } 
    else mem [p + 3 ].cint = curh + eqtb [27819 ].cint ;
  } 
  poplinklevel () ;
} 
void 
...
zdothread ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  setrectdimens ( p , parentbox , x , y , mem [p + 1 ].cint , mem [p + 2 ]
  .cint , mem [p + 3 ].cint , eqtb [27821 ].cint ) ;
  appendbead ( p ) ;
  lastthread = p ;
} 
void 
...
endthread ( void ) 
{
  ...
  mem [lastthread + 4 ].cint = curv + eqtb [27821 ].cint ;
  if ( pdflastthreadnamedid ) 
...
zdodest ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  {
    warndestdup ( mem [p + 5 ].hh .v.RH , mem [p + 5 ].hh.b1 , 1847 , 1848 
    ) ;
    return ;
  } 
  objtab [k ].int4 = p ;
  ...
  {case 0 : 
    if ( matrixused () ) 
    setrectdimens ( p , parentbox , x , y , mem [p + 1 ].cint , mem [p + 2 
    ].cint , mem [p + 3 ].cint , eqtb [27820 ].cint ) ;
    else {
...
zpdfshipout ( halfword p , boolean shippingpage ) 
{
  ...
  synctexsheet ( eqtb [27194 ].cint ) ;
  {
    ...
    if ( ! shippingpage ) 
    {
      ...
      curhoffset = eqtb [27815 ].cint + eqtb [27812 ].cint ;
      curvoffset = eqtb [27816 ].cint + eqtb [27813 ].cint ;
      if ( eqtb [27817 ].cint != 0 ) 
      curpagewidth = eqtb [27817 ].cint ;
	...
	pdfprint ( 1137 ) ;
	{
      ...
      pdfindirectln ( 1138 , pdflastresources ) ;
    } 
    pdfbeginstream () ;
    if ( shippingpage ) 
    {
	...
	pdfprint ( 1152 ) ;
	{
      ...
      pdfindirectln ( 1153 , pdflaststream ) ;
      pdfindirectln ( 1138 , pdflastresources ) ;
      pdfprint ( 1154 ) ;
	...
	pdflastpages = objptr ;
      ...
      pdfindirectln ( 1156 , pdflastpages ) ;
...
pdfprintinfo ( void ) 
{
  ...
  if ( pdfinfotoks != -268435455L ) 
  {
    ...
    creatorgiven = substrofstr ( 1208 , s ) ;
    producergiven = substrofstr ( 1209 , s ) ;
    creationdategiven = substrofstr ( 1210 , s ) ;
    moddategiven = substrofstr ( 1211 , s ) ;
    trappedgiven = substrofstr ( 1212 , s ) ;
  } 
  if ( ! producergiven ) 
  {
    pdfprint ( 1217 ) ;
  ...
  if ( eqtb [27270 ].cint % 2 == 0 ) 
  {
    pdfstrentryln ( 1216 , pdftexbanner ) ;
  } 
  pdfenddict () ;
} 
void 
...
zhpack ( halfword p , scaled w , smallnumber m ) 
{
  ...
  else if ( x > 0 ) 
  {
    ...
    ].hh .v.RH != -268435455L ) ) 
    {
      ...
      mem [r + 6 ].gr = 1.0 ;
      if ( ( - (integer) x - totalshrink [0 ]> eqtb [27802 ].cint ) || ( 
      eqtb [27203 ].cint < 100 ) ) 
      {
	if ( ( eqtb [27810 ].cint > 0 ) && ( - (integer) x - totalshrink [0 
	]> eqtb [27802 ].cint ) ) 
	{
	  while ( mem [q ].hh .v.RH != -268435455L ) q = mem [q ].hh .v.RH 
	  ;
	  mem [q ].hh .v.RH = newrule () ;
	  mem [mem [q ].hh .v.RH + 1 ].cint = eqtb [27810 ].cint ;
	} 
	println () ;
	printnl ( 1251 ) ;
...
zvpackage ( halfword p , scaled h , smallnumber m , scaled l ) 
{
  ...
  else if ( x > 0 ) 
  {
    ...
    ].hh .v.RH != -268435455L ) ) 
    {
      lastbadness = 1000000L ;
      mem [r + 6 ].gr = 1.0 ;
      if ( ( - (integer) x - totalshrink [0 ]> eqtb [27803 ].cint ) || ( 
      eqtb [27204 ].cint < 100 ) ) 
      {
	println () ;
	printnl ( 1256 ) ;
...
zappendtovlist ( halfword b ) 
{
  ...
  scaled d  ;
  halfword p  ;
  if ( curlist .auxfield .cint > eqtb [27826 ].cint ) 
  {
    d = mem [eqtb [24529 ].hh .v.RH + 1 ].cint - curlist .auxfield .cint - 
    mem [b + 3 ].cint ;
    if ( d < eqtb [27796 ].cint ) 
    p = newparamglue ( 0 ) ;
    else {
...
zmakescripts ( halfword q , scaled delta ) 
{
  ...
  if ( mem [q + 2 ].hh .v.RH == 0 ) 
  {
    ...
    mem [x + 1 ].cint = mem [x + 1 ].cint + eqtb [27806 ].cint ;
    ...
    mem [x + 4 ].cint = - (integer) shiftup ;
      ...
      mem [y + 1 ].cint = mem [y + 1 ].cint + eqtb [27806 ].cint ;
...
zmakeleftright ( halfword q , smallnumber style , scaled maxd , scaled maxh ) 
{
  ...
  delta2 = delta1 + delta1 - eqtb [27804 ].cint ;
  if ( delta < delta2 ) 
  delta = delta2 ;
	...
	( curactivewidth [5 ]!= 0 ) ) 
	{
	  if ( dolastlinefit ) 
	  {
	    if ( curp == -268435455L ) 
	    {
	      ...
	      1073741823L ) ;
	      if ( eqtb [27277 ].cint < 1000 ) 
	      g = fract ( g , eqtb [27277 ].cint , 1000 , 1073741823L ) ;
	      if ( aritherror ) {
...
zpostlinebreak ( boolean d ) 
{
  ...
  do {
    ...
    if ( eqtb [27824 ].cint != eqtb [27826 ].cint ) 
    mem [justbox + 3 ].cint = eqtb [27824 ].cint ;
    if ( eqtb [27825 ].cint != eqtb [27826 ].cint ) 
    mem [justbox + 2 ].cint = eqtb [27825 ].cint ;
    if ( ( eqtb [27822 ].cint != eqtb [27826 ].cint ) && ( curline == 
    curlist .pgfield + 1 ) ) 
    mem [justbox + 3 ].cint = eqtb [27822 ].cint ;
    if ( ( eqtb [27823 ].cint != eqtb [27826 ].cint ) && ( curline + 1 == 
    bestline ) ) 
...
zvsplit ( halfword n , scaled h ) 
{
  ...
  q = vertbreak ( mem [v + 5 ].hh .v.RH , h , eqtb [27800 ].cint ) ;
  p = mem [v + 5 ].hh .v.RH ;
  if ( p == q ) 
  ...
  lab30: ;
  q = prunepagetop ( q , eqtb [27278 ].cint > 0 ) ;
  p = mem [v + 5 ].hh .v.RH ;
  freenode ( v , 9 ) ;
  if ( q != -268435455L ) 
  ...
  Result = vpackage ( p , h , 0 , eqtb [27800 ].cint ) ;
  return Result ;
} 
void 
printtotals ( void ) 
...
zfreezepagespecs ( smallnumber s ) 
{
  ...
  pagecontents = s ;
  pagesofar [0 ]= eqtb [27798 ].cint ;
  pagemaxdepth = eqtb [27799 ].cint ;
  pagesofar [7 ]= 0 ;
  pagesofar [1 ]= 0 ;
  pagesofar [2 ]= 0 ;
  pagesofar [3 ]= 0 ;
...
buildpage ( void ) 
{
  ...
  do {
	...
	if ( mem [r ].hh.b1 != n ) 
	{
	  ...
	  if ( eqtb [27282 + n ].cint == 1000 ) 
	  h = mem [r + 3 ].cint ;
	  else h = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [27282 + n ]
	  .cint ;
	...
	insertpenalties = insertpenalties + mem [p + 1 ].cint ;
	  ...
	  if ( eqtb [27282 + n ].cint == 1000 ) 
	  h = mem [p + 3 ].cint ;
	  else h = xovern ( mem [p + 3 ].cint , 1000 ) * eqtb [27282 + n ]
	  .cint ;
	  if ( ( ( h <= 0 ) || ( h <= delta ) ) && ( mem [p + 3 ].cint + mem 
	  [r + 3 ].cint <= eqtb [27828 + n ].cint ) ) 
	  {
	    ...
	    if ( eqtb [27282 + n ].cint <= 0 ) 
	    w = 1073741823L ;
	    else {
		
	      w = pagesofar [0 ]- pagesofar [1 ]- pagesofar [7 ];
	      if ( eqtb [27282 + n ].cint != 1000 ) 
	      w = xovern ( w , eqtb [27282 + n ].cint ) * 1000 ;
	    } 
	    if ( w > eqtb [27828 + n ].cint - mem [r + 3 ].cint ) 
	    w = eqtb [27828 + n ].cint - mem [r + 3 ].cint ;
...
itsallover ( void ) 
{
  ...
  if ( privileged () ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [27797 ].cint ;
    {
...
znewgraf ( boolean indented ) 
{
  ...
  if ( indented ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [27794 ].cint ;
...
initmath ( void ) 
{
  ...
  if ( ( curcmd == 3 ) && ( curlist .modefield > 0 ) ) 
  {
    ...
    if ( eqtb [25058 ].hh .v.RH == -268435455L ) {
      ...
      if ( ( eqtb [27811 ].cint != 0 ) && ( ( ( eqtb [27218 ].cint >= 0 ) 
      && ( curlist .pgfield + 2 > eqtb [27218 ].cint ) ) || ( curlist 
      .pgfield + 1 < - (integer) eqtb [27218 ].cint ) ) ) 
      {
	l = eqtb [27797 ].cint - abs ( eqtb [27811 ].cint ) ;
	if ( eqtb [27811 ].cint > 0 ) 
	s = eqtb [27811 ].cint ;
	else s = 0 ;
      } 
    ...
    eqworddefine ( 27807 , w ) ;
    curlist .eTeXauxfield = j ;
    if ( ( eTeXmode == 1 ) ) 
    eqworddefine ( 27276 , x ) ;
    eqworddefine ( 27808 , l ) ;
...
aftermath ( void ) 
{
  ...
  {
    {
      mem [curlist .tailfield ].hh .v.RH = newmath ( eqtb [27795 ].cint , 
      0 ) ;
      curlist .tailfield = mem [curlist .tailfield ].hh .v.RH ;
    } 
    curmlist = p ;
...
zcheckpdfoutput ( strnumber s , boolean iserror ) 
{
  ...
  if ( eqtb [27239 ].cint <= 0 ) 
  {
    if ( iserror ) 
    pdferror ( s , 1768 ) ;
    else pdfwarning ( s , 1769 , true , true ) ;
  } 
} 
void 
scanpdfexttoks ( void ) 
...
zscaleimage ( integer n ) 
{
  ...
  if ( ( xr > 65535L ) || ( yr > 65535L ) ) 
  {
    xr = 0 ;
    yr = 0 ;
    pdfwarning ( 1767 , 1799 , true , true ) ;
  } 
...
scanpdfboxspec ( void ) 
{
  ...
  else if ( scankeyword ( 1802 ) ) 
  Result = pdfboxspeccrop ;
  else if ( scankeyword ( 1803 ) ) 
  Result = pdfboxspecbleed ;
  else if ( scankeyword ( 1804 ) ) 
...
scanimage ( void ) 
{
  ...
  {
    pdfwarning ( 1808 , 1809 , true , true ) ;
  ...
  {
    pdfwarning ( 1808 , 1810 , true , true ) ;
...
scanaction ( void ) 
{
  ...
  else if ( scankeyword ( 1820 ) ) 
  {
    scanpdfexttoks () ;
    mem [p ].hh.b1 = 1 ;
    mem [p ].hh .v.RH = defref ;
  } 
  else if ( scankeyword ( 1188 ) ) 
  {
...
zpdffixthread ( integer thread ) 
{
  pdffixthread_regmem 
  halfword a  ;
  pdfwarning ( 1816 , 1871 , false , false ) ;
  if ( objtab [thread ].int0 < 0 ) 
  {
    print ( 1187 ) ;
...
doextension ( void ) 
{
  ...
  case 38 : 
	...
	j = 3 ;
      ...
      else if ( scankeyword ( 1779 ) ) 
      {
	i = 1 ;
	j = 3 ;
      } 
      else if ( scankeyword ( 1780 ) ) 
      {
	i = 2 ;
	j = 2 ;
      } 
      else if ( scankeyword ( 1781 ) ) 
      {
	i = 3 ;
	j = 2 ;
      } 
  ...
  case 7 : 
	...
	if ( scankeyword ( 1790 ) ) 
	{
	  ...
	  if ( ( k <= 0 ) || ( k > objptr ) || ( objtab [k ].int4 != 0 ) ) 
	  {
	    pdfwarning ( 1788 , 1791 , true , true ) ;
	    pdfretval = -1 ;
	    k = -1 ;
	  } 
	} 
  ...
  case 44 : 
    {
      checkpdfoutput ( 1868 , true ) ;
      newwhatsit ( 44 , 2 ) ;
    } 
    break ;
  case 45 : 
    {
      checkpdfoutput ( 1869 , true ) ;
      newwhatsit ( 45 , 2 ) ;
    } 
    break ;
    default: 
...
maincontrol ( void ) 
{
  ...
  case 137 : 
    if ( curchr > 0 ) 
    {
      if ( eTeXenabled ( ( eqtb [27280 ].cint > 0 ) , curcmd , curchr ) ) 
      {
  ...
  if ( fixedpdfoutput > 0 ) 
  {
	...
	if ( fixedpdfdraftmode == 0 ) 
	{
	  ...
	  while ( objtab [k ].int4 == 0 ) {
	    ...
	    pdfwarning ( 1186 , 1191 , false , false ) ;
	    printint ( objtab [k ].int0 ) ;
	    print ( 1192 ) ;
	    println () ;
	    println () ;
	  ...
	  for_end) do 
		...
		pdfmarkchar ( i , j ) ;
	      ...
	      pdffontattr [i ], pdffontattr [k ]) ) 
	      {
		pdfwarning ( 1193 , 1194 , false , false ) ;
	  ...
	  if ( pdffirstoutline != 0 ) 
	  {
	    ...
	    pdfindirectln ( 1179 , pdffirstoutline ) ;
	    pdfindirectln ( 1180 , pdflastoutline ) ;
	    pdfintentryln ( 1181 , k ) ;
	    pdfenddict () ;
	    k = headtab [4 ];
	    while ( k != 0 ) {
		...
		pdflastoutline = k ;
	      ...
	      pdfbegindict ( k , 1 ) ;
	      pdfindirectln ( 1182 , pdfmem [objtab [k ].int4 ]) ;
	      pdfindirectln ( 65 , pdfmem [objtab [k ].int4 + 6 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 1 ]!= 0 ) 
	      pdfindirectln ( 1156 , pdfmem [objtab [k ].int4 + 1 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 2 ]!= 0 ) 
	      pdfindirectln ( 1183 , pdfmem [objtab [k ].int4 + 2 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 3 ]!= 0 ) 
	      pdfindirectln ( 1184 , pdfmem [objtab [k ].int4 + 3 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 4 ]!= 0 ) 
	      pdfindirectln ( 1179 , pdfmem [objtab [k ].int4 + 4 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 5 ]!= 0 ) 
	      pdfindirectln ( 1180 , pdfmem [objtab [k ].int4 + 5 ]) ;
	      if ( objtab [k ].int0 != 0 ) 
	      pdfintentryln ( 1181 , objtab [k ].int0 ) ;
	  ...
	  if ( threads != 0 ) 
	  pdfindirectln ( 1204 , threads ) ;
	  if ( outlines != 0 ) 
	  pdfindirectln ( 1205 , outlines ) ;
	  if ( namestree != 0 ) 
	  pdfindirectln ( 1206 , namestree ) ;
	  ...
	  if ( pdfcatalogopenaction != 0 ) 
	  pdfindirectln ( 1207 , pdfcatalogopenaction ) ;
	  pdfenddict () ;
	  pdfprintinfo () ;
	  if ( pdfosenable ) 
	  {
	    ...
	    pdfindirectln ( 1228 , root ) ;
	    pdfindirectln ( 1229 , objptr - 1 ) ;
	  ...
	  if ( ! pdfosenable ) 
	  {
	    ...
	    pdfprint ( 1231 ) ;
	    pdfintentryln ( 1225 , sysobjptr + 1 ) ;
	    pdfindirectln ( 1228 , root ) ;
	...
	if ( fixedpdfdraftmode == 0 ) 
	bclose ( pdffile ) ;
	else pdfwarning ( 0 , 1185 , true , true ) ;
      } 
      if ( logopened ) 
      {
	putc ('\n',  logfile );
