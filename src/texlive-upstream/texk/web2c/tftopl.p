...
{:109}{112:}tfmname,plname:constcstring;
{:112}{100:}procedure parsearguments;const noptions=4;
...
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'tftopl',
': Need one or two file arguments.');usage('tftopl');end;
tfmname:=cmdline(optind);end;{:100}procedure initialize;
begin kpsesetprogramname(argv[0],'tftopl');
kpseinitprog('TFTOPL',0,nil,nil);tfmfilearray:=xmallocarray(byte,1002);
parsearguments;{7:}tfmfile:=kpseopenfile(tfmname,kpsetfmformat);
if verbose then begin write(stderr,'This is TFtoPL, Version 3.3');
writeln(stderr,versionstring);end;
{:7}{17:}if optind+1=argc then begin plfile:=stdout;
...
var tfmptr:indextype;begin{20:}read(tfmfile,tfm[0]);
...
{21:}begin tfmptr:=2;begin if tfm[tfmptr]>127 then begin writeln(stderr,
...
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a TFM?')
;goto 9999;end;if(bc>ec+1)or(ec>255)then begin writeln(stderr,
...
{59:}if(fonttype=1)and(np<>22)then writeln(stderr,
...
writeln(stderr,'I have set it to zero.');end;{:62}end;
...
end{:78};end;{:98}{99:}begin initialize;if not organize then uexit(1);
dosimplethings;
