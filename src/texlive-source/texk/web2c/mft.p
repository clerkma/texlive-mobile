...
const{8:}maxbytes=60000;maxnames=6000;hashsize=353;bufsize=3000;
linelength=79;{:8}type{12:}ASCIIcode=0..255;
{:12}{13:}textfile=packed file of ASCIIcode;{:13}{50:}eightbits=0..255;
sixteenbits=0..65535;{:50}{52:}namepointer=0..maxnames;
{:52}var{9:}history:0..3;{:9}{15:}xord:array[ASCIIcode]of ASCIIcode;
xchr:array[ASCIIcode]of ASCIIcode;{:15}{23:}mffile:textfile;
changefile:textfile;stylefile:textfile;{:23}{25:}texfile:textfile;
{:25}{27:}buffer:array[0..bufsize]of ASCIIcode;{:27}{34:}line:integer;
otherline:integer;templine:integer;limit:0..bufsize;loc:0..bufsize;
inputhasended:boolean;changing:boolean;styling:boolean;
{:34}{36:}changebuffer:array[0..bufsize]of ASCIIcode;
changelimit:0..bufsize;
{:36}{51:}bytemem:packed array[0..maxbytes]of ASCIIcode;
...
{:55}{74:}translation:array[ASCIIcode]of namepointer;i:ASCIIcode;
{:74}{76:}trle,trge,trne,tramp,trsharp,trskip,trps,trquad:namepointer;
{:76}{77:}curtype:eightbits;curtok:integer;prevtype:eightbits;
prevtok:integer;scannerstatus:0..2;{:77}{79:}startofline:boolean;
emptybuffer:boolean;{:79}{80:}charclass:array[ASCIIcode]of 0..21;
{:80}{88:}outbuf:array[0..linelength]of ASCIIcode;outptr:0..linelength;
outline:integer;{:88}{117:}changename,texname:constcstring;
stylename:array[0..31]of constcstring;nstylename:cinttype;
istylename:cinttype;metapost:cinttype;{:117}{29:}procedure error;
var k,l:0..bufsize;
begin{30:}begin if styling then write(stdout,'. (style file ')else if
changing then write(stdout,'. (change file ')else write(stdout,'. (');
...
fflush(stdout);history:=2;end;{:29}{31:}procedure jumpout;
begin{116:}case history of 0:begin writeln(stdout);
...
write(stdout,'(That was a fatal error, my friend.)');end;end{:116};
writeln(stdout);if(history<>0)and(history<>1)then uexit(1)else uexit(0);
end;{:31}{118:}procedure parsearguments;const noptions=5;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
suffix:constcstring;begin{124:}metapost:=false;{:124};
{119:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:119}{120:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:120}{121:}longoptions[currentoption].name:='change';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:121}{122:}longoptions[currentoption].name:='style';
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:122}{123:}longoptions[currentoption].name:='metapost';
...
{:123}{125:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:125};nstylename:=0;
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('mft');
...
if(nstylename=32)then begin begin writeln(stdout);
write(stdout,'Too many style files specified.');error;history:=3;
jumpout;end;usage('mft');end;
stylename[nstylename]:=extendfilename(optarg,'mft');
nstylename:=nstylename+1;end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'mft',
': Need exactly one file argument.');usage('mft');end;
suffix:=findsuffix(cmdline(optind));
if suffix and(strcmp(suffix,'mp')=0)then begin metapost:=true;
texname:=basenamechangesuffix(cmdline(optind),'.mp','.tex');
end else begin texname:=basenamechangesuffix(cmdline(optind),'.mf',
'.tex');end;
if(nstylename=0)then begin if metapost then stylename[0]:='mplain.mft'
else stylename[0]:='plain.mft';nstylename:=1;end;end;
{:118}procedure initialize;var{14:}i:0..255;{:14}{56:}h:0..hashsize;
{:56}begin kpsesetprogramname(argv[0],'mft');parsearguments;
{10:}history:=0;{:10}{16:}xchr[32]:=' ';xchr[33]:='!';xchr[34]:='"';
...
{:16}{17:}for i:=1 to 31 do xchr[i]:=chr(i);
for i:=127 to 255 do xchr[i]:=chr(i);
{:17}{18:}for i:=0 to 255 do xord[chr(i)]:=127;
for i:=128 to 255 do xord[xchr[i]]:=i;
for i:=1 to 126 do xord[xchr[i]]:=i;
{:18}{21:}{:21}{26:}rewrite(texfile,texname);{:26}{54:}bytestart[0]:=0;
byteptr:=0;bytestart[1]:=0;nameptr:=1;
{:54}{57:}for h:=0 to hashsize-1 do hash[h]:=0;{:57}{78:}curtype:=1;
curtok:=0;{:78}{81:}for i:=48 to 57 do charclass[i]:=0;charclass[46]:=1;
...
charclass[13]:=21;charclass[9]:=2;charclass[12]:=2;{:81}{90:}outptr:=1;
outbuf[1]:=32;outline:=1;
if metapost then begin write(texfile,'\input mptmac');
end else write(texfile,'\input mftmac');{:90}{92:}outbuf[0]:=92;{:92};
end;{:3}{24:}procedure openinput;
begin if metapost then mffile:=kpseopenfile(cmdline(optind),kpsempformat
)else mffile:=kpseopenfile(cmdline(optind),kpsemfformat);
if changename then begin if metapost then changefile:=kpseopenfile(
changename,kpsempformat)else changefile:=kpseopenfile(changename,
kpsemfformat);end;stylefile:=kpseopenfile(stylename[0],kpsemftformat);
istylename:=1;end;{:24}{28:}function inputln(var f:textfile):boolean;
...
{:28}{37:}function linesdontmatch:boolean;label 10;var k:0..bufsize;
...
{:37}{38:}procedure primethechangebuffer;label 22,30,10;
var k:0..bufsize;begin changelimit:=0;
{39:}while true do begin line:=line+1;
...
{43:}if limit>1 then if buffer[0]=64 then begin if(buffer[1]>=88)and(
...
if not inputln(stylefile)then begin if istylename<>nstylename then begin
xfclose(stylefile,stylename[istylename-1]);
stylefile:=kpseopenfile(stylename[istylename],kpsemftformat);
istylename:=istylename+1;end else begin styling:=false;end;line:=0;end;
end{:47};
...
changing:=not changing;templine:=otherline;otherline:=line;
line:=templine;end;end;end{:48};
if not changing then begin{46:}begin line:=line+1;
...
end{:46};if changing then goto 20;end;end;end;
{:45}{58:}function lookup:namepointer;label 31;var i:0..bufsize;
h:0..hashsize;k:0..maxbytes;l:0..bufsize;p:namepointer;
begin l:=idloc-idfirst;{59:}h:=buffer[idfirst];i:=idfirst+1;
while i<idloc do begin h:=(h+h+buffer[i])mod hashsize;i:=i+1;end{:59};
{60:}p:=hash[h];
while p<>0 do begin if bytestart[p+1]-bytestart[p]=l then{61:}begin i:=
...
if p=nameptr then{62:}begin if byteptr+l>maxbytes then begin writeln(
stdout);write(stdout,'! Sorry, ','byte memory',' capacity exceeded');
error;history:=3;jumpout;end;
if nameptr+1>maxnames then begin writeln(stdout);
...
i:=i+1;end;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
{63:}ilk[p]:=36{:63};end{:62};lookup:=p;end;{:58}{82:}procedure getnext;
label 25,85,86,30,31,10;var c:ASCIIcode;classvar:ASCIIcode;
begin prevtype:=curtype;prevtok:=curtok;
if emptybuffer then{87:}begin getline;
if inputhasended then begin curtype:=2;curtok:=idfirst;goto 10;end;
buffer[limit]:=13;loc:=0;startofline:=true;emptybuffer:=false;end{:87};
25:c:=buffer[loc];idfirst:=loc;loc:=loc+1;classvar:=charclass[c];
{83:}case classvar of 0:goto 85;
1:begin classvar:=charclass[buffer[loc]];
if classvar>1 then goto 25 else if classvar<1 then goto 86;end;
2:if startofline or scannerstatus>0 then begin curtype:=0;
curtok:=idfirst;goto 10;end else goto 25;21:begin curtype:=1;
curtok:=idfirst;goto 10;end;
4:{84:}while true do begin if buffer[loc]=34 then begin loc:=loc+1;
begin curtype:=7;curtok:=idfirst;goto 10;end;end;
if loc=limit then{85:}begin begin writeln(stdout);
write(stdout,'! Incomplete string will be ignored');error;end;goto 25;
end{:85};loc:=loc+1;end{:84};5,6,7,8:goto 31;
20:{86:}begin begin writeln(stdout);
write(stdout,'! Invalid character will be ignored');error;end;goto 25;
end{:86};others:end;while charclass[buffer[loc]]=classvar do loc:=loc+1;
...
30:begin curtype:=6;curtok:=idfirst;goto 10;end{:83};31:idloc:=loc;
curtok:=lookup;curtype:=ilk[curtok];10:end;
{:82}{89:}procedure flushbuffer(b:eightbits;percent:boolean);label 30;
...
outptr:=outptr-b;end;{:89}{93:}procedure breakout;label 10;
var k:0..linelength;d:ASCIIcode;begin k:=outptr;
while true do begin if k=0 then{94:}begin begin writeln(stdout);
...
if history=0 then history:=1;flushbuffer(outptr-1,true);goto 10;
end{:94};d:=outbuf[k];if d=32 then begin flushbuffer(k,false);goto 10;
end;if(d=92)and(outbuf[k-1]<>92)then begin flushbuffer(k-1,true);
goto 10;end;k:=k-1;end;10:end;{:93}{95:}procedure outstr(p:namepointer);
var k:0..maxbytes;
...
end;end;{:95}{96:}procedure outname(p:namepointer);var k:0..maxbytes;
t:namepointer;
...
{:96}{97:}procedure outmacandname(n:ASCIIcode;p:namepointer);
...
outptr:=outptr+1;outbuf[outptr]:=125;end;end;end;
{:97}{98:}procedure copy(firstloc:integer);var k:0..bufsize;
begin for k:=firstloc to loc-1 do begin if outptr=linelength then
breakout;outptr:=outptr+1;outbuf[outptr]:=buffer[k];end;end;
{:98}{99:}procedure dothetranslation;label 20,21,30,10;var k:0..bufsize;
t:integer;begin 20:if outptr>0 then flushbuffer(outptr,false);
emptybuffer:=true;while true do begin getnext;
if startofline then{101:}if curtype>=6 then begin begin if outptr=
linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=36;end;
startofline:=false;
...
end else if curtype=5 then goto 20{:101};
21:case curtype of 6:{108:}if buffer[loc]=47 then if charclass[buffer[
loc+1]]=0 then begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=92;if outptr=linelength then breakout;
...
outptr:=outptr+1;outbuf[outptr]:=99;end;copy(curtok);getnext;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=47;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=123;end;getnext;copy(curtok);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end;end else copy(curtok)else copy(curtok){:108};
7:{102:}begin begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=55;end;copy(curtok);end{:102};
33,32:{100:}begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=92;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=109;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=102;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=116;end;
begin if outptr=linelength then breakout;outptr:=outptr+1;
...
outbuf[outptr]:=103;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=105;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=110;end;
if curtype=33 then begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=86;end;scannerstatus:=1;
end else if curtype=32 then begin begin if outptr=linelength then
breakout;outptr:=outptr+1;outbuf[outptr]:=66;end;scannerstatus:=2;end;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=123;end;outname(curtok);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end;getnext;
while curtype<>34 do begin if curtype=0 then begin begin if outptr=
linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=32;end;
end else if curtype=1 then begin flushbuffer(outptr,false);
emptybuffer:=true;end else copy(idfirst);getnext;end;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
...
outbuf[outptr]:=116;end;begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=101;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=110;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=100;end;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=123;end;outname(curtok);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end;
if scannerstatus=1 then begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=36;end;scannerstatus:=0;end{:100};
0:outstr(trquad);
1,5:{113:}begin begin if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=36;end;
...
outptr:=outptr+1;outbuf[outptr]:=97;if outptr=linelength then breakout;
outptr:=outptr+1;outbuf[outptr]:=114;end;goto 20;end;end{:113};
2:goto 10;{103:}8:outmacandname(49,curtok);9:outmacandname(50,curtok);
...
19:outmacandname(56,curtok);20:outmacandname(63,curtok);
{:103}{104:}16,27,12:outname(curtok);
23:begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=59;end;21:begin outname(curtok);getnext;
...
end;goto 21;end;{:104}{105:}22:outstr(translation[92]);15:outstr(trps);
24:outstr(trle);25:outstr(trge);26:outstr(trne);14:outstr(tramp);
{:105}{106:}31:begin outmacandname(50,curtok);
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;if outptr=linelength then breakout;outptr:=outptr+1;
...
outbuf[outptr]:=120;end;{107:}while buffer[loc]=32 do loc:=loc+1;
begin if outptr=linelength then breakout;outptr:=outptr+1;
...
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=125;end{:107};end;
{:106}28,29:{111:}begin if curtype=28 then begin if outptr=linelength
then breakout;outptr:=outptr+1;outbuf[outptr]:=92;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=57;
end;idfirst:=loc;while(loc<limit)and(buffer[loc]<>124)do loc:=loc+1;
copy(idfirst);if loc<limit then begin startofline:=true;loc:=loc+1;
k:=loc;while(k<limit)and(buffer[k]<>124)do k:=k+1;buffer[k]:=13;
...
outbuf[outptr]:=114;end;goto 20;end;end{:111};
3:{112:}begin idfirst:=loc;loc:=limit;copy(idfirst);
if outptr=0 then begin outptr:=1;outbuf[1]:=32;end;goto 20;end{:112};
4:{114:}begin startofline:=false;getnext;t:=curtype;
...
end;goto 21;end;emptybuffer:=true;goto 20;end{:114};
30,35,36:{109:}begin if bytestart[curtok+1]-bytestart[curtok]=1 then
outname(curtok)else outmacandname(92,curtok);getnext;
...
35:if curtype<30 then goto 21 else begin begin if outptr=linelength then
breakout;outptr:=outptr+1;outbuf[outptr]:=46;end;curtype:=30;goto 21;
end;
36:begin if curtype=36 then if bytemem[bytestart[curtok]]=39 then goto
21;
if(curtype=6)or(curtype>=30)then{110:}begin begin if outptr=linelength
then breakout;outptr:=outptr+1;outbuf[outptr]:=95;
if outptr=linelength then breakout;outptr:=outptr+1;outbuf[outptr]:=123;
end;
while true do begin if curtype>=30 then outname(curtok)else copy(curtok)
;if prevtype=35 then begin getnext;goto 30;end;getnext;
...
outptr:=outptr+1;outbuf[outptr]:=125;end;goto 21;
end{:110}else if curtype=27 then outstr(trsharp)else goto 21;end;end;
end{:109};end;end;10:end;{:99}{115:}begin initialize;
...
idfirst:=11;buffer[11]:=105;buffer[12]:=110;buffer[13]:=116;
buffer[14]:=101;buffer[15]:=114;buffer[16]:=105;buffer[17]:=109;
curtok:=lookup;ilk[curtok]:=(9);idfirst:=15;buffer[15]:=108;
buffer[16]:=101;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(9);
idfirst:=7;buffer[7]:=110;buffer[8]:=101;buffer[9]:=119;buffer[10]:=105;
...
curtok:=lookup;ilk[curtok]:=(9);idfirst:=8;buffer[8]:=114;buffer[9]:=97;
...
curtok:=lookup;ilk[curtok]:=(17);idfirst:=16;buffer[16]:=61;
buffer[17]:=58;curtok:=lookup;ilk[curtok]:=(16);idfirst:=15;
buffer[15]:=61;buffer[16]:=58;buffer[17]:=124;curtok:=lookup;
ilk[curtok]:=(16);idfirst:=14;buffer[14]:=61;buffer[15]:=58;
buffer[16]:=124;buffer[17]:=62;curtok:=lookup;ilk[curtok]:=(16);
idfirst:=15;buffer[15]:=124;buffer[16]:=61;buffer[17]:=58;
curtok:=lookup;ilk[curtok]:=(16);idfirst:=14;buffer[14]:=124;
buffer[15]:=61;buffer[16]:=58;buffer[17]:=62;curtok:=lookup;
ilk[curtok]:=(16);idfirst:=14;buffer[14]:=124;buffer[15]:=61;
buffer[16]:=58;buffer[17]:=124;curtok:=lookup;ilk[curtok]:=(16);
idfirst:=13;buffer[13]:=124;buffer[14]:=61;buffer[15]:=58;
buffer[16]:=124;buffer[17]:=62;curtok:=lookup;ilk[curtok]:=(16);
idfirst:=12;buffer[12]:=124;buffer[13]:=61;buffer[14]:=58;
buffer[15]:=124;buffer[16]:=62;buffer[17]:=62;curtok:=lookup;
ilk[curtok]:=(16);idfirst:=14;buffer[14]:=107;buffer[15]:=101;
buffer[16]:=114;buffer[17]:=110;curtok:=lookup;ilk[curtok]:=(11);
idfirst:=12;buffer[12]:=115;buffer[13]:=107;buffer[14]:=105;
buffer[15]:=112;buffer[16]:=116;buffer[17]:=111;curtok:=lookup;
ilk[curtok]:=(9);{:66}{67:}idfirst:=5;buffer[5]:=110;buffer[6]:=111;
buffer[7]:=114;buffer[8]:=109;buffer[9]:=97;buffer[10]:=108;
buffer[11]:=100;buffer[12]:=101;buffer[13]:=118;buffer[14]:=105;
buffer[15]:=97;buffer[16]:=116;buffer[17]:=101;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=15;buffer[15]:=111;buffer[16]:=100;
buffer[17]:=100;curtok:=lookup;ilk[curtok]:=(8);idfirst:=13;
buffer[13]:=107;buffer[14]:=110;buffer[15]:=111;buffer[16]:=119;
buffer[17]:=110;curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;
buffer[11]:=117;buffer[12]:=110;buffer[13]:=107;buffer[14]:=110;
buffer[15]:=111;buffer[16]:=119;buffer[17]:=110;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=15;buffer[15]:=110;buffer[16]:=111;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;
buffer[11]:=100;buffer[12]:=101;buffer[13]:=99;buffer[14]:=105;
buffer[15]:=109;buffer[16]:=97;buffer[17]:=108;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=11;buffer[11]:=114;buffer[12]:=101;
buffer[13]:=118;buffer[14]:=101;buffer[15]:=114;buffer[16]:=115;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(8);idfirst:=10;
buffer[10]:=109;buffer[11]:=97;buffer[12]:=107;buffer[13]:=101;
buffer[14]:=112;buffer[15]:=97;buffer[16]:=116;buffer[17]:=104;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;buffer[11]:=109;
buffer[12]:=97;buffer[13]:=107;buffer[14]:=101;buffer[15]:=112;
buffer[16]:=101;buffer[17]:=110;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=15;buffer[15]:=111;buffer[16]:=99;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=15;buffer[15]:=104;
buffer[16]:=101;buffer[17]:=120;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=13;buffer[13]:=65;buffer[14]:=83;buffer[15]:=67;buffer[16]:=73;
buffer[17]:=73;curtok:=lookup;ilk[curtok]:=(8);idfirst:=14;
buffer[14]:=99;buffer[15]:=104;buffer[16]:=97;buffer[17]:=114;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=12;buffer[12]:=108;
buffer[13]:=101;buffer[14]:=110;buffer[15]:=103;buffer[16]:=116;
buffer[17]:=104;curtok:=lookup;ilk[curtok]:=(8);idfirst:=5;
buffer[5]:=116;buffer[6]:=117;buffer[7]:=114;buffer[8]:=110;
buffer[9]:=105;buffer[10]:=110;buffer[11]:=103;buffer[12]:=110;
buffer[13]:=117;buffer[14]:=109;buffer[15]:=98;buffer[16]:=101;
buffer[17]:=114;curtok:=lookup;ilk[curtok]:=(8);idfirst:=13;
buffer[13]:=120;buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=13;
buffer[13]:=121;buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=12;
buffer[12]:=120;buffer[13]:=120;buffer[14]:=112;buffer[15]:=97;
buffer[16]:=114;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=12;buffer[12]:=120;buffer[13]:=121;buffer[14]:=112;
buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=12;buffer[12]:=121;buffer[13]:=120;
buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=12;buffer[12]:=121;
buffer[13]:=121;buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=14;
buffer[14]:=115;buffer[15]:=113;buffer[16]:=114;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=14;buffer[14]:=109;
buffer[15]:=101;buffer[16]:=120;buffer[17]:=112;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=14;buffer[14]:=109;buffer[15]:=108;
buffer[16]:=111;buffer[17]:=103;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=14;buffer[14]:=115;buffer[15]:=105;buffer[16]:=110;
buffer[17]:=100;curtok:=lookup;ilk[curtok]:=(8);idfirst:=14;
buffer[14]:=99;buffer[15]:=111;buffer[16]:=115;buffer[17]:=100;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=13;buffer[13]:=102;
buffer[14]:=108;buffer[15]:=111;buffer[16]:=111;buffer[17]:=114;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=4;buffer[4]:=117;
buffer[5]:=110;buffer[6]:=105;buffer[7]:=102;buffer[8]:=111;
buffer[9]:=114;buffer[10]:=109;buffer[11]:=100;buffer[12]:=101;
buffer[13]:=118;buffer[14]:=105;buffer[15]:=97;buffer[16]:=116;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(8);idfirst:=8;
buffer[8]:=99;buffer[9]:=104;buffer[10]:=97;buffer[11]:=114;
...
buffer[16]:=108;buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(8);
{:67}{68:}idfirst:=5;buffer[5]:=116;buffer[6]:=114;buffer[7]:=97;
...
ilk[curtok]:=(30);idfirst:=5;buffer[5]:=116;buffer[6]:=114;
buffer[7]:=97;buffer[8]:=99;buffer[9]:=105;buffer[10]:=110;
buffer[11]:=103;buffer[12]:=111;buffer[13]:=117;buffer[14]:=116;
buffer[15]:=112;buffer[16]:=117;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=6;buffer[6]:=116;buffer[7]:=114;
buffer[8]:=97;buffer[9]:=99;buffer[10]:=105;buffer[11]:=110;
buffer[12]:=103;buffer[13]:=115;buffer[14]:=116;buffer[15]:=97;
buffer[16]:=116;buffer[17]:=115;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=5;buffer[5]:=116;buffer[6]:=114;buffer[7]:=97;buffer[8]:=99;
buffer[9]:=105;buffer[10]:=110;buffer[11]:=103;buffer[12]:=111;
buffer[13]:=110;buffer[14]:=108;buffer[15]:=105;buffer[16]:=110;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(30);{:68}{69:}idfirst:=14;
buffer[14]:=121;buffer[15]:=101;buffer[16]:=97;buffer[17]:=114;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=13;buffer[13]:=109;
buffer[14]:=111;buffer[15]:=110;buffer[16]:=116;buffer[17]:=104;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=15;buffer[15]:=100;
buffer[16]:=97;buffer[17]:=121;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=14;buffer[14]:=116;buffer[15]:=105;buffer[16]:=109;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(30);idfirst:=10;
buffer[10]:=99;buffer[11]:=104;buffer[12]:=97;buffer[13]:=114;
buffer[14]:=99;buffer[15]:=111;buffer[16]:=100;buffer[17]:=101;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=12;buffer[12]:=99;
buffer[13]:=104;buffer[14]:=97;buffer[15]:=114;buffer[16]:=119;
buffer[17]:=100;curtok:=lookup;ilk[curtok]:=(30);idfirst:=12;
buffer[12]:=99;buffer[13]:=104;buffer[14]:=97;buffer[15]:=114;
buffer[16]:=104;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=12;buffer[12]:=99;buffer[13]:=104;buffer[14]:=97;
buffer[15]:=114;buffer[16]:=100;buffer[17]:=112;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=12;buffer[12]:=99;buffer[13]:=104;
buffer[14]:=97;buffer[15]:=114;buffer[16]:=105;buffer[17]:=99;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=12;buffer[12]:=99;
buffer[13]:=104;buffer[14]:=97;buffer[15]:=114;buffer[16]:=100;
buffer[17]:=120;curtok:=lookup;ilk[curtok]:=(30);idfirst:=12;
buffer[12]:=99;buffer[13]:=104;buffer[14]:=97;buffer[15]:=114;
buffer[16]:=100;buffer[17]:=121;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=8;buffer[8]:=100;buffer[9]:=101;buffer[10]:=115;
buffer[11]:=105;buffer[12]:=103;buffer[13]:=110;buffer[14]:=115;
buffer[15]:=105;buffer[16]:=122;buffer[17]:=101;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=11;buffer[11]:=120;buffer[12]:=111;
buffer[13]:=102;buffer[14]:=102;buffer[15]:=115;buffer[16]:=101;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(30);idfirst:=11;
buffer[11]:=121;buffer[12]:=111;buffer[13]:=102;buffer[14]:=102;
buffer[15]:=115;buffer[16]:=101;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=11;buffer[11]:=112;buffer[12]:=97;
buffer[13]:=117;buffer[14]:=115;buffer[15]:=105;buffer[16]:=110;
buffer[17]:=103;curtok:=lookup;ilk[curtok]:=(30);idfirst:=6;
buffer[6]:=115;buffer[7]:=104;buffer[8]:=111;buffer[9]:=119;
buffer[10]:=115;buffer[11]:=116;buffer[12]:=111;buffer[13]:=112;
buffer[14]:=112;buffer[15]:=105;buffer[16]:=110;buffer[17]:=103;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=8;buffer[8]:=102;
buffer[9]:=111;buffer[10]:=110;buffer[11]:=116;buffer[12]:=109;
buffer[13]:=97;buffer[14]:=107;buffer[15]:=105;buffer[16]:=110;
buffer[17]:=103;curtok:=lookup;ilk[curtok]:=(30);idfirst:=10;
buffer[10]:=112;buffer[11]:=114;buffer[12]:=111;buffer[13]:=111;
buffer[14]:=102;buffer[15]:=105;buffer[16]:=110;buffer[17]:=103;
...
buffer[15]:=112;buffer[16]:=101;buffer[17]:=110;curtok:=lookup;
ilk[curtok]:=(9);idfirst:=11;buffer[11]:=109;buffer[12]:=101;
buffer[13]:=115;buffer[14]:=115;buffer[15]:=97;buffer[16]:=103;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(9);idfirst:=8;
buffer[8]:=101;buffer[9]:=114;buffer[10]:=114;buffer[11]:=109;
buffer[12]:=101;buffer[13]:=115;buffer[14]:=115;buffer[15]:=97;
buffer[16]:=103;buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(9);
idfirst:=11;buffer[11]:=101;buffer[12]:=114;buffer[13]:=114;
buffer[14]:=104;buffer[15]:=101;buffer[16]:=108;buffer[17]:=112;
curtok:=lookup;ilk[curtok]:=(9);idfirst:=10;buffer[10]:=99;
buffer[11]:=104;buffer[12]:=97;buffer[13]:=114;buffer[14]:=108;
buffer[15]:=105;buffer[16]:=115;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(9);idfirst:=10;buffer[10]:=108;buffer[11]:=105;
buffer[12]:=103;buffer[13]:=116;buffer[14]:=97;buffer[15]:=98;
buffer[16]:=108;buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(9);
idfirst:=8;buffer[8]:=101;buffer[9]:=120;buffer[10]:=116;
buffer[11]:=101;buffer[12]:=110;buffer[13]:=115;buffer[14]:=105;
buffer[15]:=98;buffer[16]:=108;buffer[17]:=101;curtok:=lookup;
ilk[curtok]:=(9);idfirst:=8;buffer[8]:=104;buffer[9]:=101;
buffer[10]:=97;buffer[11]:=100;buffer[12]:=101;buffer[13]:=114;
buffer[14]:=98;buffer[15]:=121;buffer[16]:=116;buffer[17]:=101;
curtok:=lookup;ilk[curtok]:=(9);idfirst:=9;buffer[9]:=102;
buffer[10]:=111;buffer[11]:=110;buffer[12]:=116;buffer[13]:=100;
buffer[14]:=105;buffer[15]:=109;buffer[16]:=101;buffer[17]:=110;
curtok:=lookup;ilk[curtok]:=(9);idfirst:=11;buffer[11]:=115;
buffer[12]:=112;buffer[13]:=101;buffer[14]:=99;buffer[15]:=105;
buffer[16]:=97;buffer[17]:=108;curtok:=lookup;ilk[curtok]:=(9);
idfirst:=17;buffer[17]:=37;curtok:=lookup;ilk[curtok]:=(28);idfirst:=16;
...
idfirst:=17;buffer[17]:=35;curtok:=lookup;ilk[curtok]:=(27);idfirst:=14;
buffer[14]:=103;buffer[15]:=111;buffer[16]:=111;buffer[17]:=100;
curtok:=lookup;ilk[curtok]:=(35);
{:71}{72:}if not metapost then begin idfirst:=6;buffer[6]:=97;
buffer[7]:=117;buffer[8]:=116;buffer[9]:=111;buffer[10]:=114;
buffer[11]:=111;buffer[12]:=117;buffer[13]:=110;buffer[14]:=100;
...
buffer[17]:=109;curtok:=lookup;ilk[curtok]:=(30);idfirst:=10;
buffer[10]:=100;buffer[11]:=114;buffer[12]:=111;buffer[13]:=112;
buffer[14]:=112;buffer[15]:=105;buffer[16]:=110;buffer[17]:=103;
...
idfirst:=12;buffer[12]:=102;buffer[13]:=105;buffer[14]:=108;
buffer[15]:=108;buffer[16]:=105;buffer[17]:=110;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=7;buffer[7]:=103;buffer[8]:=114;
buffer[9]:=97;buffer[10]:=110;buffer[11]:=117;buffer[12]:=108;
buffer[13]:=97;buffer[14]:=114;buffer[15]:=105;buffer[16]:=116;
buffer[17]:=121;curtok:=lookup;ilk[curtok]:=(30);idfirst:=10;
buffer[10]:=105;buffer[11]:=110;buffer[12]:=119;buffer[13]:=105;
buffer[14]:=110;buffer[15]:=100;buffer[16]:=111;buffer[17]:=119;
curtok:=lookup;ilk[curtok]:=(13);idfirst:=14;buffer[14]:=104;
buffer[15]:=112;buffer[16]:=112;buffer[17]:=112;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=11;buffer[11]:=107;buffer[12]:=101;
buffer[13]:=101;buffer[14]:=112;buffer[15]:=105;buffer[16]:=110;
buffer[17]:=103;curtok:=lookup;ilk[curtok]:=(9);idfirst:=8;
buffer[8]:=110;buffer[9]:=117;buffer[10]:=109;buffer[11]:=115;
buffer[12]:=112;buffer[13]:=101;buffer[14]:=99;buffer[15]:=105;
buffer[16]:=97;buffer[17]:=108;curtok:=lookup;ilk[curtok]:=(9);
...
ilk[curtok]:=(9);idfirst:=9;buffer[9]:=115;buffer[10]:=109;
...
idfirst:=7;buffer[7]:=116;buffer[8]:=111;buffer[9]:=116;buffer[10]:=97;
buffer[11]:=108;buffer[12]:=119;buffer[13]:=101;buffer[14]:=105;
buffer[15]:=103;buffer[16]:=104;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=6;buffer[6]:=116;buffer[7]:=114;buffer[8]:=97;
buffer[9]:=99;buffer[10]:=105;buffer[11]:=110;buffer[12]:=103;
buffer[13]:=101;buffer[14]:=100;buffer[15]:=103;buffer[16]:=101;
buffer[17]:=115;curtok:=lookup;ilk[curtok]:=(30);idfirst:=8;
buffer[8]:=119;buffer[9]:=105;buffer[10]:=116;buffer[11]:=104;
buffer[12]:=119;buffer[13]:=101;buffer[14]:=105;buffer[15]:=103;
buffer[16]:=104;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(9);end;
{:72}{73:}if metapost then begin idfirst:=9;buffer[9]:=97;
buffer[10]:=114;buffer[11]:=99;buffer[12]:=108;buffer[13]:=101;
buffer[14]:=110;buffer[15]:=103;buffer[16]:=116;buffer[17]:=104;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;buffer[11]:=97;
buffer[12]:=114;buffer[13]:=99;buffer[14]:=116;buffer[15]:=105;
buffer[16]:=109;buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(9);
idfirst:=10;buffer[10]:=98;buffer[11]:=108;buffer[12]:=117;
buffer[13]:=101;buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;
buffer[11]:=98;buffer[12]:=111;buffer[13]:=117;buffer[14]:=110;
buffer[15]:=100;buffer[16]:=101;buffer[17]:=100;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=14;buffer[14]:=98;buffer[15]:=116;
buffer[16]:=101;buffer[17]:=120;curtok:=lookup;ilk[curtok]:=(32);
idfirst:=14;buffer[14]:=99;buffer[15]:=108;buffer[16]:=105;
buffer[17]:=112;curtok:=lookup;ilk[curtok]:=(9);idfirst:=11;
buffer[11]:=99;buffer[12]:=108;buffer[13]:=105;buffer[14]:=112;
buffer[15]:=112;buffer[16]:=101;buffer[17]:=100;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=9;buffer[9]:=99;buffer[10]:=108;
buffer[11]:=111;buffer[12]:=115;buffer[13]:=101;buffer[14]:=102;
buffer[15]:=114;buffer[16]:=111;buffer[17]:=109;curtok:=lookup;
ilk[curtok]:=(31);idfirst:=13;buffer[13]:=99;buffer[14]:=111;
buffer[15]:=108;buffer[16]:=111;buffer[17]:=114;curtok:=lookup;
ilk[curtok]:=(18);idfirst:=12;buffer[12]:=100;buffer[13]:=97;
buffer[14]:=115;buffer[15]:=104;buffer[16]:=101;buffer[17]:=100;
curtok:=lookup;ilk[curtok]:=(9);idfirst:=10;buffer[10]:=100;
buffer[11]:=97;buffer[12]:=115;buffer[13]:=104;buffer[14]:=112;
buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=14;buffer[14]:=101;buffer[15]:=116;
buffer[16]:=101;buffer[17]:=120;curtok:=lookup;ilk[curtok]:=(34);
idfirst:=12;buffer[12]:=102;buffer[13]:=105;buffer[14]:=108;
buffer[15]:=108;buffer[16]:=101;buffer[17]:=100;curtok:=lookup;
ilk[curtok]:=(8);idfirst:=10;buffer[10]:=102;buffer[11]:=111;
buffer[12]:=110;buffer[13]:=116;buffer[14]:=112;buffer[15]:=97;
buffer[16]:=114;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=10;buffer[10]:=102;buffer[11]:=111;buffer[12]:=110;
buffer[13]:=116;buffer[14]:=115;buffer[15]:=105;buffer[16]:=122;
buffer[17]:=101;curtok:=lookup;ilk[curtok]:=(8);idfirst:=9;
buffer[9]:=103;buffer[10]:=114;buffer[11]:=101;buffer[12]:=101;
buffer[13]:=110;buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;
buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);idfirst:=12;
buffer[12]:=105;buffer[13]:=110;buffer[14]:=102;buffer[15]:=111;
buffer[16]:=110;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(11);
idfirst:=10;buffer[10]:=108;buffer[11]:=105;buffer[12]:=110;
buffer[13]:=101;buffer[14]:=106;buffer[15]:=111;buffer[16]:=105;
buffer[17]:=110;curtok:=lookup;ilk[curtok]:=(30);idfirst:=11;
buffer[11]:=108;buffer[12]:=105;buffer[13]:=110;buffer[14]:=101;
buffer[15]:=99;buffer[16]:=97;buffer[17]:=112;curtok:=lookup;
ilk[curtok]:=(30);idfirst:=10;buffer[10]:=108;buffer[11]:=108;
buffer[12]:=99;buffer[13]:=111;buffer[14]:=114;buffer[15]:=110;
buffer[16]:=101;buffer[17]:=114;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=10;buffer[10]:=108;buffer[11]:=114;buffer[12]:=99;
buffer[13]:=111;buffer[14]:=114;buffer[15]:=110;buffer[16]:=101;
buffer[17]:=114;curtok:=lookup;ilk[curtok]:=(8);idfirst:=8;
buffer[8]:=109;buffer[9]:=105;buffer[10]:=116;buffer[11]:=101;
buffer[12]:=114;buffer[13]:=108;buffer[14]:=105;buffer[15]:=109;
buffer[16]:=105;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=10;buffer[10]:=109;buffer[11]:=112;buffer[12]:=120;
buffer[13]:=98;buffer[14]:=114;buffer[15]:=101;buffer[16]:=97;
buffer[17]:=107;curtok:=lookup;ilk[curtok]:=(17);idfirst:=10;
buffer[10]:=112;buffer[11]:=97;buffer[12]:=116;buffer[13]:=104;
buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;buffer[11]:=112;
buffer[12]:=101;buffer[13]:=110;buffer[14]:=112;buffer[15]:=97;
buffer[16]:=114;buffer[17]:=116;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=9;buffer[9]:=112;buffer[10]:=114;buffer[11]:=111;
buffer[12]:=108;buffer[13]:=111;buffer[14]:=103;buffer[15]:=117;
buffer[16]:=101;buffer[17]:=115;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=11;buffer[11]:=114;buffer[12]:=101;buffer[13]:=100;
buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=10;buffer[10]:=114;
buffer[11]:=101;buffer[12]:=97;buffer[13]:=100;buffer[14]:=102;
buffer[15]:=114;buffer[16]:=111;buffer[17]:=109;curtok:=lookup;
ilk[curtok]:=(31);idfirst:=9;buffer[9]:=115;buffer[10]:=101;
buffer[11]:=116;buffer[12]:=98;buffer[13]:=111;buffer[14]:=117;
buffer[15]:=110;buffer[16]:=100;buffer[17]:=115;curtok:=lookup;
ilk[curtok]:=(9);idfirst:=11;buffer[11]:=115;buffer[12]:=116;
buffer[13]:=114;buffer[14]:=111;buffer[15]:=107;buffer[16]:=101;
buffer[17]:=100;curtok:=lookup;ilk[curtok]:=(8);idfirst:=10;
buffer[10]:=116;buffer[11]:=101;buffer[12]:=120;buffer[13]:=116;
buffer[14]:=112;buffer[15]:=97;buffer[16]:=114;buffer[17]:=116;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=11;buffer[11]:=116;
buffer[12]:=101;buffer[13]:=120;buffer[14]:=116;buffer[15]:=117;
buffer[16]:=97;buffer[17]:=108;curtok:=lookup;ilk[curtok]:=(8);
idfirst:=2;buffer[2]:=116;buffer[3]:=114;buffer[4]:=97;buffer[5]:=99;
buffer[6]:=105;buffer[7]:=110;buffer[8]:=103;buffer[9]:=108;
buffer[10]:=111;buffer[11]:=115;buffer[12]:=116;buffer[13]:=99;
buffer[14]:=104;buffer[15]:=97;buffer[16]:=114;buffer[17]:=115;
curtok:=lookup;ilk[curtok]:=(30);idfirst:=7;buffer[7]:=116;
buffer[8]:=114;buffer[9]:=117;buffer[10]:=101;buffer[11]:=99;
buffer[12]:=111;buffer[13]:=114;buffer[14]:=110;buffer[15]:=101;
buffer[16]:=114;buffer[17]:=115;curtok:=lookup;ilk[curtok]:=(30);
idfirst:=10;buffer[10]:=117;buffer[11]:=108;buffer[12]:=99;
buffer[13]:=111;buffer[14]:=114;buffer[15]:=110;buffer[16]:=101;
buffer[17]:=114;curtok:=lookup;ilk[curtok]:=(8);idfirst:=10;
buffer[10]:=117;buffer[11]:=114;buffer[12]:=99;buffer[13]:=111;
buffer[14]:=114;buffer[15]:=110;buffer[16]:=101;buffer[17]:=114;
curtok:=lookup;ilk[curtok]:=(8);idfirst:=7;buffer[7]:=118;
buffer[8]:=101;buffer[9]:=114;buffer[10]:=98;buffer[11]:=97;
buffer[12]:=116;buffer[13]:=105;buffer[14]:=109;buffer[15]:=116;
buffer[16]:=101;buffer[17]:=120;curtok:=lookup;ilk[curtok]:=(33);
idfirst:=12;buffer[12]:=119;buffer[13]:=105;buffer[14]:=116;
buffer[15]:=104;buffer[16]:=105;buffer[17]:=110;curtok:=lookup;
ilk[curtok]:=(13);idfirst:=9;buffer[9]:=119;buffer[10]:=105;
buffer[11]:=116;buffer[12]:=104;buffer[13]:=99;buffer[14]:=111;
buffer[15]:=108;buffer[16]:=111;buffer[17]:=114;curtok:=lookup;
ilk[curtok]:=(9);idfirst:=13;buffer[13]:=119;buffer[14]:=114;
buffer[15]:=105;buffer[16]:=116;buffer[17]:=101;curtok:=lookup;
ilk[curtok]:=(9);end;{:73};{75:}for i:=0 to 255 do translation[i]:=0;
byteptr:=byteptr+2;bytemem[byteptr-2]:=92;bytemem[byteptr-1]:=36;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[36]:=curtok;byteptr:=byteptr+2;bytemem[byteptr-2]:=92;
bytemem[byteptr-1]:=35;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;translation[35]:=curtok;byteptr:=byteptr+2;
bytemem[byteptr-2]:=92;bytemem[byteptr-1]:=38;curtok:=nameptr;
nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;translation[38]:=curtok;
byteptr:=byteptr+2;bytemem[byteptr-2]:=92;bytemem[byteptr-1]:=123;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[123]:=curtok;byteptr:=byteptr+2;bytemem[byteptr-2]:=92;
bytemem[byteptr-1]:=125;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;translation[125]:=curtok;byteptr:=byteptr+2;
bytemem[byteptr-2]:=92;bytemem[byteptr-1]:=95;curtok:=nameptr;
nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;translation[95]:=curtok;
byteptr:=byteptr+2;bytemem[byteptr-2]:=92;bytemem[byteptr-1]:=37;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[37]:=curtok;byteptr:=byteptr+4;bytemem[byteptr-4]:=92;
bytemem[byteptr-3]:=66;bytemem[byteptr-2]:=83;bytemem[byteptr-1]:=32;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[92]:=curtok;byteptr:=byteptr+4;bytemem[byteptr-4]:=92;
bytemem[byteptr-3]:=72;bytemem[byteptr-2]:=65;bytemem[byteptr-1]:=32;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[94]:=curtok;byteptr:=byteptr+4;bytemem[byteptr-4]:=92;
bytemem[byteptr-3]:=84;bytemem[byteptr-2]:=73;bytemem[byteptr-1]:=32;
curtok:=nameptr;nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;
translation[126]:=curtok;byteptr:=byteptr+5;bytemem[byteptr-5]:=92;
bytemem[byteptr-4]:=97;bytemem[byteptr-3]:=115;bytemem[byteptr-2]:=116;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;translation[42]:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=65;bytemem[byteptr-2]:=77;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;tramp:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=66;bytemem[byteptr-2]:=76;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trskip:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=83;bytemem[byteptr-2]:=72;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trsharp:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=80;bytemem[byteptr-2]:=83;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trps:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=108;bytemem[byteptr-2]:=101;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trle:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=103;bytemem[byteptr-2]:=101;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trge:=curtok;byteptr:=byteptr+4;
bytemem[byteptr-4]:=92;bytemem[byteptr-3]:=110;bytemem[byteptr-2]:=101;
bytemem[byteptr-1]:=32;curtok:=nameptr;nameptr:=nameptr+1;
bytestart[nameptr]:=byteptr;trne:=curtok;byteptr:=byteptr+5;
bytemem[byteptr-5]:=92;bytemem[byteptr-4]:=113;bytemem[byteptr-3]:=117;
bytemem[byteptr-2]:=97;bytemem[byteptr-1]:=100;curtok:=nameptr;
nameptr:=nameptr+1;bytestart[nameptr]:=byteptr;trquad:=curtok;{:75};
{44:}begin openinput;line:=0;otherline:=0;changing:=true;
primethechangebuffer;changing:=not changing;templine:=otherline;
otherline:=line;line:=templine;styling:=true;limit:=0;loc:=1;
buffer[0]:=32;inputhasended:=false;end{:44};dothetranslation;
{49:}if changelimit<>0 then begin for loc:=0 to changelimit do buffer[
loc]:=changebuffer[loc];limit:=changelimit;changing:=true;
line:=otherline;loc:=changelimit;begin writeln(stdout);
write(stdout,'! Change file entry did not match');error;end;end{:49};
{116:}case history of 0:begin writeln(stdout);
write(stdout,'(No errors were found.)');end;1:begin writeln(stdout);
write(stdout,'(Did you see the warning message above?)');end;
2:begin writeln(stdout);
write(stdout,'(Pardon me, but I think I spotted something wrong.)');end;
