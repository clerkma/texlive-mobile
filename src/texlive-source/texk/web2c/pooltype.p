{2:}program POOLtype(poolfile,output);type{5:}ASCIIcode=0..255;
{:5}var{7:}xord:array[ASCIIcode]of ASCIIcode;
xchr:array[ASCIIcode]of ASCIIcode;{:7}{12:}k,l:0..255;m,n:ASCIIcode;
s:integer;{:12}{13:}count:integer;
{:13}{18:}poolfile:packed file of ASCIIcode;poolname:constcstring;
xsum:boolean;{:18}{21:}procedure parsearguments;const noptions=2;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{22:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:22}{23:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:23}{24:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:24};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval='?'then begin usage('pooltype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(POOLTYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is POOLtype, Version 3.0',nil,'D.E. Knuth',nil
);end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'pooltype',
': Need exactly one file argument.');usage('pooltype');end;
poolname:=extendfilename(cmdline(optind),'pool');
resetbin(poolfile,poolname);end;{:21}procedure initialize;
var{6:}i:integer;{:6}begin kpsesetprogramname(argv[0],'pooltype');
parsearguments;{8:}xchr[32]:=' ';xchr[33]:='!';xchr[34]:='"';
...
{:8}{10:}for i:=0 to 31 do xchr[i]:=chr(i);
...
writeln('"');end{:16};s:=256;{19:}xsum:=false;
if eof(poolfile)then begin writeln(stderr,
'! I can''t read the POOL file.');uexit(1);end;
repeat{20:}if eof(poolfile)then begin writeln(stderr,
'! POOL file contained no check sum');uexit(1);end;read(poolfile,m);
read(poolfile,n);
if m<>'*'then begin if(xord[m]<48)or(xord[m]>57)or(xord[n]<48)or(xord[n]
>57)then begin writeln(stderr,
'! POOL line doesn''t begin with two digits');uexit(1);end;
l:=xord[m]*10+xord[n]-48*11;write(s:3,': "');count:=count+l;
for k:=1 to l do begin if eoln(poolfile)then begin writeln('"');
begin writeln(stderr,'! That POOL line was too short');uexit(1);end;end;
...
if not eof(poolfile)then begin writeln(stderr,
