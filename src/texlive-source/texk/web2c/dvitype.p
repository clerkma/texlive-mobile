...
{:121}{123:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage('dvitype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(DVITYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is DVItype, Version 3.6',nil,'D.E. Knuth',nil)
...
if(optind+1<>argc)then begin writeln(stderr,'dvitype',
': Need exactly one file argument.');usage('dvitype');end;
dviname:=extendfilename(cmdline(optind),'dvi');end;
{:112}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'dvitype');parsearguments;
write(stdout,'This is DVItype, Version 3.6');
