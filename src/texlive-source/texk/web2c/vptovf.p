{2:}program VPtoVF(vplfile,vffile,tfmfile,output);const{3:}bufsize=3000;
maxheaderbytes=1000;vfsize=100000;maxstack=100;maxparamwords=254;
maxligsteps=32510;maxkerns=5000;hashsize=32579;{:3}type{23:}byte=0..255;
ASCIIcode=32..127;{:23}{66:}fourbytes=record b0:byte;b1:byte;b2:byte;
b3:byte;end;{:66}{71:}fixword=integer;
{:71}{78:}headerindex=0..maxheaderbytes;indx=0..32767;
{:78}{81:}pointer=0..1032;{:81}var{5:}vplfile:text;
{:5}{21:}vffile:packed file of 0..255;tfmfile:packed file of 0..255;
{:21}{24:}xord:array[0..255]of ASCIIcode;{:24}{27:}line:integer;
goodindent:integer;indent:integer;level:integer;
{:27}{29:}leftln,rightln:boolean;limit:0..bufsize;loc:0..bufsize;
buffer:array[1..bufsize]of 0..255;inputhasended:boolean;
{:29}{31:}charsonline:0..8;perfect:boolean;{:31}{36:}curchar:ASCIIcode;
{:36}{44:}start:array[1..100]of 0..666;
...
{:47}{52:}equiv:array[0..100]of byte;curcode:byte;
{:52}{67:}curbytes:fourbytes;zerobytes:fourbytes;
{:67}{75:}fractiondigits:array[1..7]of integer;
{:75}{77:}headerbytes:array[headerindex]of byte;headerptr:headerindex;
...
fontdsize:array[byte]of fixword;
{:77}{82:}memory:array[pointer]of fixword;memptr:pointer;
...
charremainder:array[0..256]of 0..65535;{:82}{86:}nextd:fixword;
{:86}{89:}indexvar:array[pointer]of byte;excess:byte;{:89}{91:}c:byte;
x:fixword;k:integer;{:91}{113:}lkstepended:boolean;krnptr:0..maxkerns;
{:113}{123:}hstack:array[0..maxstack]of 0..2;
vstack:array[0..maxstack]of 0..2;
wstack,xstack,ystack,zstack:array[0..maxstack]of fixword;
stackptr:0..maxstack;{:123}{138:}sevenunsafe:boolean;
{:138}{143:}delta:fixword;{:143}{147:}ligptr:0..maxligsteps;
hash:array[0..hashsize]of 0..66048;classvar:array[0..hashsize]of 0..4;
...
lf:0..32767;notfound:boolean;tempwidth:fixword;
{:158}{161:}j:0..maxheaderbytes;p:pointer;q:1..4;
parptr:0..maxparamwords;
{:161}{167:}labeltable:array[0..256]of record rr:-1..32767;cc:byte;end;
labelptr:0..256;sortptr:0..256;lkoffset:0..256;t:0..32767;
extralocneeded:boolean;{:167}{175:}vcount:integer;
{:175}{186:}verbose:cinttype;
{:186}{189:}vplname,tfmname,vfname:constcstring;
{:189}{182:}procedure parsearguments;const noptions=3;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{187:}verbose:=false;{:187};{183:}currentoption:=0;
...
{:183}{184:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:184}{185:}longoptions[currentoption].name:='verbose';
...
{:185}{188:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:188};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('vptovf');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(VPTOVFHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is VPtoVF, Version 1.6',nil,'D.E. Knuth',nil);
end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)and(optind+3<>argc)then begin
writeln(stderr,'vptovf',': Need one to three file arguments.');
usage('vptovf');end;vplname:=extendfilename(cmdline(optind),'vpl');
...
'tfm');end else begin tfmname:=makesuffix(cmdline(optind+1),'tfm');end;
end else begin vfname:=basenamechangesuffix(vplname,'.vpl','.vf');
tfmname:=basenamechangesuffix(vplname,'.vpl','.tfm');end;end;
{:182}procedure initialize;var{25:}k:integer;{:25}{48:}h:0..140;
{:48}{79:}d:headerindex;{:79}{83:}c:byte;
{:83}begin kpsesetprogramname(argv[0],'vptovf');parsearguments;
{6:}reset(vplfile,vplname);
if verbose then begin write(stderr,'This is VPtoVF, Version 1.6');
writeln(stderr,versionstring);end;{:6}{22:}rewritebin(vffile,vfname);
rewritebin(tfmfile,tfmname);
{:22}{26:}for k:=0 to 127 do xord[chr(k)]:=127;xord[' ']:=32;
...
xord['~']:=126;{:26}{28:}line:=0;goodindent:=0;indent:=0;level:=0;
{:28}{30:}limit:=0;loc:=0;leftln:=true;rightln:=true;
inputhasended:=false;{:30}{32:}charsonline:=0;perfect:=true;
{:32}{45:}startptr:=1;start[1]:=0;dictptr:=0;
{:45}{49:}for h:=0 to 140 do nhash[h]:=0;{:49}{68:}zerobytes.b0:=0;
zerobytes.b1:=0;zerobytes.b2:=0;zerobytes.b3:=0;
{:68}{80:}for d:=0 to 18*4-1 do headerbytes[d]:=0;headerbytes[8]:=11;
...
{:84}{148:}hashptr:=0;yligcycle:=256;for k:=0 to hashsize do hash[k]:=0;
{:148}end;{:2}{33:}procedure showerrorcontext;var k:0..bufsize;
...
buffer[limit+1]:=' ';end;
if leftln then{35:}begin while(loc<limit)and(buffer[loc+1]=' ')do loc:=
...
goodindent:=0;indent:=0;end;end;end{:35};end;end;
{:34}{37:}procedure getkeywordchar;
...
{:37}{38:}procedure getnext;begin while loc=limit do fillbuffer;
...
end;{:38}{39:}function gethex:byte;var a:integer;begin repeat getnext;
...
{:40}{41:}procedure copytoendofitem;label 30;var l:integer;
nonblankfound:boolean;begin l:=level;nonblankfound:=false;
...
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;end;30:end;
{:41}{43:}procedure finishtheproperty;begin while curchar=32 do getnext;
...
var k:0..20;j:0..666;notfound:boolean;begin{51:}curhash:=curname[1];
...
{:50}{53:}procedure entername(v:byte);var k:0..20;
...
{:58}{60:}function getbyte:byte;var acc:integer;t:ASCIIcode;
begin repeat getnext;until curchar<>32;t:=curchar;acc:=0;repeat getnext;
until curchar<>32;
if t=67 then{61:}if(curchar>=33)and(curchar<=126)and((curchar<40)or(
...
end{:61}else if t=68 then{62:}begin while(curchar>=48)and(curchar<=57)do
...
end{:62}else if t=79 then{63:}begin while(curchar>=48)and(curchar<=55)do
...
begin if(curchar>41)or(curchar<40)then loc:=loc-1;end;
end{:63}else if t=72 then{64:}begin while((curchar>=48)and(curchar<=57))
...
curchar:=32;end else getnext;end;
begin if(curchar>41)or(curchar<40)then loc:=loc-1;end;
end{:64}else if t=70 then{65:}begin if curchar=66 then acc:=2 else if
...
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
acc:=0;end;
end{:65}else begin begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'You need "C" or "D" or "O" or "H" or "F" here');
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
curchar:=32;getbyte:=acc;end;{:60}{69:}procedure getfourbytes;
var c:integer;r:integer;begin repeat getnext;until curchar<>32;r:=0;
curbytes:=zerobytes;
...
showerrorcontext;end;repeat getnext until(curchar=40)or(curchar=41);end;
end;getnext;end;end{:70};end;end;{:69}{72:}function getfix:fixword;
var negative:boolean;acc:integer;intpart:integer;j:0..7;
begin repeat getnext;until curchar<>32;negative:=false;acc:=0;
...
until curchar<>32{:73};
while(curchar>=48)and(curchar<=57)do{74:}begin acc:=acc*10+curchar-48;
...
{:72}{85:}function sortin(h:pointer;d:fixword):pointer;var p:pointer;
...
{:87}{88:}function shorten(h:pointer;m:integer):fixword;var d:fixword;
...
{:88}{90:}procedure setindices(h:pointer;d:fixword);var p:pointer;
...
{:96}{97:}procedure readBCPL(l:headerindex;n:byte);var k:headerindex;
...
end;{:97}{111:}procedure checktag(c:byte);begin case chartag[c]of 0:;
...
showerrorcontext;end;end;end;
{:111}{124:}{128:}procedure vffix(opcode:byte;x:fixword);
...
until k=0;end;{:128}procedure readpacket(c:byte);var cc:byte;x:fixword;
h,v:0..2;specialstart:0..vfsize;k:0..vfsize;begin packetstart[c]:=vfptr;
...
end else begin begin vf[vfptr]:=235;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;begin vf[vfptr]:=curfont;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;end;end{:126};
81:{127:}if curfont=fontptr then begin if charsonline>0 then writeln(
stderr,' ');
write(stderr,'Character cannot be typeset in undefined font');
showerrorcontext;end else begin cc:=getbyte;
if cc>=128 then begin vf[vfptr]:=128;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;begin vf[vfptr]:=cc;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;end{:127};
82:{129:}begin begin vf[vfptr]:=132;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;vffix(0,getfix);
vffix(0,getfix);end{:129};
...
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end else vffix(143,x);
end{:130};
85,86:{131:}begin if curcode=85 then x:=getfix else x:=-getfix;
...
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end else vffix(157,x);
end{:131};
87:{132:}if stackptr=maxstack then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'Don''t push so much---stack is full!');
showerrorcontext;end else begin begin vf[vfptr]:=141;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
showerrorcontext;end else vfptr:=vfptr+1;end;hstack[stackptr]:=h;
vstack[stackptr]:=v;stackptr:=stackptr+1;h:=0;v:=0;end{:132};
88:{133:}if stackptr=0 then begin if charsonline>0 then writeln(stderr,
...
h:=hstack[stackptr];v:=vstack[stackptr];end{:133};
89,90:{134:}begin begin vf[vfptr]:=239;
if vfptr=vfsize then begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I''m out of memory---increase my vfsize!');
...
if vfptr-specialstart>255 then{135:}if vfptr+3>vfsize then begin begin
...
end{:125}else if curchar=41 then skiptoendofitem else junkerror;end;
...
{:152}{153:}function f(h,x,y:indx):indx;begin case classvar[h]of 0:;
...
end;f:=ligz[h];end;{:153}{165:}procedure outscaled(x:fixword);
...
{:165}{176:}procedure voutint(x:integer);
...
putbyte((x div 256)mod 256,vffile);putbyte(x mod 256,vffile);end;
{:176}{180:}procedure paramenter;begin{57:}namelength:=5;
curname[16]:=83;curname[17]:=76;curname[18]:=65;curname[19]:=78;
curname[20]:=84;entername(31);namelength:=5;curname[16]:=83;
curname[17]:=80;curname[18]:=65;curname[19]:=67;curname[20]:=69;
entername(32);namelength:=7;curname[14]:=83;curname[15]:=84;
curname[16]:=82;curname[17]:=69;curname[18]:=84;curname[19]:=67;
curname[20]:=72;entername(33);namelength:=6;curname[15]:=83;
curname[16]:=72;curname[17]:=82;curname[18]:=73;curname[19]:=78;
curname[20]:=75;entername(34);namelength:=7;curname[14]:=88;
curname[15]:=72;curname[16]:=69;curname[17]:=73;curname[18]:=71;
curname[19]:=72;curname[20]:=84;entername(35);namelength:=4;
curname[17]:=81;curname[18]:=85;curname[19]:=65;curname[20]:=68;
entername(36);namelength:=10;curname[11]:=69;curname[12]:=88;
curname[13]:=84;curname[14]:=82;curname[15]:=65;curname[16]:=83;
curname[17]:=80;curname[18]:=65;curname[19]:=67;curname[20]:=69;
entername(37);namelength:=4;curname[17]:=78;curname[18]:=85;
curname[19]:=77;curname[20]:=49;entername(38);namelength:=4;
curname[17]:=78;curname[18]:=85;curname[19]:=77;curname[20]:=50;
entername(39);namelength:=4;curname[17]:=78;curname[18]:=85;
curname[19]:=77;curname[20]:=51;entername(40);namelength:=6;
curname[15]:=68;curname[16]:=69;curname[17]:=78;curname[18]:=79;
curname[19]:=77;curname[20]:=49;entername(41);namelength:=6;
curname[15]:=68;curname[16]:=69;curname[17]:=78;curname[18]:=79;
curname[19]:=77;curname[20]:=50;entername(42);namelength:=4;
curname[17]:=83;curname[18]:=85;curname[19]:=80;curname[20]:=49;
entername(43);namelength:=4;curname[17]:=83;curname[18]:=85;
curname[19]:=80;curname[20]:=50;entername(44);namelength:=4;
curname[17]:=83;curname[18]:=85;curname[19]:=80;curname[20]:=51;
entername(45);namelength:=4;curname[17]:=83;curname[18]:=85;
curname[19]:=66;curname[20]:=49;entername(46);namelength:=4;
curname[17]:=83;curname[18]:=85;curname[19]:=66;curname[20]:=50;
entername(47);namelength:=7;curname[14]:=83;curname[15]:=85;
curname[16]:=80;curname[17]:=68;curname[18]:=82;curname[19]:=79;
curname[20]:=80;entername(48);namelength:=7;curname[14]:=83;
curname[15]:=85;curname[16]:=66;curname[17]:=68;curname[18]:=82;
curname[19]:=79;curname[20]:=80;entername(49);namelength:=6;
curname[15]:=68;curname[16]:=69;curname[17]:=76;curname[18]:=73;
curname[19]:=77;curname[20]:=49;entername(50);namelength:=6;
curname[15]:=68;curname[16]:=69;curname[17]:=76;curname[18]:=73;
curname[19]:=77;curname[20]:=50;entername(51);namelength:=10;
curname[11]:=65;curname[12]:=88;curname[13]:=73;curname[14]:=83;
curname[15]:=72;curname[16]:=69;curname[17]:=73;curname[18]:=71;
curname[19]:=72;curname[20]:=84;entername(52);namelength:=20;
...
curname[19]:=71;curname[20]:=50;entername(40);namelength:=13;
...
curname[20]:=51;entername(41);namelength:=13;curname[8]:=66;
...
entername(42);namelength:=13;curname[8]:=66;curname[9]:=73;
curname[10]:=71;curname[11]:=79;curname[12]:=80;curname[13]:=83;
curname[14]:=80;curname[15]:=65;curname[16]:=67;curname[17]:=73;
curname[18]:=78;curname[19]:=71;curname[20]:=53;entername(43);{:57};end;
...
curname[20]:=88;entername(90);{:56};end;procedure nameenter;
begin{55:}equiv[0]:=0;namelength:=8;curname[13]:=67;curname[14]:=72;
...
showerrorcontext;end;skiptoendofitem;
end else begin case curcode of 100:{112:}begin while curchar=32 do
getnext;if curchar=66 then begin charremainder[256]:=nl;
repeat getnext until(curchar=40)or(curchar=41);
...
if minnl<=nl then minnl:=nl+1;lkstepended:=false;end{:112};
101:{114:}if not lkstepended then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'STOP must follow LIG or KRN');
showerrorcontext;end else begin ligkern[nl-1].b0:=128;
lkstepended:=false;end{:114};
102:{115:}if not lkstepended then begin if charsonline>0 then writeln(
...
showerrorcontext;end else nl:=nl+1;lkstepended:=true;end{:116};end;
finishtheproperty;end;
...
if verbose then{137:}begin if charsonline=8 then begin writeln(stderr,
' ');charsonline:=1;
...
charremainder[c]:=getbyte;end;66:readpacket(c);
67:{120:}begin if ne=256 then begin if charsonline>0 then writeln(stderr
,' ');write(stderr,'At most 256 VARCHAR specs are allowed');
showerrorcontext;end else begin checktag(c);chartag[c]:=3;
charremainder[c]:=ne;exten[ne]:=zerobytes;
while level=2 do begin while curchar=32 do getnext;
...
end{:121}else if curchar=41 then skiptoendofitem else junkerror;end;
ne:=ne+1;begin loc:=loc-1;level:=level+1;curchar:=41;end;end;end{:120};
end;finishtheproperty;end;
end{:119}else if curchar=41 then skiptoendofitem else junkerror;end;
...
showerrorcontext;end;skiptoendofitem;
end else begin{95:}case curcode of 1:begin checksumspecified:=true;
readfourbytes(0);end;2:{98:}begin nextd:=getfix;
...
7:{100:}begin while curchar=32 do getnext;
...
headerptr:=headerptr+1;end;readfourbytes(4*c);end;end{:101};
9:{102:}begin while level=1 do begin while curchar=32 do getnext;
...
end{:103}else if curchar=41 then skiptoendofitem else junkerror;end;
...
end else vtitlelength:=vfptr-vtitlestart;end;
13:{104:}begin getfourbytes;fontnumber[fontptr]:=curbytes;curfont:=0;
while(fontnumber[curfont].b3<>fontnumber[fontptr].b3)or(fontnumber[
curfont].b2<>fontnumber[fontptr].b2)or(fontnumber[curfont].b1<>
fontnumber[fontptr].b1)or(fontnumber[curfont].b0<>fontnumber[fontptr].b0
)do curfont:=curfont+1;
if curfont=fontptr then if fontptr<256 then{105:}begin fontptr:=fontptr
+1;fnamestart[curfont]:=vfsize;fnamelength[curfont]:=4;
fareastart[curfont]:=vfsize;farealength[curfont]:=0;
fontchecksum[curfont]:=zerobytes;fontat[curfont]:=1048576;
fontdsize[curfont]:=10485760;
end{:105}else begin if charsonline>0 then writeln(stderr,' ');
write(stderr,'I can handle only 256 different mapfonts');
showerrorcontext;end;
...
showerrorcontext;end;skiptoendofitem;
end else begin case curcode of 20:{107:}begin fnamestart[curfont]:=vfptr
;copytoendofitem;
...
24:fontdsize[curfont]:=getfix;end;finishtheproperty;end;
end{:106}else if curchar=41 then skiptoendofitem else junkerror;end;
begin loc:=loc-1;level:=level+1;curchar:=41;end;end{:104};
14:readcharinfo;end{:95};finishtheproperty;end;
end{:94}else if(curchar=41)and not inputhasended then begin begin if
charsonline>0 then writeln(stderr,' ');
...
begin{139:}if nl>0 then{145:}begin if charremainder[256]<32767 then
...
sevenunsafe:=false;
for c:=0 to 255 do if charwd[c]<>0 then{140:}case chartag[c]of 0:;
1:{149:}begin ligptr:=charremainder[c];
...
writeln(stderr,' had no CHARACTER spec.');end;end;
3:{141:}begin if exten[charremainder[c]].b0>0 then begin g:=exten[
...
writeln(stderr,' had no CHARACTER spec.');end;end;end{:141};end{:140};
if charremainder[256]<32767 then begin c:=256;
{149:}begin ligptr:=charremainder[c];
repeat if hashinput(ligptr,c)then begin if ligkern[ligptr].b2<128 then
begin if ligkern[ligptr].b1<>bchar then begin g:=ligkern[ligptr].b1;
if charwd[g]=0 then begin charwd[g]:=sortin(1,0);
write(stderr,'LIG character examined by',' ');printoctal(c);
...
{154:}if hashptr<hashsize then for hh:=1 to hashptr do begin tt:=
hashlist[hh];
...
{155:}if nl>0 then for ligptr:=0 to nl-1 do if ligkern[ligptr].b2<128
...
printoctal(c);writeln(stderr,'!');end;end;begin c:=ligkern[ligptr].b3;
if charwd[c]=0 then if c<>bchar then begin ligkern[ligptr].b3:=0;
if charwd[0]=0 then charwd[0]:=sortin(1,0);
write(stderr,'Unused ','LIG step',' refers to nonexistent character ');
printoctal(c);writeln(stderr,'!');end;end;end;
...
printoctal(c);writeln(stderr,'!');end;end;
...
);printoctal(c);writeln(stderr,'!');end;end;end{:155};
for c:=0 to 255 do{142:}if chartag[c]=2 then begin g:=charremainder[c];
...
printoctal(c);writeln(stderr,'.');end;end{:142};
{144:}delta:=shorten(1,255);setindices(1,delta);
...
writeln(stderr,' units.');end;{:144}{:139}end;procedure vfoutput;
var c:byte;curfont:0..256;k:integer;begin{177:}putbyte(247,vffile);
putbyte(202,vffile);putbyte(vtitlelength,vffile);
...
readinput;if verbose then writeln(stderr,'.');corrandcheck;
{157:}{159:}lh:=headerptr div 4;notfound:=true;bc:=0;
...
putbyte((np)mod 256,tfmfile);{:160};
{162:}if not checksumspecified then{163:}begin curbytes.b0:=bc;
...
for j:=0 to headerptr-1 do putbyte(headerbytes[j],tfmfile);{:162};
{164:}indexvar[0]:=0;
...
putbyte(charremainder[c],tfmfile);end{:164};
{166:}for q:=1 to 4 do begin putbyte(0,tfmfile);putbyte(0,tfmfile);
putbyte(0,tfmfile);putbyte(0,tfmfile);p:=link[q];
while p>0 do begin outscaled(memory[p]);p:=link[p];end;end;{:166};
{171:}if extralocneeded then begin putbyte(255,tfmfile);
...
{172:}if ne>0 then for c:=0 to ne-1 do begin putbyte(exten[c].b0,tfmfile
);putbyte(exten[c].b1,tfmfile);putbyte(exten[c].b2,tfmfile);
putbyte(exten[c].b3,tfmfile);end;{:172};
{173:}for parptr:=1 to np do begin if parptr=1 then{174:}begin if param[
...
end{:173}{:157};vfoutput;if not perfect then uexit(1);end.{:181}
