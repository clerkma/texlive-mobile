...
kanjiindex:integer;kanjitype:array[0..7237]of-1..255;
...
end else if getoptreturnval=63 then begin usage('ptftopl');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(PTFTOPLHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is pTFtoPL, Version 3.3-p1.7',nil,'D.E. Knuth'
,nil);
...
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'ptftopl'
,': Need one or two file arguments.');usage('ptftopl');end;
tfmname:=cmdline(optind);end;{:100}procedure initialize;var k:integer;
begin kpsesetprogramname(argv[0],'ptftopl');
...
if verbose then begin write(stderr,'This is pTFtoPL, Version 3.3-p1.7');
...
write(plfile,'J ');dig[0]:=Hi(cx)div 16;dig[1]:=Hi(cx)mod 16;
...
end else begin cx:=toBUFF(fromDVI(jiscode));write(plfile,xchr[Hi(cx)]);
write(plfile,xchr[Lo(cx)]);end;end;
...
var firstbyte,secondbyte:integer;begin firstbyte:=cx div 256-33;
secondbyte:=cx mod 256-33;
if firstbyte<8 then jistoindex:=firstbyte*94+secondbyte else jistoindex
:=(firstbyte-7)*94+secondbyte;end{:123};begin initialize;
...
end;for kanjiindex:=0 to 7237 do kanjitype[kanjiindex]:=-1;
...
for kanjiindex:=0 to 7237 do if kanjitype[kanjiindex]=typenum then begin
if(typecount mod 10)=0 then outln else write(plfile,' ');
