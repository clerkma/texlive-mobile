{2:}
 @define var tfm;
program VFtoVP(vffile,tfmfile,vplfile,output);label{3:}9999;
{:3}const{4:}vfsize=100000;maxfonts=300;ligsize=32510;hashsize=32579;
maxstack=100;{:4}{144:}charcodeascii=0;charcodeoctal=1;
charcodedefault=2;{:144}type{5:}byte=0..255;{:5}{22:}indextype=integer;
{:22}{143:}charcodeformattype=charcodeascii..charcodedefault;
{:143}var{7:}vffile:packed file of byte;
{:7}{10:}tfmfile:packed file of byte;
{:10}{12:}lf,lh,bc,ec,nw,nh,nd,ni,nl,nk,ne,np:0..32767;
{:12}{20:}vplfile:text;
{:20}{23:}#define tfm (tfmfilearray + 1001);
tfmfilearray:^byte;
{:23}{26:}charbase,widthbase,heightbase,depthbase,italicbase,ligkernbase
,kernbase,extenbase,parambase:integer;{:26}{29:}fonttype:0..2;
{:29}{30:}vf:array[0..vfsize]of byte;
fontnumber:array[0..maxfonts]of integer;
...
vfcount:integer;{:30}{37:}a:integer;l:integer;curname:^char;
b0,b1,b2,b3:byte;fontlh:0..32767;fontbc,fontec:0..32767;
{:37}{42:}defaultdirectory:packed array[1..9]of char;
{:42}{50:}ASCII04,ASCII10,ASCII14:constcstring;
xchr:packed array[0..255]of char;
MBLstring,RIstring,RCEstring:constcstring;
{:50}{52:}dig:array[0..11]of 0..9;{:52}{55:}level:0..5;
{:55}{68:}charsonline:0..8;perfect:boolean;{:68}{70:}i:0..32767;
c:0..256;d:0..3;k:indextype;r:0..65535;
{:70}{86:}labeltable:array[0..258]of record cc:0..256;rr:0..ligsize;end;
labelptr:0..257;sortptr:0..257;boundarychar:0..256;bcharlabel:0..32767;
{:86}{88:}activity:array[0..ligsize]of 0..2;ai,acti:0..ligsize;
{:88}{112:}hash:array[0..hashsize]of 0..66048;
...
h,hh:0..hashsize;xligcycle,yligcycle:0..256;{:112}{124:}top:0..maxstack;
wstack,xstack,ystack,zstack:array[0..maxstack]of integer;
vflimit:0..vfsize;o:byte;{:124}{140:}verbose:cinttype;
{:140}{145:}charcodeformat:charcodeformattype;
{:145}{148:}tfmname:cstring;vfname,vplname:constcstring;
{:148}{136:}procedure parsearguments;const noptions=4;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{141:}verbose:=false;{:141}{146:}charcodeformat:=charcodedefault;
{:146};{137:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:137}{138:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:138}{139:}longoptions[currentoption].name:='verbose';
...
stringcast(optarg),'.');end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)and(optind+3<>argc)then begin
writeln(stderr,'vftovp',': Need one to three file arguments.');
usage('vftovp');end;vfname:=cmdline(optind);
if optind+2<=argc then begin tfmname:=cmdline(optind+1);
end else begin tfmname:=basenamechangesuffix(vfname,'.vf','.tfm');end;
end;{:136}procedure initialize;var k:integer;
begin kpsesetprogramname(argv[0],'vftovp');
kpseinitprog('VFTOVP',0,nil,nil);tfmfilearray:=xmallocarray(byte,1002);
parsearguments;{11:}vffile:=kpseopenfile(vfname,kpsevfformat);
tfmfile:=kpseopenfile(tfmname,kpsetfmformat);
if verbose then begin write(stderr,'This is VFtoVP, Version 1.4');
writeln(stderr,versionstring);end;
{:11}{21:}if optind+3>argc then begin vplfile:=stdout;
end else begin vplname:=extendfilename(cmdline(optind+2),'vpl');
rewrite(vplfile,vplname);end;
{:21}{51:}ASCII04:='  !"#$%&''()*+,-./0123456789:;<=>?';
...
RIstring:=' RI ';RCEstring:=' RCE';{:51}{56:}level:=0;
{:56}{69:}charsonline:=0;perfect:=true;{:69}{87:}boundarychar:=256;
bcharlabel:=32767;labelptr:=0;labeltable[0].rr:=0;{:87}end;
{:2}{38:}procedure readtfmword;
...
if eof(tfmfile)then b3:=0 else read(tfmfile,b3);end;
{:38}{46:}function vfread(k:integer):integer;var b:byte;a:integer;
...
a:=256*a+b;k:=k-1;end;vfread:=a;end;
{:46}{48:}function tfmwidth(c:byte):integer;var a:integer;k:indextype;
begin k:=4*(widthbase+tfm[4*(charbase+c)]);a:=tfm[k];
if a>=128 then a:=a-256;
tfmwidth:=((256*a+tfm[k+1])*256+tfm[k+2])*256+tfm[k+3];end;
{:48}{53:}procedure outdigs(j:integer);begin repeat j:=j-1;
write(vplfile,dig[j]:1);until j=0;end;procedure printdigs(j:integer);
begin repeat j:=j-1;write(stderr,dig[j]:1);until j=0;end;
{:53}{54:}procedure printoctal(c:byte);var j:0..2;
begin write(stderr,'''');for j:=0 to 2 do begin dig[j]:=c mod 8;
c:=c div 8;end;printdigs(3);end;{:54}{57:}procedure outln;var l:0..5;
begin writeln(vplfile);for l:=1 to level do write(vplfile,'   ');end;
procedure left;begin level:=level+1;write(vplfile,'(');end;
procedure right;begin level:=level-1;write(vplfile,')');outln;end;
{:57}{58:}procedure outBCPL(k:indextype);var l:0..39;
begin write(vplfile,' ');l:=tfm[k];while l>0 do begin k:=k+1;l:=l-1;
write(vplfile,xchr[tfm[k]]);end;end;
{:58}{59:}procedure outoctal(k,l:indextype);var a:0..1023;b:0..32;
j:0..11;begin write(vplfile,' O ');a:=0;b:=0;j:=0;
while l>0 do{60:}begin l:=l-1;
if tfm[k+l]<>0 then begin while b>2 do begin dig[j]:=a mod 8;a:=a div 8;
b:=b-3;j:=j+1;end;case b of 0:a:=tfm[k+l];1:a:=a+2*tfm[k+l];
2:a:=a+4*tfm[k+l];end;end;b:=b+8;end{:60};
while(a>0)or(j=0)do begin dig[j]:=a mod 8;a:=a div 8;j:=j+1;end;
outdigs(j);end;{:59}{61:}procedure outchar(c:byte);
...
126)and(c<>40)and(c<>41)then write(vplfile,' C ',xchr[c])else if((c>=48)
and(c<=57))or((c>=65)and(c<=90))or((c>=97)and(c<=122))then write(vplfile
,' C ',xchr[c])else begin tfm[0]:=c;outoctal(0,1);end;end;
{:61}{62:}procedure outface(k:indextype);var s:0..1;b:0..8;
...
end;end;{:62}{63:}procedure outfix(k:indextype);var a:0..4095;f:integer;
j:0..12;delta:integer;begin write(vplfile,' R ');
a:=(tfm[k]*16)+(tfm[k+1]div 16);
f:=((tfm[k+1]mod 16)*256+tfm[k+2])*256+tfm[k+3];
if a>2047 then{66:}begin write(vplfile,'-');a:=4096-a;
if f>0 then begin f:=1048576-f;a:=a-1;end;end{:66};{64:}begin j:=0;
repeat dig[j]:=a mod 10;a:=a div 10;j:=j+1;until a=0;outdigs(j);
end{:64};{65:}begin write(vplfile,'.');f:=10*f+5;delta:=10;
repeat if delta>1048576 then f:=f+524288-(delta div 2);
write(vplfile,f div 1048576:1);f:=10*(f mod 1048576);delta:=delta*10;
until f<=delta;end;{:65};end;
{:63}{75:}procedure checkBCPL(k,l:indextype);var j:indextype;c:byte;
...
{:75}{115:}procedure hashinput;label 10;var cc:0..3;zz:0..255;y:0..255;
key:integer;t:integer;begin if hashptr=hashsize then goto 10;
{116:}k:=4*(ligkernbase+(i));y:=tfm[k+1];t:=tfm[k+2];cc:=0;zz:=tfm[k+3];
if t>=128 then zz:=y else begin case t of 0,6:;5,11:zz:=y;1,7:cc:=1;
2:cc:=2;3:cc:=3;end;end{:116};key:=256*c+y+1;h:=(1009*key)mod hashsize;
while hash[h]>0 do begin if hash[h]<=key then begin if hash[h]=key then
goto 10;t:=hash[h];hash[h]:=key;key:=t;t:=classvar[h];classvar[h]:=cc;
cc:=t;t:=ligz[h];ligz[h]:=zz;zz:=t;end;
if h>0 then h:=h-1 else h:=hashsize;end;hash[h]:=key;classvar[h]:=cc;
ligz[h]:=zz;hashptr:=hashptr+1;hashlist[hashptr]:=h;10:end;
{:115}{117:}ifdef('notdef')function ligf(h,x,y:indextype):indextype;
...
if hash[h]<key then eval:=y else eval:=ligf(h,x,y);end;
{:117}{118:}function ligf(h,x,y:indextype):indextype;
...
ligf:=ligz[h];end;
{:118}{119:}function stringbalance(k,l:integer):boolean;label 45,10;
...
45:stringbalance:=false;10:end;
{:119}{121:}procedure outasfix(x:integer);var k:1..3;
begin if abs(x)>=16777216 then begin begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ',
'Oversize dimension has been reset to zero.');end;x:=0;end;
if x>=0 then tfm[28]:=0 else begin tfm[28]:=255;x:=x+16777216;end;
for k:=3 downto 1 do begin tfm[28+k]:=x mod 256;x:=x div 256;end;
outfix(28);end;{:121}{126:}function getbytes(k:integer;
issigned:boolean):integer;var a:integer;
begin if vfptr+k>vflimit then begin begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Packet ended prematurely');end;
...
getbytes:=a;end;{:126}{132:}function vfinput:boolean;
...
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a VF?');
uexit(1);end;{32:}if eof(vffile)then begin writeln(stderr,
...
if count<>8 then begin writeln(stderr,
'Check sum and/or design size mismatch.');
writeln(stderr,'Data from TFM file will be assumed correct.');end{:32};
{33:}for k:=0 to 255 do packetstart[k]:=vfsize;fontptr:=0;
packetfound:=false;fontstart[0]:=vfptr;
...
if tempbyte<>248 then if tempbyte>242 then{35:}begin if packetfound or(
tempbyte>=247)then begin writeln(stderr,'Illegal byte ',tempbyte:1,
' at beginning of character packet!');
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a VF?');
...
if vf[vfptr-10]>0 then begin writeln(stderr,
...
vfptr:=vfptr+a+l;
if verbose then begin{36:}write(stderr,'MAPFONT ',fontptr:1,': ');
...
2,2);writeln(stderr,'pt'){:36};end;{39:}fontchars[fontptr]:=vfptr;
{44:}{:44}{45:}r:=vfptr-(fontstart[fontptr]+14);
curname:=xmallocarray(char,r);
for k:=(fontstart[fontptr]+14)to vfptr do begin curname[k-(fontstart[
fontptr]+14)]:=xchr[vf[k]];end;curname[r]:=0;{:45};
tfmname:=kpsefindtfm(curname);
if not tfmname then writeln(stderr,'---not loaded, TFM file ',stringcast
(curname),' can''t be opened!')else begin resetbin(tfmfile,tfmname);
fontbc:=0;fontec:=256;readtfmword;
if b2<128 then begin fontlh:=b2*256+b3;readtfmword;
if(b0<128)and(b2<128)then begin fontbc:=b0*256+b1;fontec:=b2*256+b3;end;
end;if fontbc<=fontec then if fontec>255 then writeln(stderr,
'---not loaded, bad TFM file ',stringcast(tfmname),'!')else begin for k
:=0 to 3+fontlh do begin readtfmword;
if k=4 then{40:}if b0+b1+b2+b3>0 then if(b0<>vf[fontstart[fontptr]])or(
b1<>vf[fontstart[fontptr]+1])or(b2<>vf[fontstart[fontptr]+2])or(b3<>vf[
fontstart[fontptr]+3])then begin if verbose then writeln(stderr,
...
if k=5 then{41:}if(b0<>vf[fontstart[fontptr]+8])or(b1<>vf[fontstart[
fontptr]+9])or(b2<>vf[fontstart[fontptr]+10])or(b3<>vf[fontstart[fontptr
]+11])then begin writeln(stderr,
...
if vfptr=vfsize then begin writeln(stderr,'I''m out of VF memory!');
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a VF?');
uexit(1);end;end;end;end;
if eof(tfmfile)then writeln(stderr,'---trouble is brewing, TFM file ',
stringcast(tfmname),' ended too soon!');free(tfmname);end;free(curname);
vfptr:=vfptr+1{:39};fontptr:=fontptr+1;fontstart[fontptr]:=vfptr;
end{:35}else{47:}begin if tempbyte=242 then begin pl:=vfread(4);
c:=vfread(4);count:=vfread(4);end else begin pl:=tempbyte;c:=vfread(1);
count:=vfread(3);end;
if((c<bc)or(c>ec)or(tfm[4*(charbase+c)]=0))then begin writeln(stderr,
...
if count<>tfmwidth(c)then writeln(stderr,
'Incorrect TFM width for character ',c:1,' in VF file');
...
packetend[c]:=vfptr-1;packetfound:=true;end{:47};end;
until tempbyte=248{:33};
{34:}while(tempbyte=248)and not eof(vffile)do begin read(vffile,tempbyte
...
'VF data not a multiple of 4 bytes'){:34}{:31};vfinput:=true;end;
function organize:boolean;var tfmptr:indextype;
begin{24:}read(tfmfile,tfm[0]);if tfm[0]>127 then begin writeln(stderr,
...
writeln(stderr,'but I''ll proceed as if it weren''t there.');end{:24};
{25:}begin tfmptr:=2;begin if tfm[tfmptr]>127 then begin writeln(stderr,
...
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a TFM?')
;uexit(1);end;if(bc>ec+1)or(ec>255)then begin writeln(stderr,
...
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a TFM?')
;uexit(1);end;if ne>256 then begin writeln(stderr,'There are ',ne:1,
' extensible recipes!');
writeln(stderr,'Sorry, but I can''t go on; are you sure this is a TFM?')
;uexit(1);end;
...
organize:=vfinput;end;{:132}{133:}procedure dosimplethings;
var i:0..32767;f:0..vfsize;k:integer;
begin{120:}if stringbalance(0,fontstart[0])then begin left;
...
charsonline:=0;
writeln(stderr,'Bad VF file: ','Title is not a balanced ASCII string');
end{:120};{71:}begin fonttype:=0;
if lh>=12 then begin{76:}begin checkBCPL(32,40);
...
tfm[42]=69)and(tfm[43]=88)then fonttype:=2;end;end{:76};
if lh>=17 then begin{78:}left;write(vplfile,'FAMILY');checkBCPL(72,20);
outBCPL(72);right{:78};if lh>=18 then{79:}begin left;
write(vplfile,'FACE');outface(95);right;for i:=18 to lh-1 do begin left;
write(vplfile,'HEADER D ',i:1);outoctal(24+4*i,4);right;end;end{:79};
end;{77:}left;write(vplfile,'CODINGSCHEME');outBCPL(32);right{:77};end;
{74:}left;write(vplfile,'DESIGNSIZE');
...
outln{:74};{72:}left;write(vplfile,'CHECKSUM');outoctal(24,4);
right{:72};{80:}if(lh>17)and(tfm[92]>127)then begin left;
write(vplfile,'SEVENBITSAFEFLAG TRUE');right;end{:80};end{:71};
{81:}if np>0 then begin left;write(vplfile,'FONTDIMEN');outln;
for i:=1 to np do{83:}begin left;
...
{84:}if i<=7 then case i of 2:write(vplfile,'SPACE');
...
'BIGOPSPACING',i-8:1)else write(vplfile,'PARAMETER D ',i:1){:84};end;
outfix(4*(parambase+i));right;end{:83};right;end;
{82:}if(fonttype=1)and(np<>22)then writeln(stderr,
...
' not 13).'){:82};{:81};{122:}for f:=0 to fontptr-1 do begin left;
write(vplfile,'MAPFONT D ',f:1);outln;{123:}a:=vf[fontstart[f]+12];
l:=vf[fontstart[f]+13];
...
writeln(stderr,'Bad VF file: ','Improper font area will be ignored');
...
false;if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Improper font name will be ignored');
end else begin left;write(vplfile,'FONTNAME ');
for k:=fontstart[f]+14+a to fontstart[f]+a+l+13 do write(vplfile,xchr[vf
[k]]);right;end{:123};for k:=0 to 11 do tfm[k]:=vf[fontstart[f]+k];
...
outfix(8);right;right;end{:122};
{85:}if(tfm[4*widthbase]>0)or(tfm[4*widthbase+1]>0)or(tfm[4*widthbase+2]
...
writeln(stderr,'I have set it to zero.');end;{:85};end;
{:133}{134:}function domap(c:byte):boolean;var k:integer;f:0..vfsize;
begin{125:}if packetstart[c]=vfsize then begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Missing packet for character ',c:1);
...
vfptr:=vfptr+1;
if(o<=131)or((o>=133)and(o<=136))then{130:}begin if o>=128 then if o>=
133 then k:=getbytes(o-132,false)else k:=getbytes(o-127,false)else k:=o;
c:=k;if(k<0)or(k>255)then begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Character ',k:1,
' is out of range and will be ignored');
end else if f=fontptr then begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Character ',c:1,
...
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Character ',c:1,' in font ',f:1,
' will be ignored');
...
end{:130}else case o of{127:}138:;
...
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','More pops than pushes!');
...
end;{:127}{128:}143,144,145,146:begin write(vplfile,'(MOVERIGHT');
...
write(vplfile,')');outln;end;
{:128}{129:}171,172,173,174,175,176,177,178,179,180,181,182,183,184,185,
...
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Undeclared font selected');
end else begin write(vplfile,'(SELECTFONT D ',f:1,')');outln;end;end;
{:129}{131:}239,240,241,242:begin k:=getbytes(o-238,false);
if k<0 then begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','String of negative length!');
...
end;end;right;end;end;
{:131}139,140,243,244,245,246,247,248,249,250,251,252,253,254,255:begin
perfect:=false;if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','Illegal DVI code ',o:1,
' will be ignored');end;end;end;
if top>0 then begin begin perfect:=false;
if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad VF file: ','More pushes than pops!');end;
repeat write(vplfile,'(POP)');top:=top-1;until top=0;end;right;
end{:125};domap:=true;end;function docharacters:boolean;label 9999,10;
var c:byte;k:indextype;ai:0..ligsize;begin{101:}sortptr:=0;
...
outchar(c);outln;{102:}begin left;write(vplfile,'CHARWD');
...
end else outfix(4*(widthbase+tfm[4*(charbase+c)]));right;end{:102};
if(tfm[4*(charbase+c)+1]div 16)>0 then{103:}if(tfm[4*(charbase+c)+1]div
...
write(vplfile,'CHARHT');
outfix(4*(heightbase+(tfm[4*(charbase+c)+1]div 16)));right;end{:103};
if(tfm[4*(charbase+c)+1]mod 16)>0 then{104:}if(tfm[4*(charbase+c)+1]mod
16)>=nd then begin perfect:=false;writeln(stderr,' ');
write(stderr,'Depth',' index for character ');printoctal(c);
writeln(stderr,' is too large;');
writeln(stderr,'so I reset it to zero.');end else begin left;
write(vplfile,'CHARDP');
outfix(4*(depthbase+(tfm[4*(charbase+c)+1]mod 16)));right;end{:104};
if(tfm[4*(charbase+c)+2]div 4)>0 then{105:}if(tfm[4*(charbase+c)+2]div 4
...
outfix(4*(italicbase+(tfm[4*(charbase+c)+2]div 4)));right;end{:105};
case(tfm[4*(charbase+c)+2]mod 4)of 0:;1:{106:}begin left;
write(vplfile,'COMMENT');outln;i:=tfm[4*(charbase+c)+3];
r:=4*(ligkernbase+(i));if tfm[r]>128 then i:=256*tfm[r+2]+tfm[r+3];
repeat{97:}begin k:=4*(ligkernbase+(i));
...
end else if tfm[k+2]>=128 then{99:}begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)
...
write(vplfile,' R 0.0');end else outfix(4*(kernbase+r));right;
end{:99}else{100:}begin if((tfm[k+1]<bc)or(tfm[k+1]>ec)or(tfm[4*(
...
r:=r-4;end;outchar(tfm[k+1]);outchar(tfm[k+3]);right;end{:100};
if tfm[k]>0 then if level=1 then{98:}begin if tfm[k]>=128 then write(
vplfile,'(STOP)')else begin count:=0;
for ai:=i+1 to i+tfm[k]do if activity[ai]=2 then count:=count+1;
write(vplfile,'(SKIP D ',count:1,')');end;outln;end{:98};end{:97};
if tfm[k]>=128 then i:=nl else i:=i+1+tfm[k];until i>=nl;right;
end{:106};2:{107:}begin r:=tfm[4*(charbase+c)+3];
...
outchar(tfm[4*(charbase+c)+3]);right;end;end;end{:107};
3:{108:}if tfm[4*(charbase+c)+3]>=ne then begin begin perfect:=false;
writeln(stderr,' ');write(stderr,'Extensible',' index for character ');
printoctal(c);writeln(stderr,' is too large;');
writeln(stderr,'so I reset it to zero.');end;
tfm[4*(charbase+c)+2]:=4*(tfm[4*(charbase+c)+2]div 4)+0;
end else begin left;write(vplfile,'VARCHAR');outln;
{109:}for k:=0 to 3 do if(k=3)or(tfm[4*(extenbase+tfm[4*(charbase+c)+3])
+k]>0)then begin left;case k of 0:write(vplfile,'TOP');
...
[4*(charbase+c)+3])+k]);right;end{:109};right;end{:108};end;
if not domap(c)then goto 9999;right;end{:101};docharacters:=true;
goto 10;9999:docharacters:=false;10:end;{:134}{135:}begin initialize;
if not organize then goto 9999;dosimplethings;
{89:}if nl>0 then begin for ai:=0 to nl-1 do activity[ai]:=0;
{92:}if tfm[4*(ligkernbase+(0))]=255 then begin left;
...
end;activity[nl-1]:=1;end{:92};end;
{90:}for c:=bc to ec do if(tfm[4*(charbase+c)+2]mod 4)=1 then begin r:=
tfm[4*(charbase+c)+3];
...
printoctal(c);writeln(stderr,' is too large;');
writeln(stderr,'so I removed it.');
tfm[4*(charbase+c)+2]:=4*(tfm[4*(charbase+c)+2]div 4)+0;
end else{91:}begin sortptr:=labelptr;
...
end{:91};end;labeltable[labelptr+1].rr:=ligsize;{:90};
if nl>0 then begin left;write(vplfile,'LIGTABLE');outln;
{93:}for ai:=0 to nl-1 do if activity[ai]=2 then begin r:=tfm[4*(
...
tfm[4*(ligkernbase+(ai))]:=128;end else activity[r]:=2;end;end{:93};
{94:}sortptr:=1;
for acti:=0 to nl-1 do if activity[acti]<>1 then begin i:=acti;
{96:}if activity[i]=0 then begin if level=1 then begin left;
write(vplfile,'COMMENT THIS PART OF THE PROGRAM IS NEVER USED!');outln;
end end else if level=2 then right{:96};
{95:}while i=labeltable[sortptr].rr do begin left;
write(vplfile,'LABEL');
if labeltable[sortptr].cc=256 then write(vplfile,' BOUNDARYCHAR')else
outchar(labeltable[sortptr].cc);right;sortptr:=sortptr+1;end{:95};
{97:}begin k:=4*(ligkernbase+(i));
if tfm[k]>128 then begin if 256*tfm[k+2]+tfm[k+3]>=nl then begin perfect
:=false;if charsonline>0 then writeln(stderr,' ');charsonline:=0;
writeln(stderr,'Bad TFM file: ',
'Ligature unconditional stop command address is too big.');end;
...
vplfile,'(STOP)')else begin count:=0;
for ai:=i+1 to i+tfm[k]do if activity[ai]=2 then count:=count+1;
write(vplfile,'(SKIP D ',count:1,')');end;outln;end{:98};end{:97};end;
if level=2 then right{:94};right;{113:}hashptr:=0;yligcycle:=256;
...
tfm[4*(ligkernbase+(i))+3];{114:}repeat hashinput;
k:=tfm[4*(ligkernbase+(i))];if k>=128 then i:=nl else i:=i+1+k;
until i>=nl{:114};end;if bcharlabel<nl then begin c:=256;i:=bcharlabel;
{114:}repeat hashinput;k:=tfm[4*(ligkernbase+(i))];
if k>=128 then i:=nl else i:=i+1+k;until i>=nl{:114};end;
...
if classvar[r]>0 then r:=ligf(r,(hash[r]-1)div 256,(hash[r]-1)mod 256);
...
{110:}if ne>0 then for c:=0 to ne-1 do for d:=0 to 3 do begin k:=4*(
extenbase+c)+d;
...
if d<3 then tfm[k]:=0;end;end;end{:110};
