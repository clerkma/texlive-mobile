{4:}program GFtoPK(gffile,pkfile,output);const{6:}linelength=79;
MAXROW=16000;{:6}type{9:}ASCIIcode=32..126;
{:9}{10:}textfile=packed file of 0..255;{:10}{37:}eightbits=0..255;
bytefile=packed file of eightbits;
{:37}var{11:}xord:array[0..255]of ASCIIcode;xchr:array[0..255]of 0..255;
{:11}{38:}gffile:bytefile;pkfile:bytefile;{:38}{41:}pkloc:integer;
gfloc:integer;pkopen:boolean;{:41}{46:}bitweight:integer;
outputbyte:integer;{:46}{48:}gflen:integer;
{:48}{49:}tfmwidth:array[0..255]of integer;
dx,dy:array[0..255]of integer;status:array[0..255]of 0..2;row:^integer;
maxrow:integer;{:49}{56:}gfch:integer;gfchmod256:integer;
predpkloc:integer;maxn,minn:integer;maxm,minm:integer;rowptr:integer;
{:56}{79:}power:array[0..8]of integer;
{:79}{83:}comment:packed array[1..0]of 0..255;
{:83}{87:}checksum:integer;designsize:integer;hmag:integer;i:integer;
{:87}{92:}verbose:cinttype;{:92}{95:}gfname,pkname:constcstring;
{:95}{88:}procedure parsearguments;const noptions=3;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{93:}verbose:=false;{:93};{89:}currentoption:=0;
...
{:89}{90:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:90}{91:}longoptions[currentoption].name:='verbose';
...
{:91}{94:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:94};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('gftopk');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(GFTOPKHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is GFtoPK, Version 2.4',nil,'Tomas Rokicki',
nil);end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'gftopk',
': Need one or two file arguments.');usage('gftopk');end;
gfname:=cmdline(optind);
if optind+2=argc then begin pkname:=cmdline(optind+1);
end else begin pkname:=basenamechangesuffix(gfname,'gf','pk');end;end;
{:88}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'gftopk');
kpseinitprog('GFTOPK',0,nil,nil);parsearguments;
if verbose then write(stdout,'This is GFtoPK, Version 2.4');
if verbose then writeln(stdout,versionstring);
...
{:42}{50:}row:=xmallocarray(integer,MAXROW);maxrow:=MAXROW;
for i:=0 to 255 do status[i]:=0;{:50}{80:}power[0]:=1;
for i:=1 to 8 do power[i]:=power[i-1]+power[i-1];{:80}end;
{:4}{30:}{function pkpackednum:integer;var i,j:integer;begin i:=getnyb;
...
{:30}{39:}procedure opengffile;
begin gffile:=kpseopenfile(gfname,kpsegfformat);gfloc:=0;end;
{:39}{40:}procedure openpkfile;begin rewritebin(pkfile,pkname);pkloc:=0;
pkopen:=true;end;{:40}{43:}function gfbyte:integer;var b:eightbits;
begin if eof(gffile)then begin writeln(stderr,'Bad GF file: ',
'Unexpected end of file!','!');uexit(1);end else begin read(gffile,b);
gfbyte:=b;end;gfloc:=gfloc+1;end;function gfsignedquad:integer;
var a,b,c,d:eightbits;begin read(gffile,a);read(gffile,b);
read(gffile,c);read(gffile,d);
if a<128 then gfsignedquad:=((a*256+b)*256+c)*256+d else gfsignedquad:=(
((a-256)*256+b)*256+c)*256+d;gfloc:=gfloc+4;end;
{:43}{45:}procedure pkhalfword(a:integer);begin if a<0 then a:=a+65536;
...
bitweight:=16;end;end;{:45}{47:}function gflength:integer;
begin xfseek(gffile,0,2,gfname);gflength:=xftell(gffile,gfname);end;
procedure movetobyte(n:integer);begin xfseek(gffile,n,0,gfname);end;
{:47}{52:}{63:}procedure packandsendcharacter;var i,j,k:integer;
{66:}extra:integer;putptr:integer;repeatflag:integer;hbit:integer;
buff:integer;{:66}{71:}dynf:integer;height,width:integer;
...
{:71}{78:}compsize:integer;count:integer;pbit:integer;ron,son:boolean;
rcount,scount:integer;ri,si:integer;max2:integer;{:78}begin{64:}i:=2;
...
xoffset:=0;yoffset:=0;;end{:64};{65:}putptr:=0;rowptr:=2;repeatflag:=0;
state:=true;buff:=0;while row[rowptr]=(-99999)do rowptr:=rowptr+1;
while row[rowptr]<>(-99998)do begin{67:}i:=rowptr;
...
repeatflag+1;rowptr:=i+1;end;i:=i+1;j:=j+1;end;end{:67};
{68:}if row[rowptr]<>(-99999)then row[rowptr]:=row[rowptr]-extra;
...
end;i:=i+1;end{:70};bcompsize:=compsize;dynf:=0;
...
:=(height*width+7)div 8;dynf:=14;end;;{72:}flagbyte:=dynf*16;
...
{73:}begin flagbyte:=flagbyte+7;begin putbyte(flagbyte,pkfile);
...
pkword(height);pkword(xoffset);pkword(yoffset);
end{:73}else if(dx[gfch]>16777215)or(width>255)or(height>255)or(xoffset>
127)or(yoffset>127)or(xoffset<-128)or(yoffset<-128)or(compsize>1015)then
{75:}begin compsize:=compsize+13;
flagbyte:=flagbyte+compsize div 65536+4;begin putbyte(flagbyte,pkfile);
pkloc:=pkloc+1 end;pkhalfword(compsize mod 65536);
begin putbyte(gfch,pkfile);pkloc:=pkloc+1 end;predpkloc:=pkloc+compsize;
pkthreebytes(tfmwidth[gfchmod256]);pkhalfword(dx[gfchmod256]div 65536);
pkhalfword(width);pkhalfword(height);pkhalfword(xoffset);
pkhalfword(yoffset);end{:75}else{74:}begin compsize:=compsize+8;
...
begin putbyte(yoffset,pkfile);pkloc:=pkloc+1 end;end{:74}{:72};
if dynf<>14 then{76:}begin bitweight:=16;max2:=208-15*dynf;i:=0;
...
pkloc:=pkloc+1 end;end{:76}else if height>0 then{77:}begin buff:=0;
...
{81:}repeat if count=0 then begin if row[i]<0 then begin if not state
...
buff:=0;pbit:=8;end;end;until hbit=width{:81};
...
pkloc:=pkloc+1 end;end{:77}{:69};end{:63};procedure rowoverflow;
var newrow:integer;begin newrow:=maxrow+MAXROW;
if verbose then writeln(stdout,'Reallocated row array to ',newrow:1,
' items from ',maxrow:1,'.');row:=xreallocarray(row,integer,newrow);
maxrow:=newrow;end;procedure convertgffile;var i,j,k:integer;
gfcom:integer;{59:}dotherows:boolean;on:boolean;state:boolean;
extra:integer;{:59}{62:}hppp,vppp:integer;q:integer;postloc:integer;
{:62}begin opengffile;
if gfbyte<>247 then begin writeln(stderr,'Bad GF file: ',
'First byte is not preamble','!');uexit(1);end;
if gfbyte<>131 then begin writeln(stderr,'Bad GF file: ',
'Identification byte is incorrect','!');uexit(1);end;
{61:}gflen:=gflength;
if gflen<8 then begin writeln(stderr,'Bad GF file: ','only ',gflen:1,
' bytes long','!');uexit(1);end;postloc:=gflen-4;
repeat if postloc=0 then begin writeln(stderr,'Bad GF file: ',
'all 223''s','!');uexit(1);end;movetobyte(postloc);k:=gfbyte;
postloc:=postloc-1;until k<>223;
if k<>131 then begin writeln(stderr,'Bad GF file: ','ID byte is ',k:1,
'!');uexit(1);end;
if postloc<5 then begin writeln(stderr,'Bad GF file: ',
'post location is ',postloc:1,'!');uexit(1);end;movetobyte(postloc-3);
q:=gfsignedquad;
if(q<0)or(q>postloc-3)then begin writeln(stderr,'Bad GF file: ',
'post pointer is ',q:1,'!');uexit(1);end;movetobyte(q);k:=gfbyte;
if k<>248 then begin writeln(stderr,'Bad GF file: ','byte at ',q:1,
...
if status[gfch]<>0 then begin writeln(stderr,'Bad GF file: ',
...
{54:}239,240,241,242:begin begin putbyte(gfcom+1,pkfile);
pkloc:=pkloc+1 end;i:=0;for j:=0 to gfcom-239 do begin k:=gfbyte;
begin putbyte(k,pkfile);pkloc:=pkloc+1 end;i:=i*256+k;end;
for j:=1 to i do begin putbyte(gfbyte,pkfile);pkloc:=pkloc+1 end;end;
243:begin begin putbyte(244,pkfile);pkloc:=pkloc+1 end;
pkword(gfsignedquad);end;244:{:54};249:;
others:begin writeln(stderr,'Bad GF file: ','Unexpected ',gfcom:1,
' in postamble','!');uexit(1);end end;until gfcom=249{:61};
movetobyte(2);openpkfile;{82:}begin putbyte(247,pkfile);
...
pkword(checksum);pkword(hppp);pkword(vppp){:82};repeat gfcom:=gfbyte;
dotherows:=false;
case gfcom of 67,68:{55:}begin if gfcom=67 then begin gfch:=gfsignedquad
...
if status[gfchmod256]=0 then begin writeln(stderr,'Bad GF file: ',
'no character locator for character ',gfch:1,'!');uexit(1);end;
{58:}begin rowptr:=2;on:=false;extra:=0;state:=true;
repeat gfcom:=gfbyte;case gfcom of{60:}0:begin state:=not state;
on:=not on;end;
...
end else begin begin if rowptr>maxrow then rowoverflow;
row[rowptr]:=extra;rowptr:=rowptr+1;end;extra:=i;end;on:=not on;
end{:60};70,71,72,73:begin i:=0;for j:=1 to gfcom-70 do i:=i*256+gfbyte;
if on=state then begin if rowptr>maxrow then rowoverflow;
row[rowptr]:=extra;rowptr:=rowptr+1;end;
for j:=0 to i do begin if rowptr>maxrow then rowoverflow;
row[rowptr]:=(-99999);rowptr:=rowptr+1;end;on:=false;extra:=0;
state:=true;end;
...
for j:=1 to i do begin putbyte(gfbyte,pkfile);pkloc:=pkloc+1 end;end;
243:begin begin putbyte(244,pkfile);pkloc:=pkloc+1 end;
pkword(gfsignedquad);end;244:{:54};
69:begin if on=state then begin if rowptr>maxrow then rowoverflow;
row[rowptr]:=extra;rowptr:=rowptr+1;end;
if(rowptr>2)and(row[rowptr-1]<>(-99999))then begin if rowptr>maxrow then
rowoverflow;row[rowptr]:=(-99999);rowptr:=rowptr+1;end;
begin if rowptr>maxrow then rowoverflow;row[rowptr]:=(-99998);
rowptr:=rowptr+1;end;packandsendcharacter;status[gfchmod256]:=2;
if pkloc<>predpkloc then begin writeln(stderr,
'Internal error while writing character!');uexit(1);end;end;
others:begin writeln(stderr,'Bad GF file: ','Unexpected ',gfcom:1,
' command in character definition','!');uexit(1);end end;
if dotherows then begin dotherows:=false;
if on=state then begin if rowptr>maxrow then rowoverflow;
row[rowptr]:=extra;rowptr:=rowptr+1;end;
begin if rowptr>maxrow then rowoverflow;row[rowptr]:=(-99999);
rowptr:=rowptr+1;end;on:=true;extra:=gfcom-74;state:=false;end;
until gfcom=69;end{:58};end{:55};
{54:}239,240,241,242:begin begin putbyte(gfcom+1,pkfile);
pkloc:=pkloc+1 end;i:=0;for j:=0 to gfcom-239 do begin k:=gfbyte;
begin putbyte(k,pkfile);pkloc:=pkloc+1 end;i:=i*256+k;end;
for j:=1 to i do begin putbyte(gfbyte,pkfile);pkloc:=pkloc+1 end;end;
243:begin begin putbyte(244,pkfile);pkloc:=pkloc+1 end;
pkword(gfsignedquad);end;244:{:54};248:;
others:begin writeln(stderr,'Bad GF file: ','Unexpected ',gfcom:1,
' command between characters','!');uexit(1);end end;until gfcom=248;
{84:}begin putbyte(245,pkfile);pkloc:=pkloc+1 end;
while(pkloc mod 4<>0)do begin putbyte(246,pkfile);
pkloc:=pkloc+1 end{:84};end;{:52}{86:}begin initialize;convertgffile;
{85:}for i:=0 to 255 do if status[i]=1 then if verbose then writeln(
stdout,'Character ',i:1,' missing raster information!'){:85};
