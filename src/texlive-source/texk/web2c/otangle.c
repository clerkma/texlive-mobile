...
#define noptions ( 3 ) 
  ...
  {
    fprintf ( stderr , "%s%s\n",  "otangle" , ": Need one or two file arguments." ) ;
    usage ( "otangle" ) ;
  } 
...
zscannumeric ( namepointer p ) 
{
  ...
  while ( true ) {
	...
	{
	  accumulator = accumulator + nextsign * ( equiv [q ]- 1073741824L ) 
	  ;
	  nextsign = 1 ;
	} 
      } 
...
void mainbody( void ) {
  ...
  if ( stringptr > 65536L ) 
  {
    {
      putc ('\n',  stdout );
      fprintf ( stdout , "%ld%s",  (long)stringptr - 65536L ,       " strings written to string pool file." ) ;
    } 
    putc ( '*' ,  pool );
