{3:}{9:}{$C-,A+,D-}{[$C+,D+]}
{:9}program DVIcopy(dvifile,outfile,output);label 9999;
const{5:}maxfonts=400;maxchars=750000;maxwidths=16000;maxpackets=65530;
maxbytes=250000;maxrecursion=10;stacksize=100;terminallinelength=256;
{:5}type{7:}int31t=0..2147483647;int24ut=0..16777215;
int24t=-8388608..8388607;int23t=0..8388607;int16ut=0..65535;
int16t=-32768..32767;int15t=0..32767;int8ut=0..255;int8t=-128..127;
int7t=0..127;{:7}{14:}ASCIIcode=0..255;
...
{:36}{70:}widthpointer=0..maxwidths;{:70}{76:}charoffset=-255..maxchars;
charpointer=0..maxchars;{:76}{79:}ftype=0..3;fontnumber=0..maxfonts;
{:79}{83:}typeflag=0..31;{:83}{116:}cmdpar=0..12;
{:116}{119:}cmdcl=0..17;{:119}{154:}vfstate=array[0..1,0..1]of boolean;
{:154}{156:}vftype=0..4;{:156}{192:}{[198:]pixvalue=-32767..32767;
[:198]}stackpointer=0..stacksize;stackindex=1..stacksize;
...
lcurname:int15t;{:62}{65:}{fres:int16ut;resdigits:array[1..5]of char;
nresdigits:int7t;}{:65}{71:}widths:array[widthpointer]of integer;
...
fntscaled:array[fontnumber]of int31t;
fntdesign:array[fontnumber]of int31t;
...
curext:int24t;curres:int8ut;curtype:typeflag;pcktext:int24t;
pcktres:int8ut;pcktdup:boolean;pcktprev:pcktpointer;
pcktmmsg,pcktsmsg,pcktdmsg:int7t;{:84}{90:}tfmfile:bytefile;
tfmext:pcktpointer;{:90}{96:}tfmb0,tfmb1,tfmb2,tfmb3:eightbits;
{:96}{100:}tfmconv:real;{:100}{108:}dvifile:bytefile;dviloc:integer;
fullname:^char;{:108}{117:}dvipar:packed array[eightbits]of cmdpar;
...
vfext:pcktpointer;vfcurfnt:fontnumber;{:134}{142:}{103:}z:integer;
alpha:integer;beta:int15t;
...
{:173}{177:}nopt:int16t;kopt:int16t;scanptr:bytepointer;
sepchar:ASCIIcode;{:177}{183:}typesetting:boolean;
...
recurext:array[recurpointer]of int24t;
...
recurused:recurpointer;{:220}{231:}dvinum:int31t;dviden:int31t;
dvimag:int31t;{:231}{244:}outfile:bytefile;outloc:integer;
outback:integer;outmaxv:int31t;outmaxh:int31t;outstack:int16ut;
outpages:int16ut;{:244}{255:}outfnts:array[fontnumber]of fontnumber;
...
var j,k,l:bytepointer;d:int8ut;begin j:=pcktstart[p]-1;
k:=pcktstart[p+1]-1;
...
k:bytepointer;m:int31t;begin write(termout,' = ');p:=fntname[f];
...
{:24}{25:}noreturn procedure overflow(p:pcktpointer;n:int16ut);
begin write(termout,' !Sorry, ','DVIcopy',' capacity exceeded [');
printpacket(p);writeln(termout,'=',n:1,'].');jumpout;end;
{:25}{94:}noreturn procedure badtfm;begin write(termout,'Bad TFM file');
printfont(curfnt);writeln(termout,'!');begin writeln(stderr,' ',
'Use TFtoPL/PLtoTF to diagnose and correct the problem','.');jumpout;
end;end;noreturn procedure badfont;begin writeln(termout);
case fnttype[curfnt]of 0:confusion(strfonts);1:badtfm;
{136:}2:begin write(termout,'Bad VF file');printfont(curfnt);
writeln(termout,' loc=',vfloc:1);begin writeln(stderr,' ',
'Use VFtoVP/VPtoVF to diagnose and correct the problem','.');jumpout;
end;end;{:136}others:begin writeln(stderr,' ','internal error','.');
jumpout;end;end;end;{:94}{109:}noreturn procedure baddvi;
...
end else if getoptreturnval=63 then begin usage('dvicopy');
...
printversionandexit('This is DVIcopy, Version 1.6','Peter Breitenlohner'
,nil,nil);
...
if k>=10 then begin writeln(stderr,'dvicopy',
...
endnum),10);if endnum=optarg+m then begin writeln(stderr,'dvicopy',
...
if(optind>argc)or(optind+2<argc)then begin writeln(stderr,'dvicopy',
': Need at most two file arguments.');usage('dvicopy');end;
...
{:293}procedure initialize;var{16:}i:int16t;{:16}{39:}h:hashcode;
{:39}begin kpsesetprogramname(argv[0],'dvicopy');parsearguments;
write(termout,'This is DVIcopy, Version 1.6');
writeln(termout,versionstring);
writeln(termout,'Copyright (C) 1990,2009 Peter Breitenlohner');
...
byteptr:=pcktstart[pcktptr];end;{:47}{50:}function pcktsbyte:int8t;
var a:eightbits;
...
function pcktubyte:int8ut;var a:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;pcktubyte:=a;end;
{:50}{51:}function pcktspair:int16t;var a,b:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
...
function pcktupair:int16ut;var a,b:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
begin b:=bytemem[curloc];curloc:=curloc+1;end;pcktupair:=a*256+b;end;
{:51}{52:}function pcktstrio:int24t;var a,b,c:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
begin b:=bytemem[curloc];curloc:=curloc+1;end;
...
*256+c;end;function pcktutrio:int24ut;var a,b,c:eightbits;
begin{if curloc>=curlimit then confusion(strpackets)else}
begin a:=bytemem[curloc];curloc:=curloc+1;end;
{if curloc>=curlimit then confusion(strpackets)else}
begin b:=bytemem[curloc];curloc:=curloc+1;end;
...
end;end;begin bytemem[byteptr]:=res;byteptr:=byteptr+1;end;end;
{:57}{58:}procedure pcktunsigned(o:eightbits;x:integer);begin;
if maxbytes-byteptr<5 then overflow(strbytes,maxbytes);
...
{:58}{59:}procedure pcktsigned(o:eightbits;x:integer);var xx:int31t;
...
{:59}{66:}{procedure makeres;var r:int16ut;begin nresdigits:=0;r:=fres;
...
n:pcktpointer;curloc,curlimit:bytepointer;{ll:int15t;}
...
var h:hashcode;p:widthpointer;x:int16t;begin widths[nwidths]:=w;
...
f:eightbits;e:int24t;begin q:=charpackets[fntchars[curfnt]+curres];
...
var p,q:pcktpointer;f:int8ut;e:integer;curloc:bytepointer;
...
widths[wp]:1,')');history:=2;end;checkwidth:=wp;end;
{:98}{99:}procedure loadfont;var l:int16t;p:charpointer;q:widthpointer;
bc,ec:int15t;lh:int15t;nw:int15t;w:integer;{103:}z:integer;
alpha:integer;beta:int15t;
...
'---not loaded, TFM file can''t be opened!','.');jumpout;end{:95};
{101:}readtfmword;if tfmb2>127 then badfont else lh:=tfmb2*256+tfmb3;
readtfmword;if tfmb0>127 then badfont else bc:=tfmb0*256+tfmb1;
...
ec:=ec-1;end;fntec[curfnt]:=ec{:102};
...
if z>=134217728 then begin writeln(stderr,' ',
'Character size is too large!','.');jumpout;end;
...
end{:106};;{[197:]fntspace[curfnt]:=fntscaled[curfnt]div 6;[:197]}
...
{:112}{113:}function dvisbyte:int8t;var a:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);dviloc:=dviloc+1;
if a<128 then dvisbyte:=a else dvisbyte:=a-256;end;
function dviubyte:int8ut;var a:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);dviloc:=dviloc+1;
dviubyte:=a;end;function dvispair:int16t;var a,b:eightbits;
...
function dviupair:int16ut;var a,b:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
if eof(dvifile)then baddvi else read(dvifile,b);dviloc:=dviloc+2;
dviupair:=a*256+b;end;function dvistrio:int24t;var a,b,c:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
...
*256+c;end;function dviutrio:int24ut;var a,b,c:eightbits;
begin if eof(dvifile)then baddvi else read(dvifile,a);
if eof(dvifile)then baddvi else read(dvifile,b);
if eof(dvifile)then baddvi else read(dvifile,c);dviloc:=dviloc+3;
...
*256+b)*256+c)*256+d;end;{:113}{114:}function dviuquad:int31t;
var x:integer;begin x:=dvisquad;if x<0 then baddvi else dviuquad:=x;end;
function dvipquad:int31t;var x:integer;begin x:=dvisquad;
...
curhdimen:=dvisquad;curupd:=(curcmd=132);end;12:curparm:=curcmd-171;
...
k:int15t;begin write(termout,'DVI: font ',curparm:1);{131:}f:=0;
...
+c;end;function vfutrio:int24ut;var a,b,c:eightbits;
begin if eof(vffile)then badfont else read(vffile,a);
if eof(vffile)then badfont else read(vffile,b);
if eof(vffile)then badfont else read(vffile,c);vfloc:=vfloc+3;
...
{:143}{144:}function vfuquad:int31t;var x:integer;begin x:=vfsquad;
if x<0 then badfont else vfuquad:=x;end;function vfpquad:int31t;
var x:integer;begin x:=vfsquad;if x<=0 then badfont else vfpquad:=x;end;
function vffixp:int31t;
begin if eof(vffile)then badfont else read(vffile,tfmb0);
if eof(vffile)then badfont else read(vffile,tfmb1);
if eof(vffile)then badfont else read(vffile,tfmb2);
if eof(vffile)then badfont else read(vffile,tfmb3);vfloc:=vfloc+4;
...
var f:fontnumber;k:int15t;begin write(termout,'VF: font ',curparm:1);
...
label 21,30,32,10;var tempbyte:int8ut;k:bytepointer;l:int15t;
saveext:int24t;saveres:int8ut;savecp:widthpointer;savewp:widthpointer;
...
{:176}{178:}function scankeyword(p:pcktpointer;l:int7t):boolean;
...
end;end;begin putbyte(res,outfile);outloc:=outloc+1;end;end;
...
{:253}{254:}procedure outsigned(o:eightbits;x:integer);var xx:int31t;
...
end;{:186}{204:}procedure dopre;{260:}var k:int15t;p,q,r:bytepointer;
...
for k:=0 to 23 do begin bytemem[byteptr]:=xord[ucharcast(comment[k])];
...
label 22,31,30;var k:recurpointer;f:int8ut;saveupd:boolean;
...
label 30,10;var tempbyte:int8ut;tempint:integer;dvistart:integer;
dviboppost:integer;dviback:integer;k:int15t;
...
{:229}{240:}procedure closefilesandterminate;var k:int15t;begin;
