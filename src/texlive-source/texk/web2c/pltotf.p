...
hashsize=32579;{:3}type{17:}byte=0..255;ASCIIcode=32..127;
...
{:15}{18:}xord:array[0..255]of ASCIIcode;{:18}{21:}line:integer;
...
{:152}{155:}tfmname,plname:constcstring;
{:155}{148:}procedure parsearguments;const noptions=3;
...
{:151}{154:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:154};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('pltotf');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(PLTOTFHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is PLtoTF, Version 3.6',nil,'D.E. Knuth',nil);
end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'pltotf',
': Need one or two file arguments.');usage('pltotf');end;
...
{:73}begin kpsesetprogramname(argv[0],'pltotf');parsearguments;
{6:}reset(plfile,plname);
if verbose then begin write(stderr,'This is PLtoTF, Version 3.6');
...
inputhasended:=false;{:24}{26:}charsonline:=0;perfect:=true;
{:26}{37:}startptr:=1;start[1]:=0;dictptr:=0;
{:37}{41:}for h:=0 to 100 do nhash[h]:=0;
...
{:119}end;{:2}{27:}procedure showerrorcontext;var k:0..bufsize;
...
var k:0..20;j:0..600;notfound:boolean;begin{43:}curhash:=curname[1];
...
{:136}{146:}procedure paramenter;begin{48:}namelength:=5;
...
for c:=0 to 255 do{113:}if chartag[c]=2 then begin g:=charremainder[c];
...
writeln(stderr,' units.');end;{:115}{:110}end;
{:146}{147:}begin initialize;nameenter;readinput;
