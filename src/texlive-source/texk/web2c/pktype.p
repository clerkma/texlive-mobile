{4:}program PKtype(input,output);type{9:}ASCIIcode=32..126;
{:9}{10:}textfile=packed file of 0..255;{:10}{30:}eightbits=0..255;
bytefile=packed file of eightbits;
{:30}var{11:}xord:array[0..255]of ASCIIcode;xchr:array[0..255]of 0..255;
{:11}{31:}pkfile:bytefile;{:31}{33:}pkname:cstring;curloc:integer;
{:33}{37:}termpos:integer;{:37}{39:}magnification:integer;
designsize:integer;checksum:integer;hppp,vppp:integer;
{:39}{41:}i,j:integer;flagbyte:integer;endofpacket:integer;
width,height:integer;xoff,yoff:integer;tfmwidth:integer;
tfms:array[0..255]of integer;dx,dy:integer;
dxs,dys:array[0..255]of integer;status:array[0..255]of boolean;
dynf:integer;car:integer;packetlength:integer;
{:41}{47:}inputbyte:eightbits;bitweight:eightbits;nybble:eightbits;
{:47}{51:}repeatcount:integer;rowsleft:integer;turnon:boolean;
hbit:integer;count:integer;{:51}{56:}procedure parsearguments;
const noptions=2;var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{57:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:57}{58:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:58}{59:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:59};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval='?'then begin usage('pktype');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(PKTYPEHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is PKtype, Version 2.3',nil,'Tomas Rokicki',
nil);end;until getoptreturnval=-1;
if(optind+1<>argc)then begin writeln(stderr,'pktype',
': Need exactly one file argument.');usage('pktype');end;end;
{:56}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'pktype');
kpseinitprog('PKTYPE',0,nil,nil);parsearguments;
write(output,'This is PKtype, Version 2.3');
writeln(output,versionstring);{12:}for i:=0 to 31 do xchr[i]:='?';
...
for i:=32 to 126 do xord[xchr[i]]:=i;{:13}end;
{:4}{32:}procedure openpkfile;
begin pkfile:=kpseopenfile(cmdline(1),kpsepkformat);curloc:=0;end;
{:32}{34:}function getbyte:integer;var b:eightbits;
...
write(stdout,value:1)else write(stdout,'(',value:1,')');end;
{23:}function pkpackednum:integer;var i,j:integer;begin i:=getnyb;
...
writeln(output,'Second repeat count for this row!');uexit(1)end;
repeatcount:=1;if i=14 then repeatcount:=pkpackednum;
sendout(true,repeatcount);pkpackednum:=pkpackednum;end;end;
{:23}{:46}{52:}procedure skipspecials;var i,j:integer;
...
244:begin write(stdout,(curloc-1):1);
writeln(stdout,':  Num special: ',signedquad:1);end;
245:writeln(stdout,(curloc-1):1,':  Postamble');
246:writeln(stdout,(curloc-1):1,':  No op');
247,248,249,250,251,252,253,254,255:begin writeln(output,'Unexpected ',
flagbyte:1,'!');uexit(1)end;end;until(flagbyte<240)or(flagbyte=245);end;
{:52}{55:}begin initialize;openpkfile;
{38:}if getbyte<>247 then begin writeln(output,
'Bad PK file:  pre command missing!');uexit(1)end;
if getbyte<>89 then begin writeln(output,'Wrong version of PK file!');
...
{:38};skipspecials;
while flagbyte<>245 do begin{40:}write(stdout,(curloc-1):1,
':  Flag byte = ',flagbyte:1);dynf:=flagbyte div 16;
flagbyte:=flagbyte mod 16;turnon:=flagbyte>=8;
if turnon then flagbyte:=flagbyte-8;
if flagbyte=7 then{42:}begin packetlength:=signedquad;car:=signedquad;
...
height:=signedquad;xoff:=signedquad;yoff:=signedquad;
end{:42}else if flagbyte>3 then{43:}begin packetlength:=(flagbyte-4)
*65536+gettwobytes;car:=getbyte;endofpacket:=packetlength+curloc;
packetlength:=packetlength+4;i:=getbyte;tfmwidth:=i*65536+gettwobytes;
dx:=gettwobytes*65536;dy:=0;width:=gettwobytes;height:=gettwobytes;
xoff:=gettwobytes;yoff:=gettwobytes;if xoff>32767 then xoff:=xoff-65536;
if yoff>32767 then yoff:=yoff-65536;
end{:43}else{44:}begin packetlength:=flagbyte*256+getbyte;car:=getbyte;
...
if xoff>127 then xoff:=xoff-256;if yoff>127 then yoff:=yoff-256;
end{:44};
...
if dynf=14 then{49:}begin for i:=1 to height do begin write(stdout,'  ')
...
if(rowsleft<>0)or(hbit<>width)then begin writeln(output,
'Bad PK file: More bits than required!');uexit(1)end;end{:50}{:48};
if endofpacket<>curloc then begin writeln(output,
'Bad PK file: Bad packet length!');uexit(1)end{:40};skipspecials;end;
j:=0;while not eof(pkfile)do begin i:=getbyte;
if i<>246 then begin writeln(output,'Bad byte at end of file: ',i:1);
