...
#define noptions ( 10 ) 
  ...
  do {
	...
	else if ( optarg [m ]== 46 ) 
	{
	  ...
	  if ( k >= 10 ) 
	  {
	    fprintf ( stderr , "%s%s\n",  "pdvitype" ,             ": More than ten count registers specified." ) ;
	    uexit ( 1 ) ;
	  } 
	  m = m + 1 ;
	} 
	  ...
	  if ( endnum == optarg + m ) 
	  {
	    fprintf ( stderr , "%s%s\n",  "pdvitype" ,             ": -page-start values must be numeric or *." ) ;
	    uexit ( 1 ) ;
	  } 
	  startthere [k ]= true ;
	  m = m + endnum - ( optarg + m ) ;
  ...
  if ( ( optind + 1 != argc ) ) 
  {
    fprintf ( stderr , "%s%s\n",  "pdvitype" , ": Need exactly one file argument." ) ;
    usage ( "pdvitype" ) ;
  } 
...
{
  integer i  ;
  kpsesetprogramname ( argv [0 ], "pdvitype" ) ;
  initkanji () ;
  parsearguments () ;
  Fputs ( stdout ,  "This is pDVItype, Version 3.6-p0.4" ) ;
  fprintf ( stdout , "%s\n",  versionstring ) ;
...
zoutkanji ( integer c ) 
{
  if ( textptr >= linelength - 3 ) 
  flushtext () ;
  c = toBUFF ( fromDVI ( c ) ) ;
  textptr = textptr + 1 ;
  textbuf [textptr ]= Hi ( c ) ;
  textptr = textptr + 1 ;
  textbuf [textptr ]= Lo ( c ) ;
} 
#ifdef HEX_CHAR_CODE
void 
