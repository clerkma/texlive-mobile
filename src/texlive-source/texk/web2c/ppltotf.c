...
#define noptions ( 5 ) 
  ...
  do {
    ...
    {
      printversionandexit ( "This is pPLtoTF, Version 3.6-p1.8" , nil , 
      "D.E. Knuth" , nil ) ;
    } 
  ...
  {
    fprintf ( stderr , "%s%s\n",  "ppltotf" , ": Need one or two file arguments." ) ;
    usage ( "ppltotf" ) ;
  } 
...
initialize ( void ) 
{
  ...
  headerindex d  ;
  byte c  ;
  kpsesetprogramname ( argv [0 ], "ppltotf" ) ;
  initkanji () ;
  parsearguments () ;
  reset ( plfile , plname ) ;
  if ( verbose ) 
  {
    Fputs ( stderr ,  "This is pPLtoTF, Version 3.6-p1.8" ) ;
    fprintf ( stderr , "%s\n",  versionstring ) ;
...
getnextraw ( void ) 
{
  ...
  loc = loc + 1 ;
  Result = buffer [loc ];
  if ( multistrlen ( ustringcast ( buffer ) , loc + 2 , loc ) == 2 ) 
  curchar = 32 ;
...
{
  register boolean Result; integer firstbyte, secondbyte  ;
  Result = true ;
  firstbyte = cx / 256 ;
  secondbyte = cx % 256 ;
  if ( ( firstbyte < 33 ) || ( ( firstbyte > 40 ) && ( firstbyte < 48 ) ) || ( 
  firstbyte > 116 ) ) 
  Result = false ;
  if ( ( secondbyte < 33 ) || ( secondbyte > 126 ) ) 
  Result = false ;
  return Result ;
} 
integer 
...
{
  register integer Result; integer firstbyte, secondbyte  ;
  firstbyte = jis / 256 - 33 ;
  secondbyte = jis % 256 - 33 ;
  if ( firstbyte < 8 ) 
  Result = firstbyte * 94 + secondbyte ;
  else Result = ( firstbyte - 7 ) * 94 + secondbyte ;
  return Result ;
} 
integer 
zindextojis ( integer ix ) 
{
  register integer Result; if ( ix <= 8 * 94 - 1 ) 
  Result = ( ix / 94 + 33 ) * 256 + ( ix % 94 + 33 ) ;
  else Result = ( ( ix + 7 * 94 ) / 94 + 33 ) * 256 + ( ( ix + 7 * 94 ) % 94 + 
  33 ) ;
  return Result ;
} 
integer 
getkanji ( void ) 
{
  ...
  else if ( multistrlen ( ustringcast ( buffer ) , loc + 2 , loc ) == 2 ) 
  {
    jiscode = toDVI ( fromBUFF ( ustringcast ( buffer ) , loc + 2 , loc ) ) ;
    loc = loc + 1 ;
    curchar = 32 ;
    if ( ! validjiscode ( jiscode ) ) 
    {
