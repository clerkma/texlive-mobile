{2:}{4:}{$C-,A+,D-}{[$C+,D+]}
{:4}program OTANGLE(webfile,changefile,Pascalfile,pool);
const{8:}bufsize=3000;maxbytes=65535;maxtoks=65535;maxnames=10239;
maxtexts=10239;hashsize=353;longestname=400;linelength=72;
outbufsize=144;stacksize=100;maxidlength=50;unambiglength=25;
{:8}type{11:}ASCIIcode=0..255;
{:11}{12:}textfile=packed file of ASCIIcode;{:12}{37:}eightbits=0..255;
sixteenbits=integer;{:37}{39:}namepointer=0..maxnames;
...
{:185}{192:}webname,chgname,pascalname,poolname:constcstring;
{:192}{30:}{procedure debughelp;forward;}{:30}{31:}procedure error;
...
{:31}{188:}procedure parsearguments;const noptions=3;
...
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:190}{191:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
...
end else if getoptreturnval=63 then begin usage('otangle');
...
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'otangle'
,': Need one or two file arguments.');usage('otangle');end;
...
{:41}{45:}zi:0..3;{:45}{51:}h:0..hashsize;
{:51}begin kpsesetprogramname(argv[0],'otangle');parsearguments;
...
bytestart[3]:=0;nameptr:=1;stringptr:=65536;poolchecksum:=271828;
...
i:=i+1;end;choppedid[s]:=0;end{:58};
...
begin if stringptr=65536 then begin poolname:=basenamechangesuffix(
webname,'.web','.pool');rewrite(pool,poolname);end;
...
,113,114,115,116,117,118,119,120,121,122:begin outcontrib[1]:=curchar;
...
if bracelevel=0 then sendout(0,125)else sendout(0,93);end;
{:121}127:begin sendout(3,0);outstate:=6;end;2:{118:}begin k:=0;
...
begin accumulator:=accumulator+nextsign*(equiv[q]-1073741824);
...
30:nextcontrol:=a;
{166:}if bal>0 then begin if bal=1 then begin writeln(stdout);
...
while true do begin write(stdout,'#');fflush(stdout);read(stdin,ddt);
...
write(stdout,'This is OTANGLE, Version 4.4');
...
if stringptr>65536 then{184:}begin begin writeln(stdout);
write(stdout,stringptr-65536:1,' strings written to string pool file.');
end;write(pool,'*');
