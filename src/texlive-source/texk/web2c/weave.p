{2:}{4:}{$C-,A+,D-}{[$C+,D+]}
{:4}program WEAVE(webfile,changefile,texfile);const{8:}maxbytes=65536;
maxnames=10220;maxmodules=2000;hashsize=353;bufsize=3000;
longestname=400;longbufsize=500;linelength=80;maxrefs=65000;
maxtoks=65000;maxtexts=10000;maxscraps=10000;stacksize=2000;
{:8}type{11:}ASCIIcode=0..255;
{:11}{12:}textfile=packed file of ASCIIcode;{:12}{36:}eightbits=0..255;
sixteenbits=0..65535;{:36}{38:}namepointer=0..maxnames;
{:38}{47:}xrefnumber=0..maxrefs;{:47}{52:}textpointer=0..maxtexts;
{:52}{201:}mode=0..1;outputstate=record endfield:sixteenbits;
tokfield:sixteenbits;modefield:mode;end;{:201}var{9:}history:0..3;
{:9}{13:}xord:array[ASCIIcode]of ASCIIcode;
...
debugcycle:integer;debugskipped:integer;}{:258}{268:}noxref:cinttype;
{:268}{270:}webname,chgname,texname:constcstring;
{:270}{264:}procedure parsearguments;const noptions=4;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{265:}currentoption:=0;longoptions[currentoption].name:='help';
...
{:265}{266:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:266}{267:}longoptions[currentoption].name:=chartostring('x');
...
{:267}{269:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:269};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('weave');
...
printversionandexit('This is WEAVE, Version 4.4',nil,'D.E. Knuth',nil);
end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'weave',
': Need one or two file arguments.');usage('weave');end;
webname:=extendfilename(cmdline(optind),'web');
if optind+2=argc then begin chgname:=extendfilename(cmdline(optind+1),
'ch');end;texname:=basenamechangesuffix(webname,'.web','.tex');end;
{:264}{30:}{procedure debughelp;forward;}{:30}{31:}procedure error;
var k,l:0..longbufsize;
begin{32:}begin if changing then write(stdout,'. (change file ')else
write(stdout,'. (');writeln(stdout,'l.',line:1,')');
...
end{:32};fflush(stdout);history:=2;{debugskipped:=debugcycle;debughelp;}
...
end;{:33}procedure initialize;var{16:}i:0..255;{:16}{40:}wi:0..1;
{:40}{56:}h:0..hashsize;{:56}{247:}c:ASCIIcode;
{:247}begin kpsesetprogramname(argv[0],'weave');parsearguments;
...
{:18}{21:}{:21}{26:}rewrite(texfile,texname);
...
write(stderr,'! Sorry, ','cross reference',' capacity exceeded');error;
...
{:88}others:begin begin if not phaseone then begin writeln(stdout);
...
30:{105:}if k>=longestname-2 then begin begin writeln(stdout);
...
{138:}begin if tokptr+2>maxtoks then begin writeln(stdout);
write(stderr,'! Sorry, ','token',' capacity exceeded');error;history:=3;
jumpout;end;tokmem[tokptr]:=32;tokptr:=tokptr+1;end;
repeat begin if tokptr+2>maxtoks then begin writeln(stdout);
write(stderr,'! Sorry, ','token',' capacity exceeded');error;history:=3;
...
begin case c of 1:write(stdout,'simp');2:write(stdout,'math');
3:write(stdout,'intro');4:write(stdout,'open');
...
others:write(stdout,'UNKNOWN')end;end;}
...
if hiptr<=scrapptr then write(stdout,'...');end;end;}
...
{182:}{if tracing=2 then begin begin writeln(stdout);
...
begin writeln(stdout);
write(stderr,'! Sorry, ','token/text',' capacity exceeded');error;
...
write(stderr,'! Sorry, ','scrap/token/text',' capacity exceeded');error;
...
writeln(stdout);write(stderr,'! Sorry, ','token',' capacity exceeded');
...
if tokptr>maxtokptr then maxtokptr:=tokptr;
if textptr>maxtxtptr then maxtxtptr:=textptr;}begin writeln(stdout);
write(stderr,'! Sorry, ','token/text/scrap',' capacity exceeded');error;
...
stdout);write(stdout,'! Illegal control code in section name:');end;
...
write(stderr,'! Sorry, ','buffer',' capacity exceeded');error;
...
writeln(stdout);write(stderr,'! Sorry, ','buffer',' capacity exceeded');
...
write(stderr,'! Sorry, ','sorting',' capacity exceeded');error;
...
begin{218:}resetinput;begin writeln(stdout);
...
fflush(stdout);end;outmod(modulecount);
begin if outptr=linelength then breakout;outptr:=outptr+1;
...
end;end;others:end;until nextcontrol>=143{:222};
...
writeln(stdout);write(stdout,'! You can''t do that in Pascal text');
...
write(stdout,'This is WEAVE, Version 4.4');
writeln(stdout,versionstring);{64:}idloc:=10;idfirst:=7;buffer[7]:=97;
buffer[8]:=110;buffer[9]:=100;curname:=idlookup(28);idfirst:=5;
buffer[5]:=97;buffer[6]:=114;buffer[7]:=114;buffer[8]:=97;
...
{:64};PhaseI;PhaseII;{239:}if noxref then begin finishline;
begin if outptr=linelength then breakout;outptr:=outptr+1;
outbuf[outptr]:=92;end;begin if outptr=linelength then breakout;
...
write(stdout,'! Change file entry did not match');error;end;end;
end{:85};jumpout;end.{:261}
