...
hashsize=32579;{:3}{158:}maxkanji=65535;yokoidnumber=11;tateidnumber=9;
...
end else if getoptreturnval=63 then begin usage('uppltotf');
...
printversionandexit('This is upPLtoTF, Version 3.6-p1.8-u1.20',nil,
'D.E. Knuth',nil);
...
'uppltotf',': Need one or two file arguments.');usage('uppltotf');end;
...
{:73}begin kpsesetprogramname(argv[0],'uppltotf');initkanji;
parsearguments;{6:}reset(plfile,plname);
if verbose then begin write(stderr,
'This is upPLtoTF, Version 3.6-p1.8-u1.20');
...
if multistrlen(ustringcast(buffer),loc+3,loc)>1 then curchar:=32 else
...
function validjiscode(cx:integer):boolean;begin validjiscode:=true;
if(cx>65535)or(not ischarkanji(fromDVI(cx)))or(toDVI(fromDVI(cx))<>cx)
then validjiscode:=false;end;function jistoindex(jis:integer):integer;
begin jistoindex:=jis;end;function indextojis(ix:integer):integer;
begin indextojis:=ix;end;function getkanji:integer;var ch:byte;
cx,jiscode:integer;begin repeat ch:=getnextraw;until ch<>' ';
...
showerrorcontext;end;
end else if(ch='U')or(ch='u')then begin repeat ch:=getnextraw;
until ch<>' ';cx:=todig(xord[ch])*4096;loc:=loc+1;ch:=xord[buffer[loc]];
cx:=cx+todig(ch)*256;loc:=loc+1;ch:=xord[buffer[loc]];
cx:=cx+todig(ch)*16;loc:=loc+1;ch:=xord[buffer[loc]];cx:=cx+todig(ch);
jiscode:=toDVI(fromUCS(cx));curchar:=ch;
if not validjiscode(jiscode)then begin if charsonline>0 then writeln(
stderr,' ');write(stderr,'jis code ',jiscode:1,' is invalid');
showerrorcontext;end;
end else if multistrlen(ustringcast(buffer),loc+4,loc)>1 then begin
curchar:=32;jiscode:=toDVI(fromBUFF(ustringcast(buffer),loc+4,loc));
