{4:}program PKtoGF(input,output);const{6:}MAXCOUNTS=400;
{:6}type{9:}ASCIIcode=32..126;{:9}{10:}textfile=packed file of 0..255;
{:10}{38:}eightbits=0..255;bytefile=packed file of eightbits;
{:38}var{11:}xord:array[0..255]of ASCIIcode;xchr:array[0..255]of 0..255;
{:11}{39:}gffile,pkfile:bytefile;{:39}{41:}gfname,pkname:cstring;
gfloc,curloc:integer;{:41}{48:}i,j:integer;endofpacket:integer;
dynf:integer;car:integer;tfmwidth:integer;xoff,yoff:integer;
{:48}{50:}comment:packed array[1..17]of 0..255;magnification:integer;
...
{:57}{63:}inputbyte:eightbits;bitweight:eightbits;maxcounts:integer;
rowcounts:^integer;rcp:integer;{:63}{67:}countdown:integer;done:boolean;
max:integer;repeatcount:integer;xtogo,ytogo:integer;
turnon,firston:boolean;count:integer;curn:integer;
{:67}{69:}flagbyte:integer;{:69}{78:}verbose:cinttype;
{:78}{74:}procedure parsearguments;const noptions=3;
var longoptions:array[0..noptions]of getoptstruct;
getoptreturnval:integer;optionindex:cinttype;currentoption:0..noptions;
begin{79:}verbose:=false;{:79};{75:}currentoption:=0;
...
{:75}{76:}longoptions[currentoption].name:='version';
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:76}{77:}longoptions[currentoption].name:='verbose';
...
{:77}{80:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:80};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('pktogf');
end else if(strcmp(longoptions[optionindex].name,'help')=0)then begin
usagehelp(PKTOGFHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is PKtoGF, Version 1.1',nil,'Tomas Rokicki',
nil);end;until getoptreturnval=-1;
if(optind+1<>argc)and(optind+2<>argc)then begin writeln(stderr,'pktogf',
': Need one or two file arguments.');usage('pktogf');end;end;
{:74}procedure initialize;var i:integer;
begin kpsesetprogramname(argv[0],'pktogf');
kpseinitprog('PKTOGF',0,nil,nil);parsearguments;
if verbose then writeln(output,'This is PKtoGF, Version 1.1');
...
{:13}{51:}rowcounts:=xmallocarray(integer,MAXCOUNTS);
maxcounts:=MAXCOUNTS;{:51}{58:}mminm:=999999;mminn:=999999;
mmaxm:=-999999;mmaxn:=-999999;for i:=0 to 255 do charpointer[i]:=-1;
{:58}end;{:4}{40:}procedure openpkfile;begin pkname:=cmdline(optind);
pkfile:=kpseopenfile(cmdline(optind),kpsepkformat);
if pkfile then begin curloc:=0;end;end;procedure opengffile;
begin if optind+1=argc then begin gfname:=basenamechangesuffix(pkname,
'pk','gf');end else begin gfname:=cmdline(optind+1);end;
rewritebin(gffile,gfname);gfloc:=0;end;
{:40}{43:}function getbyte:integer;var b:eightbits;
...
gfloc:=gfloc+1 end;end else begin i:=i+1073741824;i:=i+1073741824;
...
if verbose then writeln(output,'Unexpected ',flagbyte:1,'!');uexit(1);
end;end;until(flagbyte<240)or(flagbyte=245);end;
{:70}{73:}begin initialize;{44:}openpkfile;opengffile{:44};
{49:}if getbyte<>247 then begin verbose:=true;
if verbose then writeln(output,'Bad pk file!  pre command missing.');
uexit(1);end;begin putbyte(247,gffile);gfloc:=gfloc+1 end;
if getbyte<>89 then begin verbose:=true;
if verbose then writeln(output,'Wrong version of packed file!.');
uexit(1);end;begin putbyte(131,gffile);gfloc:=gfloc+1 end;j:=getbyte;
begin putbyte(j,gffile);gfloc:=gfloc+1 end;
if verbose then write(output,'{');for i:=1 to j do begin hppp:=getbyte;
begin putbyte(hppp,gffile);gfloc:=gfloc+1 end;
if verbose then write(output,xchr[xord[hppp]]);end;
if verbose then writeln(output,'}');designsize:=signedquad;
checksum:=signedquad;hppp:=signedquad;vppp:=signedquad;
if hppp<>vppp then if verbose then writeln(output,
...
if flagbyte=7 then{52:}begin packetlength:=signedquad;car:=signedquad;
...
*65536+gettwobytes;car:=getbyte;endofpacket:=packetlength+curloc;
i:=getbyte;tfmwidth:=i*65536+gettwobytes;horesc:=gettwobytes*65536;
veresc:=0;cwidth:=gettwobytes;cheight:=gettwobytes;
wordwidth:=(cwidth+31)div 32;xoff:=signedpair;yoff:=signedpair;
end{:53}else{54:}begin packetlength:=flagbyte*256+getbyte;car:=getbyte;
...
xoff:=signedbyte;yoff:=signedbyte;end{:54};
...
i]<>tfmwidth)then if verbose then writeln(output,'Two characters mod ',i
:1,' have mismatched parameters');end;end{:60};
...
256]:=thischarptr;begin putbyte(68,gffile);gfloc:=gfloc+1 end;
...
if rcp>maxcounts then begin if verbose then writeln(output,
'Reallocated row_counts array to ',(maxcounts+MAXCOUNTS):1,
' items from ',maxcounts:1,'.');maxcounts:=maxcounts+MAXCOUNTS;
rowcounts:=xreallocarray(rowcounts,integer,maxcounts);end;
xtogo:=xtogo-count;count:=0;end;end;end{:65};begin putbyte(69,gffile);
gfloc:=gfloc+1 end;lasteoc:=gfloc;
if endofpacket<>curloc then begin verbose:=true;
if verbose then writeln(output,'Bad pk file!  Bad packet length.');
...
then begin begin putbyte(246,gffile);gfloc:=gfloc+1 end;
...
if verbose then writeln(output,curloc:1,' bytes unpacked to ',gfloc:1,
' bytes.');end.{:73}
