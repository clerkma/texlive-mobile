...
zprintcs ( integer p ) 
{
  ...
  else if ( ( ( p >= 24527 ) && ( p <= 28082 ) ) || ( p > eqtbtop ) ) 
  printesc ( 580 ) ;
	...
	if ( memtop - 2 != pagetail ) 
	{
	  ...
	  if ( pagecontents > 0 ) 
	  {
	    ...
	    while ( r != memtop ) {
	      ...
	      printint ( t ) ;
	      print ( 1389 ) ;
	      if ( eqtb [27281 + t ].cint == 1000 ) 
	      t = mem [r + 3 ].cint ;
	      else t = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [27281 + 
	      t ].cint ;
	      printscaled ( t ) ;
	      if ( mem [r ].hh.b0 == 1 ) 
	      {
...
zprintcmdchr ( quarterword cmd , halfword chrcode ) 
{
  ...
  case 73 : 
    ...
    printparam ( chrcode - 27177 ) ;
      ...
      printesc ( 535 ) ;
      printint ( chrcode - 27281 ) ;
    } 
    break ;
  case 74 : 
    if ( chrcode < 27827 ) 
    printlengthparam ( chrcode - 27793 ) ;
    else {
	
      printesc ( 572 ) ;
      printint ( chrcode - 27827 ) ;
    } 
    break ;
  case 45 : 
    printesc ( 582 ) ;
...
zshoweqtb ( halfword n ) 
{
  ...
  if ( n < 1 ) 
  printchar ( 63 ) ;
  else if ( ( n < 24528 ) || ( ( n > 28082 ) && ( n <= eqtbtop ) ) ) 
  {
    sprintcs ( n ) ;
    printchar ( 61 ) ;
...
zscansomethinginternal ( smallnumber level , boolean negative ) 
{
  ...
  case 89 : 
	...
	{case 0 : 
	  curval = eqtb [27281 + curval ].cint ;
	  break ;
	case 1 : 
	  curval = eqtb [27827 + curval ].cint ;
	  break ;
	case 2 : 
...
openlogfile ( void ) 
{
  ...
  logopened = true ;
  {
    ...
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.14159265" , "-2.6" ,     "-1.40.16" ) ;
    else
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.14159265" , "-2.6" ,     "-1.40.16" ) ;
...
zoutwhat ( halfword p ) 
{
  ...
  case 2 : 
	...
	else if ( j < 16 ) 
	{
	  ...
	  aopenout ( writefile [j ]) ) promptfilename ( 1911 , 944 ) ;
	  writeopen [j ]= true ;
	  if ( logopened ) 
	  {
	    oldsetting = selector ;
...
zdvishipout ( halfword p ) 
{
  ...
  synctexsheet ( eqtb [27194 ].cint ) ;
  {
    ...
    printchar ( 91 ) ;
    j = 9 ;
    while ( ( eqtb [27281 + j ].cint == 0 ) && ( j > 0 ) ) decr ( j ) ;
    {register integer for_end; k = 0 ;for_end = j ; if ( k <= for_end) do 
      {
	printint ( eqtb [27281 + k ].cint ) ;
	if ( k < j ) 
	printchar ( 46 ) ;
      } 
    while ( k++ < for_end ) ;} 
    ...
    27812 ].cint > 1073741823L ) || ( mem [p + 1 ].cint + eqtb [27811 ]
    .cint > 1073741823L ) ) 
    {
      {
	if ( interaction == 3 ) 
    ...
    if ( mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [27812 ].cint > 
    maxv ) 
    maxv = mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [27812 ].cint ;
    if ( mem [p + 1 ].cint + eqtb [27811 ].cint > maxh ) 
    maxh = mem [p + 1 ].cint + eqtb [27811 ].cint ;
    dvih = 0 ;
    dviv = 0 ;
    curh = eqtb [27811 ].cint ;
    dvif = 0 ;
    curhoffset = eqtb [27811 ].cint ;
    curvoffset = eqtb [27812 ].cint ;
    if ( eqtb [27816 ].cint != 0 ) 
    curpagewidth = eqtb [27816 ].cint ;
    else curpagewidth = mem [p + 1 ].cint + 2 * curhoffset + 2 * 4736286L ;
    if ( eqtb [27817 ].cint != 0 ) 
    curpageheight = eqtb [27817 ].cint ;
    ...
    {register integer for_end; k = 0 ;for_end = 9 ; if ( k <= for_end) do 
      dvifour ( eqtb [27281 + k ].cint ) ;
    while ( k++ < for_end ) ;} 
    dvifour ( lastbop ) ;
    lastbop = pageloc ;
    curv = mem [p + 3 ].cint + eqtb [27812 ].cint ;
    tempptr = p ;
    if ( mem [p ].hh.b0 == 1 ) 
    vlistout () ;
    else hlistout () ;
...
zliteral ( strnumber s , integer literalmode , boolean warn ) 
{
  ...
  if ( literalmode == 3 ) 
  {
    if ( ! ( strinstr ( s , 1053 , 0 ) || strinstr ( s , 1054 , 0 ) ) ) 
    {
      if ( warn && ! ( strinstr ( s , 1055 , 0 ) || strinstr ( s , 1056 , 0 ) 
      || ( ( strstart [s + 1 ]- strstart [s ]) == 0 ) ) ) 
      printnl ( 1057 ) ;
      return ;
    } 
    j = j + ( strstart [1054 ]- strstart [1053 ]) ;
    if ( strinstr ( s , 1058 , ( strstart [1054 ]- strstart [1053 ]) ) ) 
    {
      j = j + ( strstart [1059 ]- strstart [1058 ]) ;
      literalmode = 2 ;
    } 
    else if ( strinstr ( s , 1059 , ( strstart [1054 ]- strstart [1053 ]) 
    ) ) 
    {
...
zadjustinterwordglue ( halfword p , halfword g ) 
{
  ...
  {
    pdfwarning ( 1073 , 1074 , false , false ) ;
    return ;
  } 
  c = 256 ;
  if ( ( p >= himemmin ) ) 
...
ztfmlookup ( strnumber s , scaled fs ) 
{
  ...
  if ( fs != 0 ) 
  {
    ...
    do 
      if ( ( fontarea [k ]!= 1072 ) && streqstr ( fontname [k ], s ) && ( 
      fontsize [k ]== fs ) ) 
      {
	flushstr ( s ) ;
	Result = k ;
...
zdovf ( internalfontnumber f ) 
{
  ...
  return ;
  stacklevel = 0 ;
  packfilename ( fontname [f ], 345 , 1094 ) ;
  if ( ! vfbopenin ( vffile ) ) 
  return ;
  if ( vfbyte () != 247 ) 
...
zwarndestdup ( integer id , smallnumber byname , strnumber s1 , strnumber s2 ) 
{
  ...
  return ;
  pdfwarning ( s1 , 1833 , false , false ) ;
  if ( byname > 0 ) 
  {
    print ( 1819 ) ;
    printmark ( id ) ;
...
zdolink ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  pushlinklevel ( p ) ;
  setrectdimens ( p , parentbox , x , y , mem [p + 1 ].cint , mem [p + 2 ]
  .cint , mem [p + 3 ].cint , eqtb [27818 ].cint ) ;
  objtab [mem [p + 6 ].cint ].int4 = p ;
  {
    pdfappendlistarg = mem [p + 6 ].cint ;
...
endlink ( void ) 
{
  ...
  -1073741824L ) ) 
  {
    ...
    if ( isshippingpage && matrixused () ) 
    {
      matrixrecalculate ( curh + eqtb [27818 ].cint ) ;
      mem [p + 1 ].cint = getllx () - eqtb [27818 ].cint ;
      mem [p + 2 ].cint = curpageheight - getury () - eqtb [27818 ].cint ;
      mem [p + 3 ].cint = geturx () + eqtb [27818 ].cint ;
      mem [p + 4 ].cint = curpageheight - getlly () + eqtb [27818 ].cint ;
    } 
    else mem [p + 3 ].cint = curh + eqtb [27818 ].cint ;
  } 
  poplinklevel () ;
} 
void 
...
endthread ( void ) 
{
  ...
  mem [lastthread + 4 ].cint = curv + eqtb [27820 ].cint ;
  if ( pdflastthreadnamedid ) 
...
zdodest ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  {
    warndestdup ( mem [p + 5 ].hh .v.RH , mem [p + 5 ].hh.b1 , 1846 , 1847 
    ) ;
    return ;
  } 
  objtab [k ].int4 = p ;
...
zpdfshipout ( halfword p , boolean shippingpage ) 
{
  ...
  synctexsheet ( eqtb [27194 ].cint ) ;
  {
    ...
    if ( ! shippingpage ) 
    {
      ...
      curhoffset = eqtb [27814 ].cint + eqtb [27811 ].cint ;
      curvoffset = eqtb [27815 ].cint + eqtb [27812 ].cint ;
      if ( eqtb [27816 ].cint != 0 ) 
      curpagewidth = eqtb [27816 ].cint ;
	...
	pdfprint ( 1136 ) ;
	{
      ...
      pdfindirectln ( 1137 , pdflastresources ) ;
    } 
    pdfbeginstream () ;
    if ( shippingpage ) 
    {
	...
	pdfprint ( 1151 ) ;
	{
      ...
      pdfindirectln ( 1152 , pdflaststream ) ;
      pdfindirectln ( 1137 , pdflastresources ) ;
      pdfprint ( 1153 ) ;
	...
	pdflastpages = objptr ;
      ...
      pdfindirectln ( 1155 , pdflastpages ) ;
...
pdfprintinfo ( void ) 
{
  ...
  if ( pdfinfotoks != -268435455L ) 
  {
    ...
    creatorgiven = substrofstr ( 1207 , s ) ;
    producergiven = substrofstr ( 1208 , s ) ;
    creationdategiven = substrofstr ( 1209 , s ) ;
    moddategiven = substrofstr ( 1210 , s ) ;
    trappedgiven = substrofstr ( 1211 , s ) ;
  } 
  if ( ! producergiven ) 
  {
    pdfprint ( 1216 ) ;
  ...
  pdfstrentryln ( 1215 , pdftexbanner ) ;
  pdfenddict () ;
} 
void 
...
zhpack ( halfword p , scaled w , smallnumber m ) 
{
  ...
  else if ( x > 0 ) 
  {
    ...
    ].hh .v.RH != -268435455L ) ) 
    {
      ...
      eqtb [27203 ].cint < 100 ) ) 
      {
	if ( ( eqtb [27809 ].cint > 0 ) && ( - (integer) x - totalshrink [0 
	]> eqtb [27801 ].cint ) ) 
	{
	  while ( mem [q ].hh .v.RH != -268435455L ) q = mem [q ].hh .v.RH 
	  ;
	  mem [q ].hh .v.RH = newrule () ;
	  mem [mem [q ].hh .v.RH + 1 ].cint = eqtb [27809 ].cint ;
	} 
	println () ;
	printnl ( 1250 ) ;
...
zappendtovlist ( halfword b ) 
{
  ...
  scaled d  ;
  halfword p  ;
  if ( curlist .auxfield .cint > eqtb [27825 ].cint ) 
  {
    d = mem [eqtb [24529 ].hh .v.RH + 1 ].cint - curlist .auxfield .cint - 
    mem [b + 3 ].cint ;
    if ( d < eqtb [27795 ].cint ) 
    p = newparamglue ( 0 ) ;
    else {
...
zmakescripts ( halfword q , scaled delta ) 
{
  ...
  if ( mem [q + 2 ].hh .v.RH == 0 ) 
  {
    ...
    mem [x + 1 ].cint = mem [x + 1 ].cint + eqtb [27805 ].cint ;
    ...
    mem [x + 4 ].cint = - (integer) shiftup ;
      ...
      mem [y + 1 ].cint = mem [y + 1 ].cint + eqtb [27805 ].cint ;
...
zmakeleftright ( halfword q , smallnumber style , scaled maxd , scaled maxh ) 
{
  ...
  delta2 = delta1 + delta1 - eqtb [27803 ].cint ;
  if ( delta < delta2 ) 
  delta = delta2 ;
...
zinitspan ( halfword p ) 
{
  ...
  curlist .auxfield .hh .v.LH = 1000 ;
    ...
    curlist .auxfield .cint = eqtb [27825 ].cint ;
    normalparagraph () ;
  } 
  curspan = p ;
} 
	...
	( curactivewidth [5 ]!= 0 ) ) 
	{
	  if ( dolastlinefit ) 
	  {
	    if ( curp == -268435455L ) 
	    {
	      ...
	      1073741823L ) ;
	      if ( eqtb [27276 ].cint < 1000 ) 
	      g = fract ( g , eqtb [27276 ].cint , 1000 , 1073741823L ) ;
	      if ( aritherror ) {
...
zpostlinebreak ( boolean d ) 
{
  ...
  do {
    ...
    if ( eqtb [27823 ].cint != eqtb [27825 ].cint ) 
    mem [justbox + 3 ].cint = eqtb [27823 ].cint ;
    if ( eqtb [27824 ].cint != eqtb [27825 ].cint ) 
    mem [justbox + 2 ].cint = eqtb [27824 ].cint ;
    if ( ( eqtb [27821 ].cint != eqtb [27825 ].cint ) && ( curline == 
    curlist .pgfield + 1 ) ) 
    mem [justbox + 3 ].cint = eqtb [27821 ].cint ;
    if ( ( eqtb [27822 ].cint != eqtb [27825 ].cint ) && ( curline + 1 == 
    bestline ) ) 
    mem [justbox + 2 ].cint = eqtb [27822 ].cint ;
...
zvsplit ( halfword n , scaled h ) 
{
  ...
  q = vertbreak ( mem [v + 5 ].hh .v.RH , h , eqtb [27799 ].cint ) ;
  p = mem [v + 5 ].hh .v.RH ;
  if ( p == q ) 
  ...
  lab30: ;
  q = prunepagetop ( q , eqtb [27277 ].cint > 0 ) ;
  p = mem [v + 5 ].hh .v.RH ;
  freenode ( v , 9 ) ;
  if ( q != -268435455L ) 
  ...
  Result = vpackage ( p , h , 0 , eqtb [27799 ].cint ) ;
  return Result ;
} 
void 
printtotals ( void ) 
...
zfreezepagespecs ( smallnumber s ) 
{
  ...
  pagecontents = s ;
  pagesofar [0 ]= eqtb [27797 ].cint ;
  pagemaxdepth = eqtb [27798 ].cint ;
  pagesofar [7 ]= 0 ;
  pagesofar [1 ]= 0 ;
  pagesofar [2 ]= 0 ;
  pagesofar [3 ]= 0 ;
...
buildpage ( void ) 
{
  ...
  do {
	...
	if ( mem [r ].hh.b1 != n ) 
	{
	  ...
	  if ( eqtb [27281 + n ].cint == 1000 ) 
	  h = mem [r + 3 ].cint ;
	  else h = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [27281 + n ]
	  .cint ;
	...
	insertpenalties = insertpenalties + mem [p + 1 ].cint ;
	  ...
	  if ( eqtb [27281 + n ].cint == 1000 ) 
	  h = mem [p + 3 ].cint ;
	  else h = xovern ( mem [p + 3 ].cint , 1000 ) * eqtb [27281 + n ]
	  .cint ;
	  if ( ( ( h <= 0 ) || ( h <= delta ) ) && ( mem [p + 3 ].cint + mem 
	  [r + 3 ].cint <= eqtb [27827 + n ].cint ) ) 
	  {
	    ...
	    if ( eqtb [27281 + n ].cint <= 0 ) 
	    w = 1073741823L ;
	    else {
		
	      w = pagesofar [0 ]- pagesofar [1 ]- pagesofar [7 ];
	      if ( eqtb [27281 + n ].cint != 1000 ) 
	      w = xovern ( w , eqtb [27281 + n ].cint ) * 1000 ;
	    } 
	    if ( w > eqtb [27827 + n ].cint - mem [r + 3 ].cint ) 
	    w = eqtb [27827 + n ].cint - mem [r + 3 ].cint ;
...
itsallover ( void ) 
{
  ...
  if ( privileged () ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [27796 ].cint ;
    {
...
normalparagraph ( void ) 
{
  ...
  if ( eqtb [27810 ].cint != 0 ) 
  eqworddefine ( 27810 , 0 ) ;
...
znewgraf ( boolean indented ) 
{
  ...
  if ( indented ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [27793 ].cint ;
...
initmath ( void ) 
{
  ...
  if ( ( curcmd == 3 ) && ( curlist .modefield > 0 ) ) 
  {
    ...
    if ( eqtb [25058 ].hh .v.RH == -268435455L ) {
      ...
      if ( ( eqtb [27810 ].cint != 0 ) && ( ( ( eqtb [27218 ].cint >= 0 ) 
      && ( curlist .pgfield + 2 > eqtb [27218 ].cint ) ) || ( curlist 
      .pgfield + 1 < - (integer) eqtb [27218 ].cint ) ) ) 
      {
	l = eqtb [27796 ].cint - abs ( eqtb [27810 ].cint ) ;
	if ( eqtb [27810 ].cint > 0 ) 
	s = eqtb [27810 ].cint ;
	else s = 0 ;
      } 
    ...
    eqworddefine ( 27806 , w ) ;
    curlist .eTeXauxfield = j ;
    if ( ( eTeXmode == 1 ) ) 
    eqworddefine ( 27275 , x ) ;
    eqworddefine ( 27807 , l ) ;
...
aftermath ( void ) 
{
  ...
  {
    {
      mem [curlist .tailfield ].hh .v.RH = newmath ( eqtb [27794 ].cint , 
      0 ) ;
      curlist .tailfield = mem [curlist .tailfield ].hh .v.RH ;
    } 
    curmlist = p ;
...
zcheckpdfoutput ( strnumber s , boolean iserror ) 
{
  ...
  if ( eqtb [27239 ].cint <= 0 ) 
  {
    if ( iserror ) 
    pdferror ( s , 1767 ) ;
    else pdfwarning ( s , 1768 , true , true ) ;
  } 
} 
void 
scanpdfexttoks ( void ) 
...
zscaleimage ( integer n ) 
{
  ...
  if ( ( xr > 65535L ) || ( yr > 65535L ) ) 
  {
    xr = 0 ;
    yr = 0 ;
    pdfwarning ( 1766 , 1798 , true , true ) ;
  } 
...
scanimage ( void ) 
{
  ...
  {
    pdfwarning ( 1807 , 1808 , true , true ) ;
  ...
  {
    pdfwarning ( 1807 , 1809 , true , true ) ;
  ...
  if ( eqtb [27249 ].cint > 0 ) 
  {
    if ( warnpdfpagebox ) 
    {
      pdfwarning ( 1807 , 1810 , true , true ) ;
      warnpdfpagebox = false ;
    } 
...
zpdffixthread ( integer thread ) 
{
  pdffixthread_regmem 
  halfword a  ;
  pdfwarning ( 1815 , 1870 , false , false ) ;
  if ( objtab [thread ].int0 < 0 ) 
  {
    print ( 1186 ) ;
...
doextension ( void ) 
{
  ...
  case 7 : 
	...
	if ( scankeyword ( 1789 ) ) 
	{
	  ...
	  if ( ( k <= 0 ) || ( k > objptr ) || ( objtab [k ].int4 != 0 ) ) 
	  {
	    pdfwarning ( 1787 , 1790 , true , true ) ;
	    pdfretval = -1 ;
	    k = -1 ;
	  } 
	} 
...
maincontrol ( void ) 
{
  ...
  case 137 : 
    if ( curchr > 0 ) 
    {
      if ( eTeXenabled ( ( eqtb [27279 ].cint > 0 ) , curcmd , curchr ) ) 
      {
  ...
  if ( fixedpdfoutput > 0 ) 
  {
	...
	if ( fixedpdfdraftmode == 0 ) 
	{
	  ...
	  while ( objtab [k ].int4 == 0 ) {
	    ...
	    pdfwarning ( 1185 , 1190 , false , false ) ;
	    printint ( objtab [k ].int0 ) ;
	    print ( 1191 ) ;
	    println () ;
	    println () ;
	  ...
	  for_end) do 
		...
		pdfmarkchar ( i , j ) ;
	      ...
	      pdffontattr [i ], pdffontattr [k ]) ) 
	      {
		pdfwarning ( 1192 , 1193 , false , false ) ;
	  ...
	  if ( pdffirstoutline != 0 ) 
	  {
	    ...
	    pdfindirectln ( 1178 , pdffirstoutline ) ;
	    pdfindirectln ( 1179 , pdflastoutline ) ;
	    pdfintentryln ( 1180 , k ) ;
	    pdfenddict () ;
	    k = headtab [4 ];
	    while ( k != 0 ) {
		...
		pdflastoutline = k ;
	      ...
	      pdfbegindict ( k , 1 ) ;
	      pdfindirectln ( 1181 , pdfmem [objtab [k ].int4 ]) ;
	      pdfindirectln ( 65 , pdfmem [objtab [k ].int4 + 6 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 1 ]!= 0 ) 
	      pdfindirectln ( 1155 , pdfmem [objtab [k ].int4 + 1 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 2 ]!= 0 ) 
	      pdfindirectln ( 1182 , pdfmem [objtab [k ].int4 + 2 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 3 ]!= 0 ) 
	      pdfindirectln ( 1183 , pdfmem [objtab [k ].int4 + 3 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 4 ]!= 0 ) 
	      pdfindirectln ( 1178 , pdfmem [objtab [k ].int4 + 4 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 5 ]!= 0 ) 
	      pdfindirectln ( 1179 , pdfmem [objtab [k ].int4 + 5 ]) ;
	      if ( objtab [k ].int0 != 0 ) 
	      pdfintentryln ( 1180 , objtab [k ].int0 ) ;
	  ...
	  if ( threads != 0 ) 
	  pdfindirectln ( 1203 , threads ) ;
	  if ( outlines != 0 ) 
	  pdfindirectln ( 1204 , outlines ) ;
	  if ( namestree != 0 ) 
	  pdfindirectln ( 1205 , namestree ) ;
	  ...
	  if ( pdfcatalogopenaction != 0 ) 
	  pdfindirectln ( 1206 , pdfcatalogopenaction ) ;
	  pdfenddict () ;
	  pdfprintinfo () ;
	  if ( pdfosenable ) 
	  {
	    ...
	    pdfindirectln ( 1227 , root ) ;
	    pdfindirectln ( 1228 , objptr - 1 ) ;
	  ...
	  if ( ! pdfosenable ) 
	  {
	    ...
	    pdfprint ( 1230 ) ;
	    pdfintentryln ( 1224 , sysobjptr + 1 ) ;
	    pdfindirectln ( 1227 , root ) ;
	...
	if ( fixedpdfdraftmode == 0 ) 
	bclose ( pdffile ) ;
	else pdfwarning ( 0 , 1184 , true , true ) ;
      } 
      if ( logopened ) 
      {
	putc ('\n',  logfile );
