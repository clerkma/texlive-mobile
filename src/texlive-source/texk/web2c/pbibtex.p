...
161:begin if((exbuf[exbufptr+1]=164)or(exbuf[exbufptr+1]=162))then begin
precedingwhite:=false;
andfound:=true end else if(exbuf[exbufptr+1]=161)then begin exbuf[
exbufptr]:=32;exbuf[exbufptr+1]:=32;precedingwhite:=true;end;
exbufptr:=exbufptr+2;end;123:begin bracelevel:=bracelevel+1;
exbufptr:=exbufptr+1;
...
precedingwhite:=true;
end else begin if(exbuf[exbufptr]>127)then exbufptr:=exbufptr+2 else
exbufptr:=exbufptr+1;precedingwhite:=false;end end;
checkbracelevel(poplitvar);end;
...
then begin if svbuffer[namebfptr]>127 then begin begin if(exbufptr=
bufsize)then bufferoverflow;begin exbuf[exbufptr]:=svbuffer[namebfptr];
exbufptr:=exbufptr+1;end;end;namebfptr:=namebfptr+1;
...
163,165,169,170:if(strpool[spptr-1]=161)then begin if(litstack[litstkptr
]>=cmdstrptr)then begin strptr:=strptr+1;poolptr:=strstart[strptr];end;
litstkptr:=litstkptr+1;
...
if(exbufptr<exbuflength)then begin if((exbuf[exbufptr-1]=164)or(exbuf[
exbufptr-1]=162))then exbufptr:=exbufptr-2 else exbufptr:=exbufptr-4;
end;
...
while(tps<spptr)do begin if strpool[tps]>127 then tps:=tps+2 else tps:=
tps+1;end;tpe:=tps;
while(tpe<spend)do begin if strpool[tpe]>127 then tpe:=tpe+2 else tpe:=
tpe+1;end;
if tps<>spptr then begin if tps>strstart[poplit3]then spptr:=spptr-1
...
while(spptr<spend)do begin if strpool[spptr]>127 then begin begin
strpool[poolptr]:=strpool[spptr];poolptr:=poolptr+1;end;spptr:=spptr+1;
begin strpool[poolptr]:=strpool[spptr];poolptr:=poolptr+1;end;
spptr:=spptr+1;end else begin begin strpool[poolptr]:=strpool[spptr];
poolptr:=poolptr+1;end;spptr:=spptr+1;end;end;pushlitstk(makestring,1);
...
end else if(strpool[spxptr1-1]>127)then begin spxptr1:=spxptr1+1;
numtextchars:=numtextchars+2;end else numtextchars:=numtextchars+1;end;
...
poplit1);end;{:455}{482:}procedure xiskanjistr;label 10;
begin poplitstk(poplit1,poptyp1);
if poptyp1<>1 then begin printwrongstklit(poplit1,poptyp1,1);
...
10:end;{:482}{326:}procedure executefn(exfnloc:hashloc);{344:}label 51;
...
begin if(not setencstring(nil,'EUC'))then uexit(1);{103:}initkanji;
parsearguments;
...
{:155}{:218}{:12}{480:}procedure setupparams;var bounddefault:integer;
...
end;{:480}{481:}procedure computehashprime;var hashwant:integer;
...
{:481}{13:}procedure initialize;var{23:}i:integer;{:23}{67:}k:hashloc;
...
xchr[9]:=chr(9);for i:=128 to 159 do xchr[i]:=chr(i-128);
for i:=160 to 255 do xchr[i]:=chr(i);xchr[27]:=chr(27);
{:27}{28:}for i:=0 to 255 do xord[i]:=127;for i:=1 to 126 do xord[i]:=i;
for i:=128 to 159 do xord[i]:=i-128;for i:=160 to 255 do xord[i]:=i;
...
for i:=97 to 122 do lexclass[i]:=2;for i:=128 to 159 do lexclass[i]:=0;
for i:=160 to 255 do lexclass[i]:=2;lexclass[27]:=2;
{:32}{33:}for i:=0 to 255 do idclass[i]:=1;
for i:=0 to 31 do idclass[i]:=0;for i:=128 to 159 do idclass[i]:=0;
idclass[27]:=1;idclass[32]:=0;idclass[9]:=0;idclass[34]:=0;
idclass[35]:=0;idclass[37]:=0;idclass[39]:=0;idclass[40]:=0;
idclass[41]:=0;idclass[44]:=0;idclass[61]:=0;idclass[123]:=0;
idclass[125]:=0;{:33}{35:}for i:=0 to 127 do charwidth[i]:=0;
charwidth[32]:=278;charwidth[33]:=278;charwidth[34]:=500;
charwidth[35]:=833;charwidth[36]:=500;charwidth[37]:=833;
charwidth[38]:=778;charwidth[39]:=278;charwidth[40]:=389;
charwidth[41]:=389;charwidth[42]:=500;charwidth[43]:=778;
charwidth[44]:=278;charwidth[45]:=333;charwidth[46]:=278;
charwidth[47]:=500;charwidth[48]:=500;charwidth[49]:=500;
charwidth[50]:=500;charwidth[51]:=500;charwidth[52]:=500;
charwidth[53]:=500;charwidth[54]:=500;charwidth[55]:=500;
charwidth[56]:=500;charwidth[57]:=500;charwidth[58]:=278;
charwidth[59]:=278;charwidth[60]:=278;charwidth[61]:=778;
charwidth[62]:=472;charwidth[63]:=472;charwidth[64]:=778;
charwidth[65]:=750;charwidth[66]:=708;charwidth[67]:=722;
charwidth[68]:=764;charwidth[69]:=681;charwidth[70]:=653;
charwidth[71]:=785;charwidth[72]:=750;charwidth[73]:=361;
charwidth[74]:=514;charwidth[75]:=778;charwidth[76]:=625;
charwidth[77]:=917;charwidth[78]:=750;charwidth[79]:=778;
charwidth[80]:=681;charwidth[81]:=778;charwidth[82]:=736;
charwidth[83]:=556;charwidth[84]:=722;charwidth[85]:=750;
charwidth[86]:=750;charwidth[87]:=1028;charwidth[88]:=750;
charwidth[89]:=750;charwidth[90]:=611;charwidth[91]:=278;
charwidth[92]:=500;charwidth[93]:=278;charwidth[94]:=500;
charwidth[95]:=278;charwidth[96]:=278;charwidth[97]:=500;
charwidth[98]:=556;charwidth[99]:=444;charwidth[100]:=556;
charwidth[101]:=444;charwidth[102]:=306;charwidth[103]:=500;
charwidth[104]:=556;charwidth[105]:=278;charwidth[106]:=306;
charwidth[107]:=528;charwidth[108]:=278;charwidth[109]:=833;
charwidth[110]:=556;charwidth[111]:=500;charwidth[112]:=556;
charwidth[113]:=528;charwidth[114]:=392;charwidth[115]:=394;
charwidth[116]:=389;charwidth[117]:=556;charwidth[118]:=528;
charwidth[119]:=722;charwidth[120]:=528;charwidth[121]:=528;
charwidth[122]:=444;charwidth[123]:=500;charwidth[124]:=1000;
charwidth[125]:=500;charwidth[126]:=500;
for i:=160 to 254 do charwidth[i]:=514;
...
const noptions=6;var longoptions:array[0..noptions]of getoptstruct;
...
longoptions[currentoption].hasarg:=1;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;currentoption:=currentoption+1;
{:478}{479:}longoptions[currentoption].name:=0;
longoptions[currentoption].hasarg:=0;longoptions[currentoption].flag:=0;
longoptions[currentoption].val:=0;{:479};
repeat getoptreturnval:=getoptlongonly(argc,argv,'',longoptions,
addressof(optionindex));if getoptreturnval=-1 then begin;
end else if getoptreturnval=63 then begin usage('pbibtex');
...
usagehelp(PBIBTEXHELP,nil);
end else if(strcmp(longoptions[optionindex].name,'version')=0)then begin
printversionandexit('This is pBibTeX, Version 0.99d-j0.33',
...
if(optind+1<>argc)then begin writeln(stderr,'pbibtex',
': Need exactly one file argument.');usage('pbibtex');end;end;
...
'This is pBibTeX, Version 0.99d-j0.33');
write(standardoutput,'This is pBibTeX, Version 0.99d-j0.33');end;
...
'This is pBibTeX, Version 0.99d-j0.33');end;
