...
#include "gftodmem.h"
typedef unsigned char internalfontnumber  ;
typedef integer poolpointer  ;
typedef integer strnumber  ;
typedef unsigned char keywordcode  ;
...
{
  integer i, j, m, n  ;
  kpsesetprogramname ( argv [0 ], "gftodvi" ) ;
...
opengffile ( void ) 
{
  gffile = kpseopenfile ( stringcast ( nameoffile ) , kpsegfformat ) ;
  curloc = 0 ;
} 
void 
opentfmfile ( void ) 
{
  tfmfile = kpseopenfile ( stringcast ( nameoffile ) , kpsetfmformat ) ;
} 
void 
opendvifile ( void ) 
{
  rewritebin ( dvifile , stringcast ( nameoffile ) ) ;
} 
void 
readtfmword ( void ) 
{
...
zpackfilename ( strnumber n , strnumber a , strnumber e ) 
{
  ...
  nameoffile = xmallocarray ( ASCIIcode , namelength ) ;
  k = -1 ;
...
startgf ( void ) 
{
  ...
  0 ) ) {
    ...
    buffer [linelength ]= xord [ucharcast ( argbuffer [argbufptr ]) ];
    linelength = linelength + 1 ;
  ...
  while ( k < maxk ) {
	...
	if ( j .B1 == curr ) {
	  ...
	  if ( j .B0 - 0 <= 128 ) {
	    ...
	    if ( j .B2 - 0 >= 128 ) 
	    {
	      kernamount = fontinfo [kernbase [f ]+ 256 * ( j .B2 - 128 ) + 
	      j .B3 ].sc ;
	      goto lab30 ;
	    } 
	    else {
...
{
  /* 10 40 */ register boolean Result; unsigned char oct  ;
  nodepointer dfl  ;
  hbox ( info [p ], 2 , false ) ;
  dvix = xx [p ];
  dviy = yy [p ];
