...
#define maxfont ( 1000 ) 
#define xmaxfont ( 1001 ) 
#define xxmaxfont ( 1002 ) 
...
xchartype bchar  ;
integer fontdir  ;
bytetype vf[vfsize + 1]  ;
integer vfptr  ;
integer vtitlestart  ;
bytetype vtitlelength  ;
integer packetstart[maxchar + 1]  ;
integer packetlength[maxchar + 1]  ;
xfonttype fontptr  ;
xfonttype curfont  ;
integer fnamestart[maxfont + 1]  ;
bytetype fnamelength[maxfont + 1]  ;
integer fareastart[maxfont + 1]  ;
bytetype farealength[maxfont + 1]  ;
fourbytes fontchecksum[maxfont + 1]  ;
integer fontnumber[xmaxfont + 1]  ;
fixword fontat[maxfont + 1]  ;
fixword fontdsize[maxfont + 1]  ;
...
constcstring HEX  ;
integer dig[33]  ;
unsigned char hstack[maxstack + 1]  ;
unsigned char vstack[maxstack + 1]  ;
...
integer inumber  ;
cinttype verbose  ;
constcstring vplname, tfmname, vfname  ;
...
#define noptions ( 3 ) 
  ...
  argc ) ) 
  {
    fprintf ( stderr , "%s%s\n",  "ovp2ovf" , ": Need one to three file arguments." ) ;
    usage ( "ovp2ovf" ) ;
  } 
  vplname = extendfilename ( cmdline ( optind ) , "ovp" ) ;
  if ( optind + 2 <= argc ) 
  {
    vfname = extendfilename ( cmdline ( optind + 1 ) , "ovf" ) ;
    if ( optind + 3 <= argc ) 
    {
      tfmname = extendfilename ( cmdline ( optind + 2 ) , "ofm" ) ;
    } 
    else {
	
      tfmname = makesuffix ( cmdline ( optind + 1 ) , "ofm" ) ;
    } 
  } 
  else {
      
    vfname = basenamechangesuffix ( vplname , ".ovp" , ".ovf" ) ;
    tfmname = basenamechangesuffix ( vplname , ".ovp" , ".ofm" ) ;
  } 
} 
void 
initialize ( void ) 
{
  ...
  headerindex d  ;
  integer c  ;
  kpsesetprogramname ( argv [0 ], "ovp2ovf" ) ;
  parsearguments () ;
  reset ( vplfile , vplname ) ;
  if ( verbose ) 
  {
    Fputs ( stderr ,  "This is OVP2OVF, Version 1.13" ) ;
    fprintf ( stderr , "%s\n",  versionstring ) ;
  } 
  ...
  vtitlelength = 0 ;
  fontptr = 0 ;
  {register integer for_end; k = 0 ;for_end = maxchar ; if ( k <= for_end) 
  do 
  ...
  {register integer for_end; k = 128 ;for_end = 255 ; if ( k <= for_end) do 
    packetlength [k ]= 2 ;
  while ( k++ < for_end ) ;} 
  {register integer for_end; k = 256 ;for_end = 65535L ; if ( k <= for_end) 
  do 
    packetlength [k ]= 3 ;
  while ( k++ < for_end ) ;} 
  {register integer for_end; k = 65536L ;for_end = maxchar ; if ( k <= 
  for_end) do 
    packetlength [k ]= 4 ;
  while ( k++ < for_end ) ;} 
...
fillbuffer ( void ) 
{
  ...
  if ( eof ( vplfile ) ) 
  {
    ...
    while ( ( limit < bufsize - 2 ) && ( ! eoln ( vplfile ) ) ) {
...
gethex ( void ) 
{
  register bytetype Result; integer a  ;
  do {
      getnext () ;
  } while ( ! ( curchar != 32 ) ) ;
  a = curchar - 41 ;
  ...
  {
    {
      if ( charsonline > 0 ) 
      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
      Fputs ( stderr ,  "Illegal hexadecimal digit" ) ;
      showerrorcontext () ;
    } 
    Result = 0 ;
  } 
  ...
  while ( true ) {
	...
	} 
	{
	  ...
	  if ( vfptr == vfsize ) 
	  {
	    if ( charsonline > 0 ) 
	    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	    Fputs ( stderr ,  "I'm out of memory---increase my vfsize!" ) ;
	    showerrorcontext () ;
	  } 
	  else vfptr = vfptr + 1 ;
	} 
...
getfourbytes ( void ) 
{
  ...
  r = 10 ;
      ...
      
    {
      if ( charsonline > 0 ) 
      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
      Fputs ( stderr ,        "Decimal (\"D\"), octal (\"O\"), or hex (\"H\") value needed here" ) ;
      showerrorcontext () ;
    } 
    do {
	getnext () ;
...
zvfoutset ( integer x ) 
{
  ...
  if ( ( x < 256 ) && ( x >= 0 ) ) 
  {
	...
	putbyte ( x / 16777216L , vffile ) ;
	  ...
	  x = x + 1073741824L ;
	  x = x + 1073741824L ;
	  putbyte ( ( x / 16777216L ) + 128 , vffile ) ;
	  x = x % 16777216L ;
	} 
...
void 
zvffix ( bytetype opcode , fixword x ) 
{
  boolean negative  ;
  unsigned char k  ;
  integer t  ;
...
zreadpacket ( integer c ) 
{
  ...
  while ( level == 2 ) {
    ...
    if ( curchar == 40 ) 
    {
      ...
      if ( curcode == 0 ) 
      skiptoendofitem () ;
      else if ( ( curcode < 210 ) || ( curcode > 220 ) ) 
      {
	{
	  if ( charsonline > 0 ) 
	  { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	  Fputs ( stderr ,  "This property name doesn't belong in a MAP list" ) 
	  ;
	  showerrorcontext () ;
	} 
	skiptoendofitem () ;
	...
	{case 210 : 
	  {
	    fontnumber [fontptr ]= getinteger () ;
	    curfont = 0 ;
	    while ( fontnumber [fontptr ]!= fontnumber [curfont ]) curfont 
	    = curfont + 1 ;
	    if ( curfont == fontptr ) 
	    {
	      if ( charsonline > 0 ) 
	      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	      Fputs ( stderr ,  "Undefined MAPFONT cannot be selected" ) ;
	      showerrorcontext () ;
	    } 
	    else vfstorefnt ( curfont ) ;
	  } 
	  break ;
	case 211 : 
	  if ( curfont == fontptr ) 
	  {
	    if ( charsonline > 0 ) 
	    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	    Fputs ( stderr ,  "Character cannot be typeset in undefined font" ) 
	    ;
	    showerrorcontext () ;
	  } 
	  else {
	...
	case 217 : 
	  if ( stackptr == maxstack ) 
	  {
	    if ( charsonline > 0 ) 
	    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	    Fputs ( stderr ,  "Don't push so much---stack is full!" ) ;
	    showerrorcontext () ;
	  } 
	  else {
	...
	case 218 : 
	  if ( stackptr == 0 ) 
	  {
	    if ( charsonline > 0 ) 
	    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	    Fputs ( stderr ,  "Empty stack cannot be popped" ) ;
	    showerrorcontext () ;
	  } 
	  else {
	...
	case 220 : 
	  {
	      ...
	      {
		{
		  if ( charsonline > 0 ) 
		  { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		  Fputs ( stderr ,                    "Special command being clipped---no room left!" ) ;
		  showerrorcontext () ;
		} 
  ...
  while ( stackptr > 0 ) {
      
    {
      if ( charsonline > 0 ) 
      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
      Fputs ( stderr ,  "Missing POP supplied" ) ;
      showerrorcontext () ;
    } 
    {
      vf [vfptr ]= 142 ;
	...
	case 3 : 
	  {
	    ...
	    else if ( frozendu ) 
	    {
	      if ( charsonline > 0 ) 
	      { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
	      Fputs ( stderr ,                "Sorry, it's too late to change the design units" ) ;
	      showerrorcontext () ;
	    } 
	...
	case 12 : 
	  {
		...
		if ( charsonline > 0 ) 
		{ putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		Fputs ( stderr ,  "VTITLE clipped to 255 characters" ) ;
		showerrorcontext () ;
	      } 
	      vtitlelength = 255 ;
	    } 
	...
	case 13 : 
	  {
		...
		if ( charsonline > 0 ) 
		{ putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		Fputs ( stderr ,  "Sorry, too many different mapfonts" ) ;
		showerrorcontext () ;
	      } 
	    } 
	    if ( curfont == fontptr ) 
		...
		if ( curcode == 0 ) 
		skiptoendofitem () ;
		else if ( ( curcode < 200 ) || ( curcode > 204 ) ) 
		{
		  {
		    if ( charsonline > 0 ) 
		    { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
		    Fputs ( stderr ,                      "This property name doesn't belong in a MAPFONT list" ) ;
		    showerrorcontext () ;
		  } 
		  skiptoendofitem () ;
		} 
		  ...
		  {case 200 : 
		      ...
		      {
			{
			  if ( charsonline > 0 ) 
			  { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
			  Fputs ( stderr ,                            "FONTNAME clipped to 255 characters" ) ;
			  showerrorcontext () ;
			} 
		  ...
		  case 201 : 
		      ...
		      {
			{
			  if ( charsonline > 0 ) 
			  { putc ( ' ' ,  stderr );  putc ( '\n',  stderr ); }
			  Fputs ( stderr ,                            "FONTAREA clipped to 255 characters" ) ;
			  showerrorcontext () ;
			} 
